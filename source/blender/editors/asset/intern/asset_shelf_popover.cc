...
/**
 * Resolve the #RegionAssetShelf of the asset-shelf region in the active area, if any.
 * \param ensure: If true, create the regiondata for the shelf region when it doesn't exist yet.
 */
static RegionAssetShelf *area_shelf_regiondata_from_context(const bContext &C, const bool ensure)
{
  ...
  return ensure ? RegionAssetShelf::ensure_from_asset_shelf_region(*shelf_region) :
...
{
  if (RegionAssetShelf *shelf_regiondata = area_shelf_regiondata_from_context(C, false)) {
    for (AssetShelf &shelf : shelf_regiondata->shelves) {
      if (shelf.is_popup && STREQ(shelf.idname, shelf_type.idname)) {
  ...
  /* Fallback: popovers can be invoked without an area in context (e.g. from menus that have not
   * yet propagated area context). Such shelves are stored in a global registry. */
  for (AssetShelf *shelf : StaticPopupShelves::shelves()) {
...
static AssetShelf *get_shelf_for_popup(const bContext &C, AssetShelfType &shelf_type)
{
  ...
  /* Prefer area-local storage so the popup shelf lives with the area; fall back to the global
   * registry when there is no area context (see #lookup_shelf_for_popup). */
  if (RegionAssetShelf *shelf_regiondata = area_shelf_regiondata_from_context(C, true)) {
    BLI_addtail(&shelf_regiondata->shelves, new_shelf);
  }
  else {
    StaticPopupShelves::shelves().append(new_shelf);
  }
  return new_shelf;
}
