...
static ImagePaintBrushStrokeSession *image_paint_brush_session_ensure(StrokeCache &cache)
{
  ...
  g_image_paint_brush_stroke_sessions.add(&cache, std::make_unique<ImagePaintBrushStrokeSession>());
                    ...
                    const IndexMask &texnode_mask)
{
  ...
  if (SCULPT_use_image_paint_brush(paint_mode_settings, ob)) {
    ...
    const PaintImageBrushSessionStepResult session_step_result = try_paint_image_brush_session_step(
        *ss.cache, static_context, step_state);
    ...
    update_simple_image_brush_step_policy(
        *ss.cache, SCULPT_do_paint_brush_image(depsgraph, paint_mode_settings, sd, ob, texnode_mask));
    return;
  }
