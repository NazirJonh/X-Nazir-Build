...
class SculptImageGradientBackend : public gradient::session::Backend {
 public:
                     ...
                     const gradient::session::DynamicState &dynamic_state) override
  {
    ...
    SCULPT_image_paint_push_undo_tiles(
        *context_->depsgraph, *context_->paint_mode_settings, *context_->ob, pixels_node_data_.mask);

    if (context_->interactive_preview) {
      MutableSpan<bke::pbvh::MeshNode> nodes = pbvh->nodes<bke::pbvh::MeshNode>();
      int64_t tile_slots_num = 0;
      for (const int node_index : pixels_node_data_.indices) {
        const bke::pbvh::pixels::NodeData &node_data = bke::pbvh::pixels::node_data_get(nodes[node_index]);
  ...
  void restore_baseline(const gradient::session::DynamicState &dynamic_state) override
  {
    ...
    auto restore_snapshot_to_buffer = [&](const RowSnapshot &snapshot,
                                          ImBuf &image_buffer,
                                          const bool strict_storage_match) {
      const int pixel_offset =
          (int(snapshot.start_image_coordinate.y) * image_buffer.x + int(snapshot.start_image_coordinate.x)) * 4;
  ...
  void apply_preview(const gradient::session::DynamicState &dynamic_state) override
  {
    ...
    last_tick_had_updates_ = SCULPT_do_paint_brush_image_gradient(*context_->depsgraph,
 ...
 private:
  ...
  void capture_baseline(bke::pbvh::Tree &pbvh)
  {
      ...
      for (const bke::pbvh::pixels::UDIMTilePixels &tile_data : node_data.tiles) {
        ...
        if (image_buffer->float_buffer.data != nullptr) {
          for (const bke::pbvh::pixels::PackedPixelRow &pixel_row : tile_data.pixel_rows) {
            ...
            const int pixel_offset =
                (int(pixel_row.start_image_coordinate.y) * image_buffer->x +
    ...
    if (context_ != nullptr && context_->interactive_preview) {
      std::sort(
          baseline_rows_.begin(), baseline_rows_.end(), [](const RowSnapshot &a, const RowSnapshot &b) {
  ...
  void collect_dirty_regions_for_next_restore(const int64_t source_tick_version)
  {
    ...
    for (const image::session::DirtyTileRegion &dirty_tile_region : dirty_regions) {
      ...
      const auto first_y_it = std::lower_bound(
          row_begin_it,
          row_end_it,
      ...
      const auto last_y_it = std::lower_bound(
          first_y_it,
          row_end_it,
...
class SculptColorBackend : public gradient::session::Backend {
 public:
                     ...
                     const gradient::session::DynamicState &dynamic_state) override
  {
    ...
    const Span<float3> positions = bke::pbvh::vert_positions_eval(*context_->depsgraph, *context_->ob);
    ...
    node_mask.foreach_index([&](const int node_index) {
      ...
      fill_factor_from_hide_and_mask(attribute_data.hide_vert, attribute_data.mask, node_verts, factors);
  ...
  void apply_preview(const gradient::session::DynamicState &dynamic_state) override
  {
    ...
    for (const NodeSnapshot &snapshot : snapshots_) {
      for (const int i : snapshot.verts.index_range()) {
        ...
        factor = paint_gradient_finalize_factor(*context_->brush, factor, clamp_to_range, brush_alpha);
...
static SculptColorGradientSessionData *sculpt_color_gradient_session_data_ensure(const Scene &scene,
                                                                                  wmOperator *op,
                                                                                  Object &ob,
                                                                                  Depsgraph &depsgraph,
                                                                                  ARegion &region,
                                                                                  ...
                                                                                  const bool use_image_backend)
{
  ...
  session_data->handle = std::make_unique<gradient::session::Handle>(std::move(backend), static_context);
  return session_data;
}
...
static wmOperatorStatus sculpt_color_gradient_exec(bContext *C, wmOperator *op)
{
  ...
  if (is_interactive) {
    ...
    if (session_data->has_last_gradient_params &&
        gradient_params_equal_with_screen_epsilon(session_data->last_gradient_params, gradient_params))
    {
      return OPERATOR_FINISHED;
    }
  ...
  const int64_t debug_tick_version =
      is_interactive ? (session_data != nullptr ? session_data->tick_version : 1) : dynamic_state.tick_version;
...
void SCULPT_OT_color_gradient(wmOperatorType *ot)
{
  ...
  paint_gradient_operator_properties(
      ot, WPAINT_GRADIENT_TYPE_LINEAR, PAINT_GRADIENT_SPACE_SCREEN);
