...
static void rna_AssetShelf_reorder_operator_get(PointerRNA *ptr, char *value)
{
  AssetShelf *shelf = static_cast<AssetShelf *>(ptr->data);
  strcpy(value, shelf->type->reorder_operator.c_str());
}

static int rna_AssetShelf_reorder_operator_length(PointerRNA *ptr)
{
  AssetShelf *shelf = static_cast<AssetShelf *>(ptr->data);
  return shelf->type->reorder_operator.size();
}

static void rna_AssetShelf_reorder_operator_set(PointerRNA *ptr, const char *value)
{
  AssetShelf *shelf = static_cast<AssetShelf *>(ptr->data);
  shelf->type->reorder_operator = value;
}

static void rna_AssetShelf_reorder_direction_operator_get(PointerRNA *ptr, char *value)
{
  AssetShelf *shelf = static_cast<AssetShelf *>(ptr->data);
  strcpy(value, shelf->type->reorder_direction_operator.c_str());
}

static int rna_AssetShelf_reorder_direction_operator_length(PointerRNA *ptr)
{
  AssetShelf *shelf = static_cast<AssetShelf *>(ptr->data);
  return shelf->type->reorder_direction_operator.size();
}

static void rna_AssetShelf_reorder_direction_operator_set(PointerRNA *ptr, const char *value)
{
  AssetShelf *shelf = static_cast<AssetShelf *>(ptr->data);
  shelf->type->reorder_direction_operator = value;
}
...
{
  AssetShelf *shelf = static_cast<AssetShelf *>(ptr->data);
  const AssetLibraryReference new_ref = ed::asset::library_reference_from_enum_value(value);
  ed::asset::shelf::settings_swap_asset_library(*shelf, new_ref);
}
 ...
 */
static void rna_AssetShelf_popup_size_update(bContext *C, PointerRNA *ptr)
{
  AssetShelf *shelf = static_cast<AssetShelf *>(ptr->data);
  if (!shelf->is_popup || shelf->idname[0] == '\0' || !C) {
    return;
  }
...
/**
 * Update callback for the "Recent Brushes" limit. Persists the new value into the User Preferences
 * and immediately trims the in-memory recent list (and the JSON file) so the Recent pseudo-catalog
 * never shows more entries than the configured limit.
 */
static void rna_AssetShelf_recent_max_count_update(Main * /*bmain*/,
                                                   Scene * /*scene*/,
                                                   PointerRNA *ptr)
{
  AssetShelf *shelf = static_cast<AssetShelf *>(ptr->data);
  if (shelf->is_popup && shelf->idname[0] != '\0') {
    BKE_preferences_asset_shelf_popup_view_store(&U,
                                                 ...
                                                 shelf->settings.recent_max_count);
    U.runtime.is_dirty = true;
    ed::asset::shelf::shelf_asset_lists_recent_max_count_set(shelf->idname,
                                                       shelf->settings.recent_max_count);
  }
...
static void rna_def_uigrid(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "bl_reorder_operator", PROP_STRING, PROP_NONE);
  RNA_def_property_string_sdna(prop, nullptr, "type->reorder_operator");
  RNA_def_property_flag(prop, PROP_REGISTER_OPTIONAL);
  RNA_def_property_ui_text(
      prop,
      "Reorder Operator",
      "Operator run to reorder an item via Shift-drag. Receives target_identifier (string) and "
      "drop_location ('BEFORE'/'AFTER') in addition to source_identifier (string)");
...
static void rna_def_asset_shelf(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "bl_reorder_operator", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop,
                                "rna_AssetShelf_reorder_operator_get",
                                "rna_AssetShelf_reorder_operator_length",
                                "rna_AssetShelf_reorder_operator_set");
  RNA_def_property_flag(prop, PROP_REGISTER_OPTIONAL);
  RNA_def_property_ui_text(
      prop,
      "Reorder Operator",
      "Operator to call when Shift-dragging an item to reorder it within this shelf. Receives "
      "target_identifier (string) and drop_location ('BEFORE'/'AFTER') in addition to the usual "
      "asset reference properties");

  prop = RNA_def_property(srna, "bl_reorder_direction_operator", PROP_STRING, PROP_NONE);
  RNA_def_property_string_funcs(prop,
                                "rna_AssetShelf_reorder_direction_operator_get",
                                "rna_AssetShelf_reorder_direction_operator_length",
                                "rna_AssetShelf_reorder_direction_operator_set");
  RNA_def_property_flag(prop, PROP_REGISTER_OPTIONAL);
  RNA_def_property_ui_text(
      prop,
      "Reorder Direction Operator",
      "Operator to call from the item context menu's Move Left/Right and Reorder to Front/Back "
      "entries. Receives a 'direction' enum ('LEFT'/'RIGHT'/'FRONT'/'BACK') in addition to the "
      "usual asset reference properties. Separate from bl_reorder_operator, which uses a "
      "target-identifier calling convention for Shift-drag reordering instead");
  ...
  RNA_def_property_update(prop, NC_SPACE | ND_REGIONS_ASSET_SHELF, "rna_AssetShelf_asset_library_update");
  ...
  prop = RNA_def_property(srna, "show_favorite_icons", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_negative_sdna(
      prop, nullptr, "settings.display_flag", ASSETSHELF_HIDE_FAVORITE_ICON);
  RNA_def_property_ui_text(
      prop,
      "Favorite Icons",
      "Show the favorite star on brush asset tiles in the shelf. Favorites can still be managed "
      "from the Favorites catalog and the Only Favorites filter");
  RNA_def_property_update(
      prop, NC_SPACE | ND_REGIONS_ASSET_SHELF, "rna_AssetShelf_popup_settings_update");
  ...
  prop = RNA_def_property(srna, "recent_max_count", PROP_INT, PROP_UNSIGNED);
