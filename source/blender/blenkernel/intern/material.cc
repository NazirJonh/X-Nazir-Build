    ...
    case SH_NODE_TEX_IMAGE: {
      ...
      if (effective_slot_type == NODE_TEX_IMAGE_SLOT_NONE && nodetree != nullptr) {
        ...
        if (effective_slot_type != NODE_TEX_IMAGE_SLOT_NONE) {
          ...
          printf("[DEBUG]   fill_texpaint_slots_cb: Auto-detected slot_type=%d for node '%s'\n",
                 (int)effective_slot_type, node->name);
        }
      }
      ...
      printf("[DEBUG]   fill_texpaint_slots_cb: Slot[%d] assigned from NodeTexImage\n", index);
      printf("[DEBUG]     NodeTexImage::paint_slot_type = %d\n", (int)storage->paint_slot_type);
      printf("[DEBUG]     TexPaintSlot::slot_type = %d\n", (int)slot->slot_type);
      if (slot->ima) {
...
void BKE_texpaint_slot_refresh_cache(Scene *scene, Material *ma, const Object *ob)
{
  ...
  printf("[DEBUG] BKE_texpaint_slot_refresh_cache: Starting for material '%s'\n", ma->id.name + 2);
  ...
  if (scene->toolsettings->imapaint.mode == IMAGEPAINT_MODE_IMAGE) {
    ...
    printf("[DEBUG]   Found %d texture nodes\n", count);
    ...
    if (count == 0) {
      ...
      printf("[DEBUG]   Created %d TexPaintSlot entries\n", count);
      
      /* Log each slot's slot_type */
      for (int i = 0; i < count; i++) {
        printf("[DEBUG]   Slot[%d]: slot_type=%d, ima=%p\n", 
               i, 
               (int)ma->texpaintslot[i].slot_type,
               (void*)ma->texpaintslot[i].ima);
        if (ma->texpaintslot[i].ima) {
          printf("[DEBUG]     Image name: %s\n", ma->texpaintslot[i].ima->id.name + 2);
        }
      }
