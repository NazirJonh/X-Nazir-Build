...
void main()
{
  ...
  /* Увеличенный bias: в Texture Paint контур может теряться из-за несовпадения глубины
   * между отображаемой поверхностью и геометрией контура. */
  gl_Position.z -= 1e-3 * gl_Position.w;
#endif
