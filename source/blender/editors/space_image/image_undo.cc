...
static void selection_mask_snapshot_restore(const SelectionMaskSnapshot &entry, ImBuf *src_ibuf)
{
  ...
  if (src_ibuf) {
    if (mask_ptr && *mask_ptr) {
      ImBuf *cur = *mask_ptr;
      if (cur->float_data() && src_ibuf->float_data()) {
        if (cur->x == src_ibuf->x && cur->y == src_ibuf->y) {
          ...
          /* Dimensions changed тАФ replace the stored ImBuf entirely. */
          IMB_freeImBuf(cur);
      ...
      /* Mask was absent тАФ recreate it from the snapshot. */
      runtime->paint_selection_masks.add(entry.tile_number, IMB_dupImBuf(src_ibuf));
    }
  }
  else {
    /* Snapshot had no mask тАФ remove whatever currently exists. */
    if (mask_ptr && *mask_ptr) {
