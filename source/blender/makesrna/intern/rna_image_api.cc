...
static void rna_Image_get_selection_bounding_box(
    Image *image, ReportList * /*reports*/, int tile_number, int r_bbox[4], bool *r_has_selection)
{
  if (!image->runtime) {
                                          ...
                                          int *r_tiles_len)
{
  ...
  for (const ImageTile &tile : image->tiles) {
     ...
     */
    if (BKE_image_paint_selection_mask_lookup(const_cast<const Image *>(image),
                                              tile.tile_number) != nullptr)
    {
                                         ...
                                         int height)
{
  ...
  /* The mask must match the tile's actual pixel dimensions -- callers that pass mismatched
   * width/height would otherwise silently create a mask that no consumer (brush painting,
   * gradient, floating tools) can sample correctly, since they all assume mask size == tile
   * ImBuf size. */
  ImageUser iuser;
  BKE_imageuser_default(&iuser);
  iuser.tile = tile_number;
  void *lock;
  ImBuf *tile_ibuf = BKE_image_acquire_ibuf(image, &iuser, &lock);
  if (!tile_ibuf) {
    BKE_reportf(reports, RPT_ERROR, "Could not acquire image buffer for tile %d", tile_number);
    return;
  }
  if (tile_ibuf->x != width || tile_ibuf->y != height) {
    BKE_reportf(reports,
                RPT_ERROR,
                "Mask dimensions %dx%d do not match tile %d dimensions %dx%d",
                width,
                height,
                tile_number,
                tile_ibuf->x,
                tile_ibuf->y);
    BKE_image_release_ibuf(image, tile_ibuf, lock);
    return;
  }
  BKE_image_release_ibuf(image, tile_ibuf, lock);
...
void RNA_api_image(StructRNA *srna)
{
  ...
  parm = RNA_def_int_array(func,
                           ...
                           INT_MAX,
                           "Bounding Box",
                           "Pixel bounding box [x_min, y_min, x_max, y_max]",
                           0,
                           INT_MAX);
  ...
  parm = RNA_def_int_array(func,
                           ...
                           2000,
                           "Tiles",
                           "Tile numbers with a selection mask",
                           1001,
                           2000);
  ...
  RNA_def_parameter_flags(parm, PropertyFlag(0), PARM_OUTPUT);
  parm = RNA_def_int(func, "height", 0, 0, INT_MAX, "Height", "Mask height in pixels", 0, INT_MAX);
  RNA_def_parameter_flags(parm, PropertyFlag(0), PARM_OUTPUT);
  ...
  parm = RNA_def_int(func,
                     ...
                     INT_MAX,
                     "X",
                     "Left edge of the extracted region in tile space",
                     0,
                     INT_MAX);
  RNA_def_parameter_flags(parm, PropertyFlag(0), PARM_OUTPUT);
  parm = RNA_def_int(func,
  ...
  parm = RNA_def_int(func,
                     ...
                     INT_MAX);
  RNA_def_parameter_flags(parm, PropertyFlag(0), PARM_OUTPUT);
  parm = RNA_def_int(func, "channels", 0, 0, 4, "Channels", "Number of channels per pixel", 0, 4);
  RNA_def_parameter_flags(parm, PropertyFlag(0), PARM_OUTPUT);
  ...
  parm = RNA_def_int(func, "width", 1, 1, INT_MAX, "Width", "Mask width in pixels", 1, INT_MAX);
  RNA_def_parameter_flags(parm, PropertyFlag(0), PARM_REQUIRED);
  parm = RNA_def_int(func, "height", 1, 1, INT_MAX, "Height", "Mask height in pixels", 1, INT_MAX);
  RNA_def_parameter_flags(parm, PropertyFlag(0), PARM_REQUIRED);
  ...
  parm = RNA_def_int(func,
                     ...
                     INT_MAX,
                     "X",
                     "Left edge of the destination region in tile space",
                     0,
                     INT_MAX);
  RNA_def_parameter_flags(parm, PropertyFlag(0), PARM_REQUIRED);
  parm = RNA_def_int(func,
                     ...
                     INT_MAX);
  RNA_def_parameter_flags(parm, PropertyFlag(0), PARM_REQUIRED);
  parm = RNA_def_int(func, "width", 1, 1, INT_MAX, "Width", "Region width in pixels", 1, INT_MAX);
  RNA_def_parameter_flags(parm, PropertyFlag(0), PARM_REQUIRED);
  parm = RNA_def_int(
  ...
  RNA_def_function_ui_description(func,
                                  "Clear the paint selection mask for one tile or all tiles");
  RNA_def_function_flag(func, FUNC_USE_REPORTS);
  RNA_def_int(func,
              "tile",
              1001,
              ...
              2000,
              "Tile",
              "UDIM tile number (ignored when all_tiles is true)",
              1001,
              2000);
  RNA_def_boolean(func,
                  "all_tiles",
