...
void CurvePatchSpline2D::build_from_positions(const Span<float2> positions,
                                              ...
                                              const bool cyclic)
{
  ...
  if (!radii.is_empty()) {
    BLI_assert(radii.size() == positions.size());
  }

  /* Drop degenerate duplicates so every surviving segment advances the arc length: a
   * zero-length segment would only add a redundant knot. `core` tolerates such segments, but a
   * filtered polyline keeps `lengths_3d` strictly monotonic and per-point `radii` aligned. */
  constexpr float min_step_sq = 1e-8f * 1e-8f;
  Vector<float3> poly_3d;
  Vector<float> radii_filtered;
  poly_3d.reserve(positions.size());
  if (!radii.is_empty()) {
    radii_filtered.reserve(radii.size());
  }
  poly_3d.append(float3(positions[0], 0.0f));
  if (!radii.is_empty()) {
    radii_filtered.append(radii[0]);
  }
  float2 last_kept = positions[0];
  for (const int i : IndexRange(1, positions.size() - 1)) {
    const float2 &p = positions[i];
    if (math::distance_squared(p, last_kept) < min_step_sq) {
      continue;
    }
    poly_3d.append(float3(p, 0.0f));
    if (!radii.is_empty()) {
      radii_filtered.append(radii[i]);
    }
    last_kept = p;
  }
  ...
  core.build_from_positions(poly_3d, radii_filtered, cyclic);
}

bool CurvePatchSpline2D::is_empty() const
{
  return core.is_empty();
}

float CurvePatchSpline2D::total_length() const
{
  return core.total_length();
}

float2 CurvePatchSpline2D::evaluate(const float s) const
{
  const float3 co = core.evaluate(s);
  return float2(co.x, co.y);
}

float2 CurvePatchSpline2D::tangent_at(const float s) const
{
  const float3 tan = core.tangent_at(s);
  return float2(tan.x, tan.y);
}

float CurvePatchSpline2D::radius_at(const float s) const
{
  return core.radius_at(s);
}

float CurvePatchSpline2D::distance_sq_to(const float2 query) const
{
  return core.distance_sq_to(float3(query, 0.0f));
}

void CurvePatchSpline2D::closest_point_dist(const float2 query,
                                            float &r_s,
                                            float2 &r_tangent,
                                            float &r_dist) const
{
  float3 tangent_3d;
  core.closest_point_dist(float3(query, 0.0f), r_s, tangent_3d, r_dist);
  r_tangent = float2(tangent_3d.x, tangent_3d.y);
}
