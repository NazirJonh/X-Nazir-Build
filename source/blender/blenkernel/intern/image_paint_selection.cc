...
/**
 * Single point of contact for "the binary mask of one tile just changed": invalidates that
 * tile's derived blend mask and bumps the image-wide revision. Every writer of the binary mask
 * -- direct pixel edits and the mutable accessors that hand out a writable #ImBuf alike -- must
 * go through this rather than bumping the revision on its own, so a new writer cannot forget to
 * drop the now-stale blend mask (see #BKE_image_paint_selection_blend_mask_invalidate_tile).
 */
static void paint_selection_mark_tile_dirty(Image *image, int tile_number)
{
...
ImBuf *BKE_image_paint_selection_mask_get(Image *image, int tile_number, int width, int height)
{
  ...
  if (mask_ptr) {
    ...
    if (mask->x == width && mask->y == height) {
      ...
      paint_selection_mark_tile_dirty(image, tile_number);
      return mask;
    }
...
void BKE_image_paint_selection_blend_mask_invalidate_tile(Image *image, int tile_number)
{
   ...
   * here. Whoever edits the binary mask goes through #paint_selection_mark_tile_dirty instead,
   * which is the same rule #BKE_image_paint_selection_edge_policy_set relies on. */
  paint_selection_blend_mask_tile_invalidate(image, tile_number);
}
...
void BKE_image_paint_selection_mask_tile_reassign(Image *image,
                                                  int old_tile_number,
                                                  int new_tile_number)
{
  if (!image || !image->runtime || old_tile_number == new_tile_number) {
    return;
  }
  bke::ImageRuntime *runtime = image->runtime;
  ImBuf **mask_ptr = runtime->paint_selection_masks.lookup_ptr(old_tile_number);
  if (!mask_ptr) {
    return;
  }
  ImBuf *mask = *mask_ptr;
  runtime->paint_selection_masks.remove(old_tile_number);

  /* Drop any stale mask that already sat at the destination tile number. */
  ImBuf **existing_ptr = runtime->paint_selection_masks.lookup_ptr(new_tile_number);
  if (existing_ptr) {
    IMB_freeImBuf(*existing_ptr);
    runtime->paint_selection_masks.remove(new_tile_number);
  }
  runtime->paint_selection_masks.add_new(new_tile_number, mask);

  paint_selection_blend_mask_tile_invalidate(image, old_tile_number);
  paint_selection_blend_mask_tile_invalidate(image, new_tile_number);
  paint_selection_revision_bump(image);
}
 ...
 */
static bool paint_selection_mask_bounds_cached(
    const Image *image, const int tile_number, const ImBuf *mask, int r_min[2], int r_max[2])
{
  if (!image || !image->runtime) {
...
{
  BKE_image_paint_selection_edge_policy_set(image,
                                            use_feather ?
                                                BKE_image_paint_selection_edge_policy_feathered() :
                                         ...
                                         const int y)
{
  ...
  if (runtime->paint_selection_faces_empty) {
    return 0.0f;
  }
  const ImBuf *binary = paint_selection_face_mask_lookup(image, tile_number);
  if (!binary || !binary->float_buffer.data) {
...
void BKE_image_paint_selection_face_mask_free(Image *image)
{
  ...
  runtime->paint_selection_faces_empty = true;
}
...
bool BKE_image_paint_selection_blocks_all_paint(const Image *image)
{
  ...
  return image->runtime->paint_selection_from_faces &&
         image->runtime->paint_selection_faces_empty;
}
...
{
  /* User-authored part: unchanged semantics, applied only when an actual user selection exists. */
  float weight = 1.0f;
  if (BKE_image_paint_selection_mask_has_any(image)) {
    const PaintSelectionEdgePolicy &edge_policy = BKE_image_paint_selection_edge_policy_get(image);
    if (!edge_policy.use_outward_feather) {
      weight = BKE_image_paint_selection_mask_sample(image, tile_number, x, y) >
                       IMAGE_PAINT_SELECTION_MASK_THRESHOLD ?
                   1.0f :
                   0.0f;
    }
      ...
      if (!blend) {
        ...
        weight = paint_selection_blend_mask_sample_imbuf(blend, x, y);
      }
    }
  }
  /* Derived face-selection part: while the sync is active, pixels the face selection doesn't
   * cover are blocked (see #paint_selection_face_weight). */
  weight *= paint_selection_face_weight(image, tile_number, x, y);
  return weight;
}
                                                      ...
                                                      const float fy)
{
  ...
  float weight = 1.0f;
  if (BKE_image_paint_selection_mask_has_any(image)) {
    const PaintSelectionEdgePolicy &edge_policy = BKE_image_paint_selection_edge_policy_get(
        image);
    if (!edge_policy.use_outward_feather) {
      const int x = int(floorf(fx));
      const int y = int(floorf(fy));
      ...
      if (!blend) {
        ...
        weight = BKE_image_paint_selection_sample_mask_imbuf_bilinear(blend, fx, fy);
      }
    }
  }
  /* Derived face-selection part. */
  weight *= paint_selection_face_weight_bilinear(image, tile_number, fx, fy);
  return weight;
}
        ...
        dst[y * w + x] =
            edge_policy.use_outward_feather ?
                image_paint_selection_smooth_blend_weight(
                    src, w, h, x, int(y), 1, edge_policy.blend_radius_px, edge_policy.edge_gamma) :
...
bool BKE_image_paint_selection_mask_has_any(const Image *image)
{
  ...
  /* A tile can hold a fully zeroed mask (e.g. after subtracting the whole selection), so the
   * presence of a map entry alone is not enough: check that at least one tile has a selected
   * pixel. Bounds are memoized, so this is cheap after the first query per revision. */
...
bool BKE_image_paint_selection_gates_paint(const Image *image)
{
  ...
  /* While the mask mirrors the mesh face selection, an empty selection gates every pixel;
   * "no mask" would otherwise mean "paint everywhere". */
  return image->runtime->paint_selection_from_faces ||
    ...
    for (int row = 0; row < rh; row++) {
      ...
      for (int col = 0; col < rw; col++) {
        ...
        for (int c = 0; c < channels; c++) {
          ...
          dst[dst_idx + c] = static_cast<uint8_t>(std::max(0.0f, std::min(1.0f, src_f)) * 255.0f +
                                                  0.5f);
        }
      }
    }
