...
``state.category_orders_cache`` by reference (in-place mutations) and uses
...
def get_category_order(tag_combination):
    ...
    result = state.category_orders_cache.get(tag_combination, []).copy()
    ...
    if tag_combination == "NODE:":
        ...
        _log_once(f"[CAT ORDER][PY] state.category_orders_cache keys: {list(state.category_orders_cache.keys())[:10]}")
    return result
...
def set_category_order(tag_combination, category_list):
    ...
    state.category_orders_cache[tag_combination] = category_list.copy()
    # Trigger save
...
def clear_category_order(tag_combination):
    ...
    if tag_combination in state.category_orders_cache:
        del state.category_orders_cache[tag_combination]
        from bl_ui.glyph_tag_system.glyph_cache import _save_glyph_mappings_to_file
        _save_glyph_mappings_to_file()
...
def get_all_category_orders():
    ...
    if not is_glyph_cache_loaded():
        from bl_ui.glyph_tag_system.glyph_cache import _load_glyph_mappings_from_file
        _load_glyph_mappings_from_file()
    return state.category_orders_cache.copy()
