...
/**
 * Canonicalize `library_root_path` the same way #AssetLibrary::root_path() does (native slashes +
 * guaranteed trailing separator), so the #ImageLibraryIndex::library_root round-trip check in
 * #image_library_index_read() agrees regardless of which caller's root-string convention wrote or
 * is reading the index (e.g. raw #bUserAssetLibrary::dirpath vs #AssetLibrary::root_path() /
 * #FileList::root, which always has a trailing separator). Plain #BLI_path_normalize() does not
 * add a trailing separator, so a root written from a separator-less caller and read back with a
 * separator-terminated caller (or vice versa) would fail this check and make a perfectly valid,
 * up-to-date index look absent -- this is what made "All Libraries" reads of a nested image
 * library's index intermittently see zero entries even right after a direct read of the same
 * library had just indexed it successfully.
 */
static void image_library_normalize_root(const char *library_root_path,
                                         char r_norm_root[FILE_MAX])
{
  BLI_strncpy(r_norm_root, library_root_path, FILE_MAX);
  BLI_path_slash_native(r_norm_root);
  BLI_path_normalize_dir(r_norm_root, FILE_MAX);
}
...
static std::unique_ptr<ImageLibraryIndex> image_library_index_read(const char *library_root_path)
{
  ...
  image_library_normalize_root(library_root_path, norm_root);
