...
void draw_image_paint_selection_mask(const bContext *C, ARegion *region)
{
  ...
  /* The outline is cached in UV space, so it is drawn through the rotation-aware View2D matrix
   * rather than in region pixels. Both matrices are saved: this runs in the middle of
   * #draw_image_main_helpers and the drawers that follow rely on the ambient region-pixel space. */
