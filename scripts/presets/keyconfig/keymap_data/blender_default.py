...
def _template_items_image_paint_select_lasso_gesture(params):
    """Temporary lasso for Image Paint selection mask (does not switch the active tool).

    Uses #KM_PRESS (not #KM_PRESS_DRAG) so the gesture starts immediately and is not blocked
    by other Image Editor operators that also bind Ctrl+action_mouse on press.
    """
    if params.select_mouse == 'RIGHTMOUSE':
        return [
            ("paint.image_select_lasso",
             {"type": params.action_mouse, "value": 'PRESS', "shift": True, "ctrl": True},
             {"properties": [("mode", 'SUB')]}),
            ("paint.image_select_lasso",
             {"type": params.action_mouse, "value": 'PRESS', "shift": True, "ctrl": True, "alt": True},
             {"properties": [("mode", 'ADD')]}),
        ]
    else:
        return [
...
def km_image(params):
    ...
    items.extend([
        ...
        *((
        ...
        # Temporary lasso selection mask in Paint mode (does not switch the active tool).
        ...
        *_template_items_context_menu("IMAGE_MT_mask_context_menu", params.context_menu_event),
        ...
        ("paint.image_select_all", {"type": 'A', "value": 'PRESS'}, None),
        ("paint.image_select_none", {"type": 'A', "value": 'PRESS', "alt": True}, None),
...
def km_asset_browser_main(params):
    items = []
    keymap = (
        "Asset Browser Main",
        {"space_type": 'FILE_BROWSER', "region_type": 'WINDOW'},
    ...
    items.extend([
        ...
        # drag instead of selecting on press. This keeps the item under the cursor unselected while
        # starting an LMB drag-scroll gesture (and lets that gesture pan instead of selecting).
        # This keymap is processed before "File Browser Main", and the modal generic-select returns
        # `PASS_THROUGH | RUNNING_MODAL` (handled as break), so the press-select there is shadowed.
        ("file.select", {"type": 'LEFTMOUSE', "value": 'PRESS'},
         {"properties": [
             ("open", params.use_file_single_click),
             ("deselect_all", not params.legacy),
             ("use_select_on_click", True),
         ]}),
    ])
...
def km_sculpt_curves(params):
    ...
    items.extend([
        ...
        *_template_items_hide_reveal_actions("sculpt_curves.hide", "sculpt_curves.reveal"),
...
def km_edit_curves(params):
    ...
    items.extend([
        ...
        *_template_items_select_actions(params, "curves.select_all"),
        ...
        ("curves.make_segment", {"type": 'F', "value": 'PRESS'}, None),
        ...
        *_template_items_hide_reveal_actions("curves.hide", "curves.reveal"),
...
def km_image_editor_tool_paint_select_gradient(params, *, fallback):
    return (
        ...
        {"items": [
            *([] if (fallback and not params.use_fallback_tool) else [
                ("paint.image_select_gradient",
                 {"type": params.tool_mouse, "value": 'PRESS'},
                 None),
                ("paint.image_select_gradient_apply",
                 {"type": 'RET', "value": 'PRESS'},
                 None),
                ("paint.image_select_gradient_apply",
                 {"type": 'NUMPAD_ENTER', "value": 'PRESS'},
                 None),
                ("paint.image_select_gradient_cancel",
...
def km_3d_view_tool_cursor(params):
    ...
    # Bind cursor placement inside the tool keymap so it uses the tool's properties.
    if params.cursor_set_event:
        items.append(("view3d.cursor3d", params.cursor_set_event, None))
        if params.cursor_tweak_event:
            items.append((
                "transform.translate", params.cursor_tweak_event,
                {"properties": [("release_confirm", True), ("cursor_transform", True)]},
