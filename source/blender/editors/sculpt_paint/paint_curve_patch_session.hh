...
struct CurvePatchEditState {
   ...
   *
   * Owned conceptually by the live-edit modal (`SCULPT_OT_curve_patch_edit`), but stored here
   * rather than in its `op->customdata` so the small operators the modal's context menu invokes --
   * which cannot reach a running modal's customdata -- can act on the clicked point. Because this
   * session outlives the modal, the index is reset to -1 whenever an edit session ends or a new
   * control curve is built; consumers must still validate it against `control_curve.points_num()`.
   */
  int active_point = -1;
 ...
 * stamped here and no viewport is touched, which is what lets the headless apply path share it
