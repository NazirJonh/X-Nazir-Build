                             ...
                             const Span<bool> select_poly)
{
  ...
  if (color_domain == bke::AttrDomain::Corner) {
    if (!select_poly.is_empty()) {
      /* Face selection masking: average only the corners of selected faces. */
      float4 r_color(0.0f);
      int tot_selected = 0;
      ...
      return r_color / float(tot_selected);
    }
    float4 r_color(0.0f);
    for (const int face : vert_to_face_map[vert]) {
      const int corner = bke::mesh::face_find_corner_from_vert(faces[face], corner_verts, vert);
                           ...
                           const Span<bool> select_poly)
{
  if (color_domain == bke::AttrDomain::Corner) {
    for (const int i_face : vert_to_face_map[vert]) {
      if (!select_poly.is_empty() && !select_poly[i_face]) {
        /* Face selection masking: leave the corners of unselected faces untouched. */
        continue;
      }
                        ...
                        const Span<bool> select_poly)
{
  ...
  if (color_domain == bke::AttrDomain::Point) {
    ...
    to_static_color_type(color_attribute.type(), [&](auto dummy) {
      using T = decltype(dummy);
      for (const int i : verts.index_range()) {
        r_colors[i] = color_vert_get<T>(faces,
                                        corner_verts,
                                        vert_to_face_map,
                                        color_attribute,
                                        color_domain,
                                                             ...
                                                             const float ref_radius)
{
  ...
  for (Object *object_ptr : objects) {
    ...
    /* Face selection masking with nothing selected on this object: it contributes no samples
     * (its own mask state reads as "masking disabled" and must not un-mask this object). The
     * state is cached in the object's stroke cache, so this is O(1) per dab. */
...
{
  /* Face selection masking with nothing selected: nothing is paintable, neither on the image
   * canvas nor on color attributes. Skip the whole dab instead of gathering factors, sampling and
   * blending into unchanged data. The state is cached in the stroke, so this is O(1) per dab. */
  const FaceSelectionMask &face_selection_mask = face_selection_mask_ensure(ob);
  if (face_selection_mask.state == FaceSelectionMaskState::Empty) {
    return;
  }
  if (SCULPT_use_image_paint_brush(paint_mode_settings,
                                   ob,
                                   BKE_paint_brush_for_read(&sd.paint),
  ...
  for (const int i : verts.index_range()) {
             ...
             faces, corner_verts, vert_to_face_map, attribute_data.hide_poly, vert, neighbors))
    {
                                                                 ...
                                                                 neighbor_neighbors))
      {
        ...
        /* Face selection masking: vertices without a selected face read as zero (corner-domain)
         * and are not paintable; letting them contribute would darken the boundary. */
        if (face_selection_mask.state == FaceSelectionMaskState::Active &&
            !face_selection_mask.vert_paintable[neighbor_neighbor])
        {
          continue;
        }
                    ...
                    const IndexMask &node_mask)
{
   ...
   * (including the full-mesh #prev_colors initialization below). The state is cached in the
   * stroke, so this is O(1) per dab. */
  const FaceSelectionMask &face_selection_mask = face_selection_mask_ensure(ob);
  if (face_selection_mask.state == FaceSelectionMaskState::Empty) {
    return;
  }
                   ...
                   const IndexMask &node_mask)
{
  ...
  Mesh &mesh = *id_cast<Mesh *>(ob.data);
  /* Face selection masking with nothing selected: nothing is paintable, skip the whole dab. The
   * state is cached in the stroke, so this is O(1) per dab. */
  const FaceSelectionMask &face_selection_mask = face_selection_mask_ensure(ob);
  if (face_selection_mask.state == FaceSelectionMaskState::Empty) {
    return;
  }
