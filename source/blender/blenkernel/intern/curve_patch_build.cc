...
void curve_patch_build_from_control_curve(const CurvesGeometry &control_curve,
                                          ...
                                          const Span<float> stamp_texture_weights_cdf,
                                          const CurvePatchBuildMode mode,
                                          CurvePatchGeometry &r_geometry)
{
  ...
  /* Left empty for #CurvePatchBuildMode::PlanarSingleWindow. An empty span is what pins
   * #curve_patch_geometry_build to the single-window ribbon: #curve_patch_frames_partition needs
   * per-point normals and declines without them. On a flat canvas that is the wanted outcome --
   * there is no surface to wrap onto, and windowed frames would only introduce seams. */
  Array<float3> evaluated_normals;

  if (mode == CurvePatchBuildMode::SurfaceWindowed) {
    ...
    evaluated_normals = Array<float3>(evaluated_positions.size(), float3(0.0f));
     ...
     * Grids/CCG has no snapshot (see #curve_patch_session_publish), so the curve keeps the
     * positions and normals it already has from the control points above. */
                                ...
                                CurvePatchGeometry &r_geometry)
{
   ...
   */
  const bool has_extended_endpoint = ELEM(params.start_point_shape,
