...
/**
 * Bakeable Principled channels, as a flag enum (each value is the bit for its channel index).
 *
 ...
 *
 * Shared with the `IMAGE_OT_bake_from_material` operator (`render_material_bake_ops.cc`): one
 * array for every flag-style bake-channel selector.
 */
const EnumPropertyItem rna_enum_material_paint_bake_channel_items[] = {
