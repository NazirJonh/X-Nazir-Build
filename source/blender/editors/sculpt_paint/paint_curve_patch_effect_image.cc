...
struct TileSnapshot {
  ...
  /** Dense id assigned in creation order, used to build the `CurvePatchCache::pass_weight_accum`
    ...
    : paint_mode_settings_(&paint_mode_settings), canvas_key_(std::move(canvas_key))
{
   ...
   * (`paint_curve_patch_cache.cc:680`), so opening a second one here would free the first's
...
void ImageColorEffect::session_undo_begin()
{
   ...
   * preview synchronously (`paint_curve_patch_cache.cc:697` -> `:554` -> `:201` ->
...
{
  /* `CurvePatchCache` (which owns this effect through a `unique_ptr`) is destroyed on four paths,
   ...
   *   calls `commit()`);
   * - `curve_patch_commit_on_session_end()` (`paint_curve_patch_cache.cc:115`), after `commit()`;
   ...
   * - session-end commit (`paint_curve_patch_cache.cc:108` -> `:115`): identical sequence -> PUSHED;
   ...
   * - session-end discard (`paint_curve_patch_cache.cc:156` -> `:169`): same restore -> ABORTED.
   ...
   * bailed out on its own element-count guard (`paint_curve_patch_cache.cc:206-215`) BEFORE
 ...
 * against `CurvePatchCache::element_num` every restamp -- it must NOT be the snapshot's size, which
...
void ImageColorEffect::restore(Object &ob, const CurvePatchCache &patch)
{
  if (this->element_num(ob) != patch.element_num) {
    /* See `ColorEffect::restore()`: writing the snapshot back would corrupt an unrelated mesh. */
    return;
  }
                                  ...
                                  CurvePatchCache &patch)
{
  ...
  /* `BKE_brush_curve_strength()` inside the sampler reads `brush.curve_distance_falloff`'s lookup
   * table for the CUSTOM preset; initialize it ONCE here so the parallel PHASE 1 loop only ever
   * reads an already-built table (a lazy init inside a worker thread would race). */
   ...
   * `curve_patch_begin_editing()` (`paint_curve_patch_cache.cc:697`), and it reaches `apply_pass()`
   ...
   * `curve_patch_commit_on_session_end()` (`paint_curve_patch_cache.cc:97`) -- and `restore()` runs
          ...
          for (const bke::pbvh::pixels::PackedPixelRow &pixel_row : tile_data.pixel_rows) {
            ...
            threading::parallel_for(IndexRange(row_size), 512, [&](const IndexRange range) {
              ...
              const CurvePatchSampler sampler(
                  patch, cache, brush, source, chunk_mask, ss.tex_pool);
  ...
  for (LocalData &local : all_tls) {
    for (ChunkWrite &chunk : local.chunks) {
      ...
      for (const AcceptedPixel &pixel : chunk.accepted) {
        ...
        float2 &accum = patch.pass_weight_accum.lookup_or_add(accum_key, float2(0.0f, 0.0f));
        accum.x += pixel.weight;
        accum.y += pixel.weight * pixel.value;
        const float blended = accum.y / accum.x;

        /* A negative mix factor has no meaning (unlike relief, where a negative height carves
         * inward), so clamp rather than preserve the sign. The texture's own alpha further
  ...
  /* `set_bits()` ORs the mask's set bits into the target in place: `last_restamp_nodes` is already
   * sized and cleared by the orchestrator before the first symmetry pass of this restamp, and
   * `all_touched_nodes` accumulates over the patch's whole life, so neither may be cleared or
   * resized here. `touched` is built per-write rather than from the whole `node_mask` query for the
   * same reason `ColorEffect` scopes its own tag to the nodes it actually wrote. */
...
void ImageColorEffect::end_restamp(bContext & /*C*/, Object &ob, CurvePatchCache &patch)
{
  bke::pbvh::Tree *pbvh = bke::object::pbvh_get(ob);
  ...
  IndexMaskMemory memory;
  const IndexMask touched_mask = IndexMask::from_bits(patch.last_restamp_nodes, memory);
  MutableSpan<bke::pbvh::MeshNode> nodes = pbvh->nodes<bke::pbvh::MeshNode>();
  MutableSpan<bke::pbvh::pixels::PixelNode> pixel_nodes = pixel_data.nodes;
  ...
  flush_update_step(patch.view_context, ob, UpdateType::Image);
}

void ImageColorEffect::commit(bContext &C, Object &ob, const CurvePatchCache & /*patch*/)
{
  /* No pixel work: every write already landed live in the `ImBuf` during `apply_pass()`, and the
   ...
   *
   * What is left is the redraw. Both commit paths issue their own
   * `flush_update_done(..., UpdateType::Position)` right after calling this
   * (`paint_curve_patch_edit.cc:1608`, `paint_curve_patch_cache.cc:109`), and that is SHARED with
   * `ReliefEffect` and `ColorEffect` -- but only `UpdateType::Image` reaches the `SPACE_IMAGE` loop
   * in `flush_update_done()` (`mesh/sculpt.cc:5431-5439`), so with an Image editor open beside the
   * viewport the committed canvas would stay stale there until some unrelated event repainted it.
   * The extra flush is issued here, where it can be scoped to this effect, rather than by changing
   * the update type at those two shared call sites.
   *
   * Issuing both is safe: every `UpdateType::Position`-only step (`store_bounds_orig()`,
   * `fake_neighbors_free()`, `BKE_pbvh_bmesh_after_stroke()`) is skipped by this one, and
   * everything it does do -- clearing `RV3D_PAINTING`, tagging regions for redraw, and tagging the
   * ID when the mesh has linked duplicates OR any non-active 3D region cannot draw from the PBVH
   * (`mesh/sculpt.cc:5422-5424`) -- is idempotent. It is also
   * distinct from `end_restamp()`'s `flush_update_step()`, which only arms the interactive
   * paint-redraw path for the duration of the session.
   *
   * Skipped when the session painted nothing: `orig_tiles_` gains an entry on the first pixel
   * written, so an empty map means no canvas pixel ever changed and there is nothing to repaint.
   * `flush_update_done()` dereferences the object's `pbvh::Tree`, which is the same precondition
   * the shared call immediately after this one already has. */
  if (orig_tiles_.is_empty()) {
    return;
  }
