        ...
        bke::object::pbvh_get(ob)->type() == bke::pbvh::Type::Mesh)
    {
      ...
      if (brush.material_paint != nullptr) {
        const Vector<StringRef, PAINT_MATERIAL_CHANNEL_NUM> names =
            material::enabled_scalar_attribute_names(
                *brush.material_paint, paint_mode_settings, visible_material_channels);
        const Vector<StringRef, PAINT_MATERIAL_CHANNEL_NUM> color_names =
            material::enabled_color_attribute_names(
                *brush.material_paint, paint_mode_settings, visible_material_channels);
        /* Type::Material covers both scalar and color channel writes now that color undo is
         * name-keyed (see #undo::StepData::material_attributes); a stroke touching only
         * color channels still goes through Type::Material with an empty scalar name list,
         * replacing the old Type::Color-only fallback for that case. */
        if (!names.is_empty() || !color_names.is_empty()) {
          ...
          undo::push_nodes(depsgraph,
                           ...
                           node_mask,
                           undo::Type::Material,
                           {names.as_span(), color_names.as_span(), created_names.as_span()});
        }
      }
    }
...
void SculptPaintStroke::update_step(wmOperator * /*op*/, PointerRNA *itemptr)
{
  ...
  if (sculpt_brush_is_texture_fill(brush)) {
...
void SculptPaintStroke::done(bool is_cancel, bool stroke_started)
{
  ...
  /* NOTE: nothing is done here to refresh the painted images on the GPU.
   *
   * Each dab already queues its dirty regions for a partial upload through
   * #mark_gpu_texture_regions_dirty, so the 3D Viewport and Shader Editor pick the new pixels up
   * incrementally, for every painted channel, whether or not it is open in an Image Editor.
   *
   * This used to call #BKE_image_free_gputextures per target instead. That discarded every
   * channel's whole texture - all tiles, all mipmaps - at the end of every stroke, forcing a full
   * re-upload and a GPU material rebuild, and throwing away all the partial-update work the
   * stroke had just done. See "Refactor: Images: Incorrect usage of BKE_image_free_gputextures"
   * upstream: use the partial update mechanism instead. */
  ...
  else if (sculpt_brush_uses_image_canvas(*brush, *paint_mode_settings_, sd.paint, ob)) {
