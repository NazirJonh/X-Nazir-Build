...
static void rna_def_image(BlenderRNA *brna)
{
  ...
  /* Which layer an image belongs to is an input of the composite: an add-on that regroups its maps
   * changes what the Image Editor should be showing, and nothing else would tell it to look. The
   * caches need no invalidation of their own -- both fields are part of the stack hash. */
  RNA_def_property_update(prop, NC_IMAGE | ND_DISPLAY, nullptr);
  ...
  /* Same reasoning as #Image.paint_layer_id: it decides which map a pass resolves to. */
  RNA_def_property_update(prop, NC_IMAGE | ND_DISPLAY, nullptr);
