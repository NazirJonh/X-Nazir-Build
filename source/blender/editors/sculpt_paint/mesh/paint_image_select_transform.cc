...
/* Own state (#ImageSelectTransformState). */
#include "paint_image_select_transform_intern.hh"
/* #image_select_move_is_floating_in_space / #image_select_move_state_get for the move -> transform
 * hand-over. */
#include "paint_image_select_move_intern.hh"

namespace blender {

struct ImageSelectTransformTarget {
  Image *image = nullptr;
  ImageUser iuser;
...
struct ImageSelectTransformState : public PaintSelectFloatingSession {
  ...
  /* PBR channel images have independent pixel-space fragments but share the active image's UV
   * transform. Only the active image is drawn as the floating preview. */
  blender::Vector<ImageSelectTransformTarget> material_targets;
  ...
  enum HandleType {
    ...
    HANDLE_C1,
    HANDLE_C2,
    HANDLE_C3, /* Corners: BL, BR, TR, TL */
    HANDLE_M_BOTTOM,
    HANDLE_M_RIGHT,
    HANDLE_M_TOP,
    HANDLE_M_LEFT /* Edge Midpoints */
...
static void image_select_transform_lift_source(bContext *C, ImageSelectTransformState *state)
{
  ...
  image_select_fragment_lift_source(C, sima->image, state->iuser, state->fragments);
  for (ImageSelectTransformTarget &target : state->material_targets) {
    image_select_fragment_lift_source(C, target.image, target.iuser, target.fragments);
  }
}
...
static void image_select_transform_restore_source(bContext *C, ImageSelectTransformState *state)
{
  ...
  image_select_fragment_restore_source(C, ima, state->iuser, state->fragments);
  for (ImageSelectTransformTarget &target : state->material_targets) {
    image_select_fragment_restore_source(C, target.image, target.iuser, target.fragments);
  }
}

void image_select_transform_session_cancel(bContext *C, SpaceImage *sima)
{
...
void image_select_transform_state_free(ImageSelectTransformState *state)
{
  ...
  for (ImageSelectTransformTarget &target : state->material_targets) {
    selection_tile_fragments_free(target.fragments);
  }
  state->material_targets.clear();
  MEM_delete(state);
}
    ...
    const ImageSelectTransformState *state, const float2 &mouse_co)
{
  ...
  if (image_select_transform_point_in_handle_rect(mouse_co, state->screen_rotate_handle, hit_half))
  {
    return ImageSelectTransformState::HANDLE_ROTATE;
  }

  for (int i = 0; i < 4; i++) {
    if (image_select_transform_point_in_handle_rect(mouse_co, state->screen_corners[i], hit_half))
    {
                                                     ...
                                                     ARegion *region)
{
  ...
  float uv_x_min, uv_y_min, uv_x_max, uv_y_max;
  if (image_select_transform_source_uv_bounds(state, uv_x_min, uv_y_min, uv_x_max, uv_y_max)) {
    state->drag_start_anchor_inside_bounds = (state->drag_start_anchor.x >= uv_x_min - 0.001f &&
                                         ...
                                         ARegion *region)
{
  ...
  const ImageSelectTransformState::HandleType active = image_select_transform_handle_to_internal(
      handle);
...
static void image_select_transform_commit_write_final(bContext *C,
                                                       Image *ima,
                                                       ImageSelectTransformState *state);

static Vector<ImageSelectTransformTarget> image_select_transform_material_targets_extract(
    bContext *C, wmOperator *op, SpaceImage *sima)
{
  Vector<ImageSelectTransformTarget> result;
  for (const ImagePaintSelectionTarget &selection_target : image_paint_selection_targets_get(C,
                                                                                              sima))
  {
    if (selection_target.image == sima->image) {
      continue;
    }
    ImageSelectTransformTarget target;
    target.image = selection_target.image;
    target.iuser = selection_target.iuser;
    if (image_select_extract_per_tile(op, target.image, target.iuser, &target.fragments)) {
      result.append(std::move(target));
    }
    else {
      selection_tile_fragments_free(target.fragments);
    }
...
static void image_select_transform_undo_snapshot_fragments(Image *ima,
                                                            const ImageUser &base_iuser,
                                                            const Span<SelectionTileFragment> fragments,
                                                            ImageUndoStep *us_open)
{
  for (const SelectionTileFragment &frag : fragments) {
    ImageUser iuser = base_iuser;
    iuser.tile = frag.geom.tile_number;
    void *lock = nullptr;
    ImBuf *ibuf = BKE_image_acquire_ibuf(ima, &iuser, &lock);
    if (ibuf) {
      ED_image_undo_push(ima, ibuf, &iuser, us_open);
...
static void image_select_transform_commit_material_targets(bContext *C,
                                                           Image *ima,
                                                           ImageSelectTransformState *state)
{
  /* Restore every target before opening the undo step so its "before" snapshot never sees the
   * temporary hole left by the floating preview. */
  image_select_fragment_restore_source(nullptr, ima, state->iuser, state->fragments);
  for (ImageSelectTransformTarget &target : state->material_targets) {
    image_select_fragment_restore_source(nullptr, target.image, target.iuser, target.fragments);
  }

  const int first_tile = state->fragments[0].geom.tile_number;
  ImageUser first_iuser = state->iuser;
  first_iuser.tile = first_tile;
  void *first_lock = nullptr;
  ImBuf *first_ibuf = BKE_image_acquire_ibuf(ima, &first_iuser, &first_lock);
  if (first_ibuf) {
    ED_image_undo_push_begin_with_image("Transform Selection", ima, first_ibuf, &first_iuser);
    BKE_image_release_ibuf(ima, first_ibuf, first_lock);
  }
  else {
  ...
  for (const ImageSelectTransformTarget &target : state->material_targets) {
    if (!us_open) {
      break;
    }
    image_select_transform_undo_snapshot_fragments(
        target.image, target.iuser, target.fragments, us_open);
  }

  image_select_transform_commit_write_final(C, ima, state);
  for (ImageSelectTransformTarget &target : state->material_targets) {
    /* Reuse the transform math with channel-local fragments. */
    std::swap(state->fragments, target.fragments);
    std::swap(state->iuser, target.iuser);
    image_select_transform_commit_write_final(C, target.image, state);
    std::swap(state->iuser, target.iuser);
    std::swap(state->fragments, target.fragments);
  }
...
/**
 * Runs entirely inside #image_select_fragment_commit_with_undo's self-contained undo step: that
 * helper restores the source region to its pre-lift original first, then
 * #image_select_transform_commit_write_final re-clears it (matching what lift_source did
 * originally) before writing the transformed result to the destination. Undoing the resulting
 * step restores both source and destination in one go.
 */
static void image_select_transform_commit(bContext *C, ImageSelectTransformState *state)
{
  SpaceImage *sima = state->owner_sima;
  ...
  if (!state->material_targets.is_empty()) {
    image_select_transform_commit_material_targets(C, ima, state);
    return;
  }

  image_select_fragment_commit_with_undo(
      C, ima, state->iuser, state->fragments, "Transform Selection", [&]() {
        image_select_transform_commit_write_final(C, ima, state);
      });
}
                                                      ...
                                                      ImageSelectTransformState *state)
{
  ...
  for (const SelectionTileFragment &frag : state->fragments) {
    ...
    const float2 pixel_translation = float2(state->uv_translation.x * float(canvas_w),
                                            state->uv_translation.y * float(canvas_h));
    const float2 pixel_anchor = float2((state->uv_pivot.x - float(src_col)) * float(canvas_w),
    ...
    const float2 corners_local[4] = {{0.0f, 0.0f},
    ...
    for (ImageTile *dest_tile : ListBaseWrapper<ImageTile>(ima->tiles)) {
      ...
      IMB_transform(frag.pixels.fragment_ibuf,
                    temp_pixels,
      ...
      if (frag.pixels.fragment_mask_ibuf) {
        IMB_transform(frag.pixels.fragment_mask_ibuf,
                      temp_mask,
                      IMB_TRANSFORM_MODE_CROP_SRC,
                      IMB_FILTER_NEAREST,
                      backward_region,
      ...
      BKE_image_paint_selection_compute_blend_mask_to_4ch(
          temp_mask->float_buffer.data, reg_w, reg_h, blend_fill, 4, frag.edge_policy);
  ...
  BKE_image_paint_selection_blend_mask_invalidate(ima);
...
static wmOperatorStatus image_select_transform_exec(bContext *C, wmOperator *op)
{
  ...
  state->material_targets = image_select_transform_material_targets_extract(C, op, sima);
                                                     ...
                                                     const wmEvent *event)
{
  ...
  if (!state->is_dragging) {
    ...
    if (event->type == EVT_MODAL_MAP) {
      /* Confirm commits; both Cancel and Undo Step discard the floating transform. Neither is
       * passed through to the WM: no image undo step is ever open while floating (see
       * #image_select_fragment_commit_with_undo), so there is nothing for WM's own undo to
       * interact with, and #image_select_transform_restore_source just restores pixels. */
      switch (event->val) {
    ...
    const bool proportional)
{
  ...
  /* Move is deliberately single-image, but a Transform adopted from a floating move must finish
   * as a Material PBR transform. Extract the still-unlifted channel selections now and lift them
   * with the adopted active fragment. Outside Material canvas mode the resolver only returns the
   * active image, preserving the existing single-image hand-over. */
  state->material_targets = image_select_transform_material_targets_extract(C, op, sima);
  for (ImageSelectTransformTarget &target : state->material_targets) {
    image_select_fragment_lift_source(C, target.image, target.iuser, target.fragments);
  }
...
void PAINT_OT_image_select_transform(wmOperatorType *ot)
{
  ...
  /* No OPTYPE_UNDO, see #PAINT_OT_image_select_move: no image undo step is ever open while this
   * operator runs (lift never opens one -- see #image_select_fragment_commit_with_undo), so there
   * is nothing meaningful for WM to push on FINISHED. */
...
void PAINT_OT_image_select_transform_confirm(wmOperatorType *ot)
{
  ...
  /* No OPTYPE_UNDO: commit opens and closes its own image undo step entirely within
   * #image_select_transform_commit. See the note in #PAINT_OT_image_select_move. */
  ot->flag = OPTYPE_REGISTER;
}
...
void PAINT_OT_image_select_transform_cancel(wmOperatorType *ot)
{
  ...
  /* No OPTYPE_UNDO: restore_source is a plain pixel restore with no undo step to close or leave
   * open. See the note in #PAINT_OT_image_select_move. */
  ot->flag = OPTYPE_REGISTER;
}
