...
/**
 * Flat 2D adapter over #CurvePatchSpline for Image Editor painting (`PaintMode::Texture2D`).
 *
 * 2D positions are embedded as `(u, v, 0)` and only the geometry-only subset that is meaningful
 * on a plane is exposed: arc length, evaluation, tangent, radius and raw nearest-point queries.
 * Surface-frame methods (#normal_at, #closest_point, #curve_patch_spline_smooth_normals,
 * `plane_normal`) and the ribbon/LUT machinery are deliberately NOT wrapped -- the 2D mapping
 * path is flat by contract and never touches 3D surface data.
 *
 * Units are caller-defined (image UV space for the Image Editor); nothing here assumes a world
 * scale.
 */
struct CurvePatchSpline2D {
  /** Embedded `(u, v, 0)` spline -- the only authoritative polyline representation. */
  CurvePatchSpline core;

  /** Rebuild from a 2D polyline. Duplicate/degenerate steps (shorter than an epsilon) are
   * dropped so arc length stays strictly monotonic; `radii`, when non-empty, must match
   * `positions` in size and is filtered in lockstep. Fewer than two surviving points clears the
   * spline. */
  void build_from_positions(Span<float2> positions, Span<float> radii = {}, bool cyclic = false);

  bool is_empty() const;
  float total_length() const;
  /** Position on the spline at arc-length `s`, clamped to `[0, total_length()]`. */
  float2 evaluate(float s) const;
  /** Unit tangent at arc-length `s`; falls back to a stable `(1, 0)` when the spline is empty or
   * the segment is degenerate. */
  float2 tangent_at(float s) const;
  float radius_at(float s) const;
  /** Minimum SQUARED Euclidean distance from `query` to the polyline. */
  float distance_sq_to(float2 query) const;
  /** Raw closest point: arc-length `r_s` (clamped to `[0, total_length()]`), unit `r_tangent`,
   * and the true Euclidean distance `r_dist`. No-op (leaves outputs unchanged) when empty. */
  void closest_point_dist(float2 query, float &r_s, float2 &r_tangent, float &r_dist) const;
};
 ...
 */
/**
 * How #curve_patch_build_from_control_curve orients the strip.
 *
 * This is a build decision, stated rather than inferred. It used to be implicit in whether the
 * `evaluated_normals` span handed to #curve_patch_geometry_build came back empty, which made
 * "which path did this build take" a property no caller could ask about and forced a caller
 * wanting the planar path to bypass this wrapper and re-implement the tessellation.
 */
enum class CurvePatchBuildMode : int8_t {
  /**
   * Seed the normals from the control curve's own per-point normal attribute, refine both
   * positions and normals against `r_geometry.surface` when it is ready, and hand a non-empty
   * normals span to the build -- which is what lets #curve_patch_frames_partition run and produce
   * the windowed frames a curved surface needs. The path every 3D target takes.
   */
  SurfaceWindowed = 0,
  /**
   * Build no normals at all and hand the build an empty span, which pins it to the single-window
   * ribbon in the plane of `CurvePatchParams::plane_normal`. The path a flat canvas takes: there
   * is no surface to wrap onto, and windowed frames on a plane would only add seams.
   *
   * `r_geometry.surface` is ignored in this mode, ready or not.
   */
  PlanarSingleWindow = 1,
};
 ...
 * `cyclic` off curve 0, and -- in #CurvePatchBuildMode::SurfaceWindowed, when the snapshot is
 * ready -- pulling the polyline onto it.
 *
 * `r_geometry.surface` is an INPUT for #CurvePatchBuildMode::SurfaceWindowed. When it is ready the
 * polyline is projected onto it before the spline is built; otherwise the strip stays in the
 * curve's own plane. WHICH mesh the snapshot was taken from is deliberately the caller's decision:
 * a sculpt session snapshots the original mesh, while a script reading a patch back may ask for
 * the evaluated one.
 *
