...
/* Own state (#ImageSelectMoveState). */
#include "paint_image_select_move_intern.hh"
/* #image_select_transform_adopt_move_state for the move -> transform hand-over. */
...
static void draw_select_move_preview(const bContext * /*C*/, ARegion *region, void *arg)
{
  ...
  for (const SelectionTileFragment &frag : state->fragments) {
    ...
    if (frag.preview.fragment_display_ibuf) {
      draw_select_move_preview_ibuf(
          frag.preview.fragment_display_ibuf, sx0, sy0, zoom_x, zoom_y, false, GPU_BLEND_ALPHA);
    }
    else {
      draw_select_move_preview_ibuf(
          frag.pixels.fragment_ibuf, sx0, sy0, zoom_x, zoom_y, false, GPU_BLEND_ALPHA);
    }
 ...
 * visual feedback that the fragment has been lifted. No undo interaction: the commit re-applies
 * this same clear inside its own self-contained undo step (see #image_select_move_commit).
 */
...
void image_select_move_session_cancel(bContext *C, SpaceImage *sima)
{
...
static void image_select_move_commit_write_final(bContext *C,
                                                 Image *ima,
 ...
 * helper restores the source region to its pre-lift original first (so the step's "before"
 * snapshot is the true original), then calls the body below, which re-clears the source (matching
 * what lift_source did originally) before blitting to the destination. Undoing the resulting step
 * restores both source and destination in one go.
 *
...
void image_select_move_commit(bContext *C, ImageSelectMoveState *state)
{
  ...
  image_select_fragment_commit_with_undo(
      C, ima, state->iuser, state->fragments, "Move Selection", [&]() {
        image_select_move_commit_write_final(C, ima, state);
      });
}
        ...
        threading::parallel_for(IndexRange(dp_y0, dp_rows), 16, [&](const IndexRange range) {
          for (const int64_t py : range) {
            ...
            const float bly = std::max(0.0f,
                                       std::min(float(frag.geom.size_px.y) - 1.0001f, fy - 0.5f));
          ...
          for (const int64_t py : range) {
            ...
            for (int px = dp_x0; px < dp_x1; px++) {
              ...
              for (int c = 0; c < 4; c++) {
                ...
                dst_px[c] = uint8_t(
                    std::clamp((1.0f - blend) * float(dst_px[c]) + blend * frag_color[c] + 0.5f,
                               0.0f,
                               255.0f));
              }
            }
...
static wmOperatorStatus image_select_move_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  SpaceImage *sima = CTX_wm_space_image(C);
  if (!sima || !sima->runtime) {
...
static wmOperatorStatus image_select_move_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  SpaceImage *sima = CTX_wm_space_image(C);
  if (!sima || !sima->runtime) {
    return OPERATOR_CANCELLED;
   ...
   * Switch the active tool to Move Selection so that LMB press is actually routed to this
   * operator -- otherwise whatever tool was active before Ctrl+T (e.g. a brush, or a select
   * tool whose gesture would consume the click) keeps handling LMB and dragging never starts.
   * Mirrors what the paste operator already does after lifting a pasted fragment. */
...
static wmOperatorStatus image_select_paste_exec(bContext *C, wmOperator *op)
{
  ...
  if (cb && cb->has_mask) {
    ...
    /* Content comes from the clipboard, not the canvas: #image_select_fragment_lift_source /
     * #image_select_fragment_restore_source must never touch this fragment's footprint on the
     * canvas, since there is no "original" there to clear or restore -- the undo "before" state
     * for a paste is simply the untouched canvas, captured at commit time like any other. */
    frag.is_pasted = true;
  ...
  /* Any other tool's floating session is ended (committed/discarded) first, same as a normal move
   * invoke, so the paste starts from a settled canvas. */
  image_select_floating_sessions_end(C, sima, PaintSelectTool::Move);
  ...
  /* No undo step opened here: like a normal lift, the paste's undo "before" snapshot is captured
   * at commit time by #image_select_fragment_commit_with_undo, from inside a single self-contained
   * operator call that cannot be interrupted. */
  image_select_session_set(sima, state);
                                                        ...
                                                        const wmEvent *event)
{
  ...
  /* No undo step to hand over: neither move nor transform holds one open while floating (see
   * #image_select_fragment_commit_with_undo), so there is nothing here that a cancelled adoption
   * would need to close. */
...
void PAINT_OT_image_select_move(wmOperatorType *ot)
{
   ...
   * No OPTYPE_UNDO: lift_source never opens an image undo step (see
   * #image_select_fragment_commit_with_undo); the step is opened and closed entirely inside the
   * `_confirm` operator's own #image_select_move_commit call. Letting WM push its own undo step
   * for this operator (or for `_confirm`/`_cancel` below) would register a step with nothing
   * useful in it and confuse the history list. */
