...
void SculptPaintStroke::post_done(bContext *C, const bool is_cancel, const bool stroke_started)
{
  ...
  /* #SCULPT_OT_insert_mesh uses TBB parallel algorithms that do not spawn workers while the
   * calling thread is still inside a #threading::isolate_task region (e.g. from
   * #restore_position_from_undo_step). Defer the operator until after #done returns so it runs with
   * the full scheduler thread count. */
