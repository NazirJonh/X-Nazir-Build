...
class PaintCurveCursor : Overlay {
 ...
 private:
  ...
  /** Create a LINE_STRIP batch. Vertices are stored as float2 in an SSBO-compatible VBO. */
  ...
  void fill_pass_from_pod(Resources & /*res*/, const State &state)
  {
    ...
    /* --- 1. Faint silhouettes --- */
    {
           ...
           silhouettes_.faint_objects)
      {
        for (const Vector<float2> &polyline : entry.polylines) {
          if (gpu::Batch *b = make_line_strip(polyline)) {
            draw_strip(b);
          }
        }
      }
    ...
    /* --- 2. Hover silhouette --- */
    if (!silhouettes_.hover.is_empty()) {
      float hover_col[4];
      ...
      for (const Vector<float2> &polyline : silhouettes_.hover) {
        if (gpu::Batch *b = make_line_strip(polyline)) {
          draw_strip(b);
        }
      }
