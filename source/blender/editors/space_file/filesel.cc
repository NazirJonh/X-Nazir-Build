...
static void fileselect_ensure_updated_asset_params(SpaceFile *sfile)
{
  ...
  if (!asset_params) {
    ...
    BLI_listbase_clear(&asset_params->catalog_collapsed_states);
    ...
    if (global_settings) {
      LISTBASE_FOREACH (
          const AssetCatalogState *, global_state, &global_settings->catalog_collapsed_states)
      {
        ...
        BLI_addtail(&asset_params->catalog_collapsed_states, new_state);
      }
    }
  }
