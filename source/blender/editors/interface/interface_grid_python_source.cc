...
class PyGridDragController : public AbstractViewItemDragController {
  std::string source_identifier_;
  std::string drag_operator_;

 public:
  PyGridDragController(AbstractGridView &view,
                       StringRef source_identifier,
  ...
  {
    return WM_DRAG_GRID_ITEM_PY;
  }

  void *create_drag_data() const override
  {
    wmDragGridItemPy *drag_data = MEM_new<wmDragGridItemPy>(__func__);
    drag_data->source_identifier = source_identifier_;
    return drag_data;
  }
  ...
  void on_drag_start(bContext &C, AbstractViewItem & /*item*/) override
  {
    ...
    RNA_string_set(op_props, "identifier", source_identifier_.c_str());
...
class PyGridReorderDragController : public AbstractViewItemDragController {
  std::string source_identifier_;
  std::string grid_id_;

 public:
  PyGridReorderDragController(AbstractGridView &view,
                              StringRef source_identifier,
                              StringRef grid_id)
      : AbstractViewItemDragController(view),
        source_identifier_(source_identifier),
  ...
  std::optional<eWM_DragDataType> get_drag_type() const override
  {
    return WM_DRAG_GRID_ITEM_REORDER_PY;
  }

  void *create_drag_data() const override
  {
    wmDragGridItemReorderPy *drag_data = MEM_new<wmDragGridItemReorderPy>(__func__);
    drag_data->grid_id = grid_id_;
    drag_data->source_identifier = source_identifier_;
    return drag_data;
  }
};
...
class PyGridItem : public PreviewGridItem {
 ...
 public:
  ...
  {
    /* Unconditionally true when reordering is available: entering BUTTON_STATE_WAIT_DRAG on
     * every press (not just Shift-held presses) is required for the drag-threshold state machine
     * to be able to recognize a *later* Shift-held move as the start of a reorder drag -- see
     * AssetViewItem::supports_drag() for the identical reasoning already established for the
     * asset shelf. */
    if (!reorder_operator_.empty()) {
      return true;
    }
    return !drag_operator_.empty();
  }
  ...
  std::unique_ptr<AbstractViewItemDragController> create_drag_controller() const override
  {
    ...
    return std::make_unique<PyGridDragController>(this->get_view(), identifier_, drag_operator_);
  }
      ...
      const wmEvent *event) const override
  {
    ...
    if (shift && !reorder_operator_.empty()) {
      return std::make_unique<PyGridReorderDragController>(
          this->get_view(), identifier_, this->get_view().session_grid_id());
    }
  ...
  std::unique_ptr<GridViewItemDropTarget> create_drop_target() override
  {
    ...
    return std::make_unique<GridItemReorderDropTarget>(this->get_view(),
                                                       *this,
                                                       reorder_operator_,
                                                       activate_operator_,
                                                       this->get_view().session_grid_id());
  }
