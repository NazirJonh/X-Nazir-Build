...
/* Own state (#ImageSelectWarpState). Warp has no cross-tool dependency on move/transform. */
...
static float2 warp_catmull_rom_1d(
    const float2 &p0, const float2 &p1, const float2 &p2, const float2 &p3, const float t)
{
  const float t2 = t * t;
  const float t3 = t2 * t;
...
static void image_select_warp_commit_write_final(bContext *C,
                                                  Image *ima,
                                                  ImageSelectWarpState *state,
                                                  ReportList *reports);

static void image_select_warp_undo_snapshot_fragment(Image *ima,
                                                      const ImageUser &base_iuser,
                                                      const SelectionTileFragment &fragment,
                                                      ImageUndoStep *us_open)
{
  ...
  iuser.tile = fragment.geom.tile_number;
  void *lock = nullptr;
  ...
  ED_image_undo_capture_selection_mask(ima, fragment.geom.tile_number);
}

/** Map the active preview's UV-space control grid into a target fragment's native pixel space. */
static void image_select_warp_grid_for_target(const ImageSelectWarpState &state,
                                              const SelectionTileFragment &target_fragment,
                                              Array<float2> &r_src_pts,
                                              Array<float2> &r_tgt_pts)
{
  float2 active_origin_uv, active_size_uv;
  image_select_warp_capture_uv_rect(state.fragment, &active_origin_uv, &active_size_uv);
  float2 target_origin_uv, target_size_uv;
  image_select_warp_capture_uv_rect(target_fragment, &target_origin_uv, &target_size_uv);

  r_src_pts.reinitialize(state.src_pts.size());
  r_tgt_pts.reinitialize(state.tgt_pts.size());
  for (const int i : state.src_pts.index_range()) {
    const float2 src_uv = active_origin_uv + state.src_pts[i] * active_size_uv;
    const float2 tgt_uv = active_origin_uv + state.tgt_pts[i] * active_size_uv;
    r_src_pts[i] = (src_uv - target_origin_uv) / target_size_uv;
    r_tgt_pts[i] = (tgt_uv - target_origin_uv) / target_size_uv;
  }
}

static void image_select_warp_restore_all(bContext *C, Image *ima, ImageSelectWarpState *state)
{
  image_select_fragment_restore_source(C, ima, state->iuser, {state->fragment});
  for (ImageSelectWarpTarget &target : state->material_targets) {
    image_select_fragment_restore_source(C, target.image, target.iuser, {target.fragment});
  }
}

static void image_select_warp_commit_material_targets(bContext *C,
                                                      Image *ima,
                                                      ImageSelectWarpState *state,
                                                      ReportList *reports)
{
  /* The undo step must see the original pixels, not the holes used by the floating previews. */
  image_select_warp_restore_all(nullptr, ima, state);

  ImageUser first_iuser = state->iuser;
  first_iuser.tile = state->fragment.geom.tile_number;
  void *first_lock = nullptr;
  ImBuf *first_ibuf = BKE_image_acquire_ibuf(ima, &first_iuser, &first_lock);
  if (first_ibuf) {
    ED_image_undo_push_begin_with_image("Warp Selection", ima, first_ibuf, &first_iuser);
    BKE_image_release_ibuf(ima, first_ibuf, first_lock);
  }
  else {
    ED_image_undo_push_begin("Warp Selection", PaintMode::Texture2D);
  }
  ED_image_undo_capture_selection_mask(ima, state->fragment.geom.tile_number);

  ImageUndoStep *us_open = image_select_undo_session_step_get();
  for (const ImageSelectWarpTarget &target : state->material_targets) {
    image_select_warp_undo_snapshot_fragment(target.image, target.iuser, target.fragment, us_open);
  }

  image_select_warp_commit_write_final(C, ima, state, reports);
  for (ImageSelectWarpTarget &target : state->material_targets) {
    Array<float2> target_src_pts;
    Array<float2> target_tgt_pts;
    image_select_warp_grid_for_target(*state, target.fragment, target_src_pts, target_tgt_pts);

    std::swap(state->fragment, target.fragment);
    std::swap(state->iuser, target.iuser);
    std::swap(state->src_pts, target_src_pts);
    std::swap(state->tgt_pts, target_tgt_pts);
    image_select_warp_commit_write_final(C, target.image, state, reports);
    std::swap(state->tgt_pts, target_tgt_pts);
    std::swap(state->src_pts, target_src_pts);
    std::swap(state->iuser, target.iuser);
    std::swap(state->fragment, target.fragment);
...
/**
 * Runs entirely inside #image_select_fragment_commit_with_undo's self-contained undo step. Unlike
 * move/transform, warp writes in place: #image_select_warp_commit_write_final's row task
 * unconditionally writes every pixel of the captured footprint (warped where the tessellation
 * covers it, an unwarped "restore" sample elsewhere -- see the row task), so no re-clear of the
 * source is needed after the wrapper restores the original there.
 */
void image_select_warp_commit(bContext *C, ImageSelectWarpState *state, ReportList *reports)
{
  ...
  if (!state->material_targets.is_empty()) {
    image_select_warp_commit_material_targets(C, ima, state, reports);
    return;
  }

  image_select_fragment_commit_with_undo(
      C, ima, state->iuser, {state->fragment}, "Warp Selection", [&]() {
        image_select_warp_commit_write_final(C, ima, state, reports);
      });
}
                                                 ...
                                                 ReportList *reports)
{
  ...
  BLI_task_parallel_range(
      0, state->fragment.geom.size_px.y, &task_data, image_select_warp_commit_row_task, &settings);
...
void image_select_warp_session_cancel(bContext *C, SpaceImage *sima)
{
  ...
  image_select_warp_restore_all(C, sima->image, state);
  image_select_warp_state_free(state);
}
...
void PAINT_OT_image_select_warp_confirm(wmOperatorType *ot)
{
   ...
   * #image_select_warp_commit. See the note in #PAINT_OT_image_select_move. */
...
static wmOperatorStatus image_select_warp_cancel_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  image_select_session_clear(state->owner_sima);
  image_select_warp_restore_all(C, state->owner_sima->image, state);
  image_select_warp_state_free(state);
  ARegion *region = CTX_wm_region(C);
  if (region) {
    ED_region_tag_redraw(region);
...
static wmOperatorStatus image_select_warp_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  /* Another tool may still have a session floating in this space; end (commit/discard) it before
   * starting this one. See #image_select_floating_sessions_end. The warp branch above already
   * returned, so the slot is guaranteed empty from here on. */
  ...
  if (!image_select_warp_extract(op, ima, state->iuser, tile_number, true, &state->fragment)) {
    MEM_delete(state);
    return OPERATOR_CANCELLED;
  }
  image_select_warp_init_grid(state);

  image_select_fragment_lift_source(C, ima, state->iuser, {state->fragment});
  for (const ImagePaintSelectionTarget &selection_target : image_paint_selection_targets_get(C,
                                                                                               sima))
  {
    if (selection_target.image == ima) {
      continue;
    }
    ImageSelectWarpTarget target;
    target.image = selection_target.image;
    target.iuser = selection_target.iuser;
    if (!image_select_warp_extract(
            op, target.image, target.iuser, tile_number, false, &target.fragment))
    {
      selection_tile_fragment_free(target.fragment);
      continue;
    }
    image_select_fragment_lift_source(C, target.image, target.iuser, {target.fragment});
    state->material_targets.append(std::move(target));
  }
...
void PAINT_OT_image_select_warp(wmOperatorType *ot)
{
  ...
  /* No OPTYPE_UNDO, see #PAINT_OT_image_select_move and #PAINT_OT_image_select_transform: no
   * image undo step is ever open while this operator runs, so there is nothing meaningful for WM
   * to push on FINISHED. */
  ot->flag = OPTYPE_REGISTER;
}
