...
struct Layout : public Item, NonCopyable, NonMovable {
 ...
 public:
   ...
   * clamped to \a height_px regardless of content, every descendant button is shifted vertically by
   * \a offset_px and flagged #BUT_GRID_SCROLL_CLIP, and the owning block is told to clip those
   * buttons to the visible window. This lets an embedded grid view scroll its rows by sub-row pixel
   * amounts while occupying a fixed slot in the surrounding layout (brush texture image grid).
   *
