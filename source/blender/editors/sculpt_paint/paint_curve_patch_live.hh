...
struct CurvePatchLiveInputs {
  ...
  int blend = -1;
  /** 2D Image Editor only: `Paint::symmetry_flags`, masked to the axes the flat canvas honors
   * (#PAINT_SYMM_X | #PAINT_SYMM_Y -- Z has no meaning on a UV plane). A DIFFERENT field from
   * #symm: that one is the MESH's own symmetry setting the 3D canvas reads, not this brush-level
   * mirror toggle, and the two must not be conflated or a mesh-symmetry change on an unrelated
   * object would re-stamp the 2D canvas for nothing. -1 where the writer does not mirror. */
  int image_symmetry_flags = -1;
