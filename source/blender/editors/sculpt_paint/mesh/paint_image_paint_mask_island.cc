 ...
 * marked after (or without) a re-unwrap still has one welded UV component. Delimiting by
 * #UVDelimitMode::Seam on top makes both cases agree -- the flood fill stops at a seam edge as well
 * as at a UV split, which is also how the 3D Viewport's "Select Linked" in paint modes defines
 * linked faces (`paintface_select_linked_faces`).
 */
                                          ...
                                          Array<bool> &r_island)
{
   ...
   * delimited by seams, see #UVDelimitMode::Seam. */
  Array<bool> island_faces(bm->totface, false);
  ED_uvedit_uv_islands_tag_from_face_indices(
      &scene, bm, offsets, seed_faces, int(UVDelimitMode::Seam), island_faces);
...
static bool paint_mask_island_any_selected(const Span<Object *> objects)
{
  for (Object *ob : objects) {
    if (ed::sculpt_paint::face_selection_state(*id_cast<Mesh *>(ob->data)) ==
        ed::sculpt_paint::FaceSelectionState::Active)
    {
      return true;
    }
                                                     ...
                                                     const eSelectOp sel_op)
{
   ...
   * #VectorSet::add deduplicates. */
  VectorSet<Object *> modified;
  const auto mark_modified = [&](Object *ob) { modified.add(ob); };
