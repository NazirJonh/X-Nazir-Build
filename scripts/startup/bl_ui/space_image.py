...
class IMAGE_HT_header(Header):
    ...
    def draw(self, context):
        ...
        # Shading mode. Not offered over a Render Result or in Mask mode, where enabling it would
        # replace the displayed image with nothing, since the image engine is bypassed entirely.
        if not show_render and not show_maskedit:
            row = layout.row(align=True)
            row.prop(sima, "shading_mode", text="", expand=True)
                ...
                if canvas_source == 'MATERIAL' and ob:
                    ...
                    if has_image_fn is not None:
                        ...
                        have_image = any(
                            ...
                            for channel in ('BASE_COLOR', 'METALLIC', 'ROUGHNESS', 'SPECULAR')
                        )
