...
/** Project onto the roll stroke's frozen projection plane (see `roll_->roll_proj_normal`). */
                                 ...
                                 float y_tilt)
{
  ...
  if (need_roll_mapping()) {
     ...
     * roll_max_points() + roll_->initial_backward_ext_count. When the budget is exceeded we drop
     * the oldest knot:
     *
     ...
     * roll_->stroke_distance_world (raw) and roll_->stroke_distance_normalized (pressure-weighted,
     * for the pressure-scale feature). These offsets are subtracted in spline_uv() so the texture
     * V coordinate stays continuous despite the shrinking polyline. */
    const int budget = roll_max_points() + roll_->initial_backward_ext_count;
    const int total_knots = int(roll_->backward_ext_2d.size()) + num_points_;
    bool wrap_ring = false;

    if (total_knots >= budget) {
      if (!roll_->backward_ext_2d.is_empty()) {
        /* Consume the oldest virtual knot. The ring buffer keeps growing; no real stroke data is
         * lost yet. Accumulate the arc-length of the removed span so that
         * roll_->stroke_distance_world compensates and the texture stays put. */
        float seg_len = 0.0f;
        if (!roll_->roll_spline.lengths_3d.is_empty()) {
          seg_len = roll_->roll_spline.segment_length_3d(0);
        }
        else if (roll_->backward_ext_3d.size() >= 2) {
          seg_len = math::distance(roll_->backward_ext_3d[0], roll_->backward_ext_3d[1]);
        }
        roll_->stroke_distance_world += seg_len;
        /* Normalized distance: use the average pressure of the consumed segment's two endpoints,
         * matching the grid V computation. */
        if (roll_->roll_initial_radius > 0.0f) {
          const float p0 = (!roll_->roll_spline.pressures.is_empty()) ?
                               roll_->roll_spline.pressures[0] :
                               1.0f;
          const float p1 = (roll_->roll_spline.pressures.size() > 1) ?
                               roll_->roll_spline.pressures[1] :
                               p0;
          const float p_avg = std::max((p0 + p1) * 0.5f, 0.01f);
          roll_->stroke_distance_normalized += seg_len / (roll_->roll_initial_radius * p_avg);
        }
        roll_->backward_ext_2d.remove(0);
        roll_->backward_ext_3d.remove(0);
      }
      else {
        /* All virtual knots consumed -- ring buffer wraps, oldest real point is abandoned.
         * Accumulate its distance so roll_->stroke_distance_world stays correct. */
...
void PaintStroke::prepend_virtual_roll_points()
{
  ...
  roll_->backward_ext_2d.clear();
  roll_->backward_ext_3d.clear();
  generate_virtual_extension(m_start,
                             ...
                             step_3d,
                             -1.0f,
                             roll_->backward_ext_2d,
                             roll_->backward_ext_3d);
  roll_->initial_backward_ext_count = n_backward;
}

void PaintStroke::make_roll_spline(bContext * /*C*/)
{
  if (!need_roll_mapping() || num_points_ < 4) {
    return;
  }

  if (!roll_->roll_virtual_prepended) {
    roll_->roll_virtual_prepended = true;
  ...
  roll_->roll_spline.clear();
  roll_->roll_spline.poly_2d.extend(roll_->backward_ext_2d);
  roll_->roll_spline.poly_3d.extend(roll_->backward_ext_3d);
  ...
  {
    const int n_back = int(roll_->backward_ext_2d.size());
    ...
    extrapolate_pressures(base_p, ratio, n_back, true, roll_->roll_spline.pressures);
    ...
    for (int i = 0; i < n_back; i++) {
      roll_->roll_spline.normals.append(back_n);
    }
  }
  for (int i = 0; i < num_points_; i++) {
    const int idx = (oldest + i) % buf_cap;
    roll_->roll_spline.poly_2d.append(points_[idx].mouse_out);
    roll_->roll_spline.poly_3d.append(points_[idx].location);
    roll_->roll_spline.pressures.append(points_[idx].pressure);
    roll_->roll_spline.normals.append(points_[idx].surface_normal);
  }

  roll_->n_virtual_poly_points = int(roll_->backward_ext_2d.size());

  roll_->roll_spline.update_lengths();
   ...
   * roll_->stroke_distance_world compensates; updating roll_->roll_virtual_length here would cause
   * double-counting. */
  if (roll_->roll_virtual_length == 0.0f && roll_->n_virtual_poly_points > 0 &&
      roll_->n_virtual_poly_points - 1 < int(roll_->roll_spline.lengths_3d.size()))
  {
    roll_->roll_virtual_length = roll_->roll_spline.lengths_3d[roll_->n_virtual_poly_points - 1];
    /* Compute normalized virtual length for pressure-scaled V offset. This is the
     * pressure-weighted equivalent of roll_->roll_virtual_length: sum of seg_len / (R * p_avg)
     * over the virtual extension. It must use the same midpoint-pressure formula as the grid V
     * coords and as roll_->stroke_distance_normalized in add_roll_point(), so the subtraction in
     * spline_uv() cancels exactly. */
    if (roll_->roll_initial_radius > 0.0f) {
      const Span<float> pres = roll_->roll_spline.pressures.as_span();
      float accum = 0.0f;
      for (int i = 1;
           i <= roll_->n_virtual_poly_points - 1 && i < int(roll_->roll_spline.lengths_3d.size());
           i++)
      {
        const float seg = roll_->roll_spline.segment_length_3d(i);
        const float p = (i < int(pres.size()) && i - 1 >= 0) ?
                            std::max((pres[i] + pres[i - 1]) * 0.5f, 0.01f) :
                            1.0f;
        accum += seg / (roll_->roll_initial_radius * p);
      }
      roll_->roll_virtual_length_normalized = accum;
    }
  }
}
                                     ...
                                     float pressure)
{
   ...
   *    polyline. */
  if (num_points_ >= 2 && !roll_->roll_spline.is_empty()) {
    constexpr int cap = PAINT_MAX_INPUT_SAMPLES;
    ...
    if (len3 > 1e-7f && len2 > 1e-7f) {
      ...
      generate_virtual_extension(m_end,
                                 ...
                                 step_3d,
                                 +1.0f,
                                 roll_->roll_spline.poly_2d,
                                 roll_->roll_spline.poly_3d);
      /* Extrapolate pressure trend for forward virtual extension. */
      {
        ...
        extrapolate_pressures(p_new, ratio, n_forward, false, roll_->roll_spline.pressures);
      }
      /* Forward extension: use newest point's surface normal. */
      for (int i = 0; i < n_forward; i++) {
        roll_->roll_spline.normals.append(points_[newest].surface_normal);
      }
   ...
   *  (a) roll_->last_painted_roll_idx exists -- continue from there.
  ...
  int flush_start;
  if (roll_->last_painted_roll_idx >= 0) {
    flush_start = roll_->last_painted_roll_idx;
  }
...
{
  {
...

    {
       ...
       *
       * The matching offset (roll_->stroke_distance_normalized) is accumulated in add_roll_point()
       * using the same (p0+p1)/2 averaging, and applied in spline_uv() to keep the texture
       * continuous as the polyline shrinks. */
...
void PaintStroke::compute_roll_center(StrokeCache &cache)
{
  ...
  RollCenterBuild build;
  if (!roll_center_locate_on_spline(roll_->roll_spline, cache, build)) {
    return;
  }
  ...
  roll_borders_build(roll_->roll_spline,
                     brush,
                     roll_->roll_proj_normal,
                     roll_->roll_initial_radius,
                     cache,
                     build);
  roll_borders_fix_self_intersections(roll_->roll_spline, build, cache);
  roll_grid_and_lut_build(roll_->roll_spline, build, cache);
}
                            ...
                            float r_tan[3]) const
{
   ...
   *
   * Deliberately NOT guarded on #need_roll_mapping: this runs once per vertex per dab, and the
   * caller already gates on it (`sculpt_apply_texture()`, `mesh/sculpt.cc`). The `roll_` reads at
   * the end of this function rely on that gate. */
  ...
  const bool use_norm_v = brush && brush->roll_pressure_scale &&
                          BKE_brush_use_size_pressure(brush);
  if (use_norm_v) {
    r_out[1] += roll_->stroke_distance_normalized - roll_->roll_virtual_length_normalized;
  }
  else {
    r_out[1] += roll_->stroke_distance_world - roll_->roll_virtual_length;
  }
}

float PaintStroke::spline_length() const
{
  return roll_ ? roll_->roll_spline.total_length_3d() : 0.0f;
}
                                              ...
                                              Vector<float> &r_radii) const
{
  r_positions.clear();
  r_radii.clear();
  if (!need_roll_mapping() || num_points_ < 2) {
    return;
  }
   ...
   * `roll_->roll_spline.poly_3d`, never in `points_`, so they are excluded by construction. */
  ...
  const float3 plane_normal = math::length_squared(roll_->roll_proj_normal) > 1e-12f ?
                                  math::normalize(roll_->roll_proj_normal) :
                                  float3(0.0f);
  const float scale = roll_->roll_initial_radius > 1e-6f ? roll_->roll_initial_radius :
...
{
  if (!need_roll_mapping() || roll_->roll_spline.is_empty()) {
    return;
  }
  ...
  /* --- 2D overlay: center polyline + tick marks --- */
  {
    ...
    const int n_pts = int(roll_->roll_spline.poly_2d.size());

    /* Polyline: red = virtual extension, green = real. */
    GPU_line_width(3.0f);
    if (roll_->n_virtual_poly_points > 1) {
      immUniformColor4ub(255, 50, 50, 200);
      const int cnt = std::min(roll_->n_virtual_poly_points + 1, n_pts);
      immBegin(GPU_PRIM_LINE_STRIP, cnt);
      for (int i = 0; i < cnt; i++) {
        immVertex2f(
            pos_attr, roll_->roll_spline.poly_2d[i].x + ox, roll_->roll_spline.poly_2d[i].y + oy);
      }
      immEnd();
    }
    if (roll_->n_virtual_poly_points < n_pts) {
      immUniformColor4ub(50, 255, 50, 200);
      const int cnt = n_pts - roll_->n_virtual_poly_points;
      immBegin(GPU_PRIM_LINE_STRIP, cnt);
      for (int i = roll_->n_virtual_poly_points; i < n_pts; i++) {
        immVertex2f(
            pos_attr, roll_->roll_spline.poly_2d[i].x + ox, roll_->roll_spline.poly_2d[i].y + oy);
      }
      immEnd();
    ...
    for (int i = 0; i < n_pts; i++) {
      const float2 tan = roll_->roll_spline.tangent_2d_at_index(std::min(i, n_pts - 2));
      const float2 perp(-tan.y, tan.x);
      const float2 &pt = roll_->roll_spline.poly_2d[i];
    ...
    if (roll_->n_virtual_poly_points > 0 && roll_->n_virtual_poly_points < n_pts) {
      GPU_line_width(2.5f);
      immUniformColor4ub(255, 255, 0, 255);
      const float2 tan = roll_->roll_spline.tangent_2d_at_index(roll_->n_virtual_poly_points);
      const float2 perp(-tan.y, tan.x);
      const float2 &pt = roll_->roll_spline.poly_2d[roll_->n_virtual_poly_points];
      immBegin(GPU_PRIM_LINES, 2);
...
void PaintStroke::draw_roll_preview(bContext *C) const
{
  ...
  int painted_poly;
  if (roll_->last_painted_roll_idx < 0 || num_points_ < half) {
    painted_poly = roll_->n_virtual_poly_points;
  }
  else {
    const int oldest_idx = (cur_point_ - num_points_ + buf_cap) % buf_cap;
    const int dist = (roll_->last_painted_roll_idx - oldest_idx + buf_cap) % buf_cap;
    painted_poly = roll_->n_virtual_poly_points + dist;
...
void PaintStroke::init_roll_cursors()
{
  ...
  roll_->roll_cursor = WM_paint_cursor_activate(
  ...
  if (U.experimental.use_paint_debug) {
    roll_->debug_cursor = WM_paint_cursor_activate(
