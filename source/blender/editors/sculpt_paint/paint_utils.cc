...
{
  const float gradient_spacing = (brush.gradient_spacing > 0) ? float(brush.gradient_spacing) : 1.0f;
    ...
    const int8_t radial_symmetry[3])
{
  ...
  const auto evaluate_with_mirror = [&](const float3 &sample_position) {
    ...
    for (int symm_iter = 1; symm_iter <= symmetry_mask; symm_iter++) {
      ...
      const float3 symm_position =
          ed::sculpt_paint::symmetry_flip(sample_position, ePaintSymmetryFlags(symm_iter));
    ...
    const int8_t radial_symmetry[3])
{
  ...
  const bool radial_sym_enabled = (radial_symmetry[0] != 1 || 
                                   radial_symmetry[1] != 1 || 
                                   radial_symmetry[2] != 1);
...
bool paint_brush_color_varies_during_stroke(const Paint &paint,
                                            ...
                                            const float pressure)
{
  const bool gradient_updates_color =
