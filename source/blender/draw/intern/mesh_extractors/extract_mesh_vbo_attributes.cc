                                   ...
                                   uint32_t len)
{
   ...
   *
   * Resolved against the tool settings the render data was built with, so an add-on-managed layer
   * attribute bound to a channel gets that channel's shader input too. */
  const PaintModeSettings *paint_mode = mr.toolsettings ? &mr.toolsettings->paint_mode : nullptr;
  if (const char *alias = DRW_material_paint_vertex_input_alias(name, paint_mode)) {
