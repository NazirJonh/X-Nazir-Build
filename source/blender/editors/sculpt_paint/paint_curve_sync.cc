                                                  ...
                                                  Vector<PaintCurvePoint> &r_screen_points)
{
  ...
  for (const int i : IndexRange(point_num)) {
    ...
    /* Object-space curves require a 3D viewport projection matrix. A PaintCurve can be shared
     * between paint modes, so it may be encountered in the Image Editor without one. Keep such
     * points outside the editable region instead of calling #ED_view3d_project_v2, which reads an
     * uninitialized GPU matrix in that context. */
    if (vc == nullptr || vc->region == nullptr || vc->rv3d == nullptr) {
      for (const int j : IndexRange(3)) {
        pcp.bez.vec[j][0] = -1e6f;
        pcp.bez.vec[j][1] = -1e6f;
      }
      continue;
    }
                                       ...
                                       const bool to_3d_space)
{
  ...
  /* Only the object-to-world direction still needs a raw pointer, for the C-style `mul_v3_m4v3`
   * calls below; the inverse is consumed through `math::transform_point(world_to_ob, ...)`. */
