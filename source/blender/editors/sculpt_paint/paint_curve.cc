...
static int paintcurve_find_in_screen_points(const Span<PaintCurvePoint> screen_points,
                                            const float pos[2],
                                 ...
                                 const bool snap_to_surface = false)
{
  ...
  /* Compute 3D object-space position. */
  float obj_co[3];
...
static wmOperatorStatus paintcurve_duplicate_exec(bContext *C, wmOperator *op)
{
  ...
  /* Collect splines that have at least one selected point. */
  const OffsetIndices<int> points_by_curve = geom.points_by_curve();
  Vector<int> curves_to_dup;
  for (const int curve_i : geom.curves_range()) {
    for (const int pt_i : points_by_curve[curve_i]) {
      if (paintcurve_geom_get_selection(geom, pt_i) & 0x07) {
        curves_to_dup.append(curve_i);
        break;
      }
    }
  }

  if (curves_to_dup.is_empty()) {
    return OPERATOR_CANCELLED;
  }

  ED_paintcurve_undo_push_begin(C, op->type->name);

  /* Cache all source data before any resize invalidates internal spans. */
  struct SplineData {
    int point_num;
    bool cyclic;
    Vector<float3> positions;
    Vector<float3> handles_left;
    Vector<float3> handles_right;
    Vector<int8_t> types_left;
    Vector<int8_t> types_right;
    Vector<float> radii;
  };
  Vector<SplineData> src;
  src.reserve(curves_to_dup.size());
  {
    const MutableSpan<float3> pos = geom.positions_for_write();
    const MutableSpan<float3> hl = geom.handle_positions_left_for_write();
    const MutableSpan<float3> hr = geom.handle_positions_right_for_write();
    const MutableSpan<int8_t> tl = geom.handle_types_left_for_write();
    const MutableSpan<int8_t> tr = geom.handle_types_right_for_write();
    const MutableSpan<float> rad = geom.radius_for_write();
    const VArray<bool> cyc = geom.cyclic();
    for (const int curve_i : curves_to_dup) {
      const IndexRange pts = points_by_curve[curve_i];
      src.append(SplineData());
      SplineData &d = src.last();
      d.point_num = pts.size();
      d.cyclic = cyc[curve_i];
      d.positions.extend(pos.slice(pts));
      d.handles_left.extend(hl.slice(pts));
      d.handles_right.extend(hr.slice(pts));
      d.types_left.extend(tl.slice(pts));
      d.types_right.extend(tr.slice(pts));
      d.radii.extend(rad.slice(pts));
    }
  }

  /* Deselect all originals; the duplicates will receive selection instead. */
  paintcurve_geom_set_all_selection(geom, 0);

  /* Compute total new points and resize geometry once. */
  int total_new_points = 0;
  for (const SplineData &d : src) {
    total_new_points += d.point_num;
  }
  const int old_points = geom.points_num();
  const int old_curves = geom.curves_num();
  const int new_curves_num = old_curves + int(curves_to_dup.size());
  geom.resize(old_points + total_new_points, new_curves_num);

  /* Set up per-curve attributes for new splines. */
  {
    MutableSpan<int> offsets = geom.offsets_for_write();
    MutableSpan<int8_t> curve_types = geom.curve_types_for_write();
    MutableSpan<int> resolutions = geom.resolution_for_write();
    MutableSpan<bool> cyclic = geom.cyclic_for_write();
    int write_pt = old_points;
    for (const int i : src.index_range()) {
      const int ci = old_curves + i;
      offsets[ci] = write_pt;
      curve_types[ci] = CURVE_TYPE_BEZIER;
      resolutions[ci] = PAINT_CURVE_NUM_SEGMENTS;
      cyclic[ci] = src[i].cyclic;
      write_pt += src[i].point_num;
    }
  }

  /* Copy point attributes for each duplicated spline. */
  {
    MutableSpan<float3> positions = geom.positions_for_write();
    MutableSpan<float3> handles_left = geom.handle_positions_left_for_write();
    MutableSpan<float3> handles_right = geom.handle_positions_right_for_write();
    ...
    int write_pt = old_points;
    for (const SplineData &d : src) {
      const IndexRange dst(write_pt, d.point_num);
      positions.slice(dst).copy_from(d.positions.as_span());
      handles_left.slice(dst).copy_from(d.handles_left.as_span());
      handles_right.slice(dst).copy_from(d.handles_right.as_span());
      types_left.slice(dst).copy_from(d.types_left.as_span());
      types_right.slice(dst).copy_from(d.types_right.as_span());
      radii.slice(dst).copy_from(d.radii.as_span());
      write_pt += d.point_num;
    }
  }

  /* Select all handles on the new points. */
  for (const int pt_i : IndexRange(old_points, total_new_points)) {
    paintcurve_geom_set_selection(geom, pt_i, 0x07);
  }

  /* Curve types were written directly via #curve_types_for_write, so the cached per-type
   * counts in the runtime are now stale. Recompute them before anything queries
   * #has_curve_with_type (e.g. #calculate_bezier_auto_handles during the following transform). */
  geom.update_curve_types();
  geom.tag_topology_changed();

  /* Set first duplicate as the active spline. */
...
{
  /* identifiers */
  ot->name = "Duplicate Paint Curve Spline";
  ot->description = "Duplicate selected splines of the paint curve";
    ...
    bContext *C, wmOperator *op, PaintCurve *pc, const int segment_index, const float edge_t)
{
  ...
  const bke::curves::bezier::Insertion inserted = bke::curves::bezier::insert(
      ...
      geom.handle_positions_right().value()[segment_index],
      geom.handle_positions_left().value()[point_i2],
    ...
    case MOUSEMOVE: {
      ...
      if (psd->use_3d_view) {
        ...
        if (psd->is_segment) {
          ...
          const bool snap_to_surface = (event->modifier & (KM_CTRL | KM_SHIFT)) ==
                                       (KM_CTRL | KM_SHIFT);
          bool snapped = false;
...
static wmOperatorStatus paintcurve_to_curve_object_exec(bContext *C, wmOperator *op)
{
  ...
  if (!paintcurve_sync_to_source_object(C, pc)) {
