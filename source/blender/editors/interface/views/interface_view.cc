...
bool region_view_item_topmost_at(const ARegion *region,
                                 ...
                                 const StringRef idname)
{
  /* #but_find_mouse_over is the same top-most hit test the window manager uses to route a press, so
   * a button drawn over the tile (the overlay scrollbar) or below the grid (the resize grip) is
   * returned instead of the tile behind it. #region_view_item_has_idname_at cannot be used here: it
   * searches view-item buttons only and so reports the tile even when another widget covers it. */
