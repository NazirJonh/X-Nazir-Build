    ...
    switch (event->type) {
      ...
      case INBETWEEN_MOUSEMOVE: {
        ...
        if (gesture->move) {
          ...
          /* The segment being drawn starts at the last confirmed point. */
          const int2 anchor(border[gesture->points - 1][0], border[gesture->points - 1][1]);
          const float2 raw_point(raw_mval);

          if (event->modifier & KM_SHIFT) {
            /* Keep the segment on the direction it had when Shift was first pressed, so only its
             * length follows the cursor. */
            if (!gesture->use_angle_lock) {
              const float2 direction = raw_point - float2(anchor);
              const float direction_len = math::length(direction);
              if (direction_len > 0.01f) {
                gesture->use_angle_lock = true;
                gesture->angle_lock_anchor = anchor;
                gesture->angle_lock_direction = direction / direction_len;
              }
            }

            if (gesture->use_angle_lock) {
              const float2 lock_anchor(gesture->angle_lock_anchor);
              const float length = std::max(
                  0.0f, math::dot(raw_point - lock_anchor, gesture->angle_lock_direction));
              const float2 locked_point = lock_anchor + gesture->angle_lock_direction * length;
              gesture->mval = int2(int(locked_point.x), int(locked_point.y));
            }
            else {
              gesture->mval = raw_mval;
            }
            ...
            if (gesture->use_snap) {
              const float2 snapped = wm_gesture_snap_point_to_angle(
                  float2(anchor), raw_point, snap_angle.increment);
              gesture->mval = int2(int(snapped.x), int(snapped.y));
            }
            else {
              gesture->mval = raw_mval;
            }
