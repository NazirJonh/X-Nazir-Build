...
{
  return image_select_session_active(CTX_wm_space_image(C)) != nullptr;
}
...
bool image_select_canvas_paint_blocked(bContext *C)
{
  ...
  if (image_select_session_active(sima) != nullptr) {
    return true;
  }
  return image_select_canvas_borrowed_elsewhere(sima->image, sima);
}

/** \} */

/* -------------------------------------------------------------------- */
/** \name Tool dispatch table
 *
 * #PaintSelectFloatingSession is deliberately non-polymorphic (see the file header): the concrete
 * state is only ever reached through the tool that defines it, via
 * #PaintSelectFloatingSession::tool. This table is the single place that maps the tag to each
 * tool's three teardown entry points, so adding a fifth floating tool means filling one row here
 * instead of adding a case to every `switch` below (and risking one of them silently falling
 * through on a forgotten case).
 * \{ */

struct PaintSelectToolType {
  void (*end_for_takeover)(bContext *C, SpaceImage *sima);
  void (*cancel)(bContext *C, SpaceImage *sima);
  void (*free)(PaintSelectFloatingSession *session);
};

/* Indexed by #PaintSelectTool; keep in the same order as that enum. */
static const PaintSelectToolType g_tool_types[] = {
    /* Move */
    {image_select_move_session_end_for_takeover,
     image_select_move_session_cancel,
     image_select_move_session_free},
    /* Transform */
    {image_select_transform_session_end_for_takeover,
     image_select_transform_session_cancel,
     image_select_transform_session_free},
    /* Gradient */
    {image_select_gradient_session_end_for_takeover,
     image_select_gradient_session_cancel,
     image_select_gradient_session_free},
    /* Warp */
...
{
  const int64_t index = int64_t(tool);
  BLI_assert(index >= 0 && index < int64_t(ARRAY_SIZE(g_tool_types)));
  return g_tool_types[index];
}
...
void image_select_floating_sessions_end_all(bContext *C, SpaceImage *sima)
{
  ...
  image_select_tool_type_get(session->tool).end_for_takeover(C, sima);
}
...
void image_select_floating_sessions_cancel(bContext *C, SpaceImage *sima)
{
  ...
  image_select_tool_type_get(session->tool).cancel(C, sima);
}
...
void image_select_floating_session_free(PaintSelectFloatingSession *session)
{
  ...
  image_select_tool_type_get(session->tool).free(session);
}
