                                           ...
                                           const float3 &fallback)
{
   ...
   * degrades gracefully for a nearly planar one, and view-independent. */
  ...
  if (math::length_squared(normal) < 1e-12f) {
    /* A straight curve spans no plane at all, which is not a rare case -- every anchor stroke
     * starts as a two-point segment. The caller's own measurement (the stroke's surface normal,
     * or the roll's frozen projection normal) describes the surface far better than any constant
     * this function could pick, so it decides. */
