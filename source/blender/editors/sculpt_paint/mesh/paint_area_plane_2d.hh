...
class AreaPlaneMesh {
  ...
  /** Object-space median triangle edge length; the symmetry snap scale. */
  float edge_length_median_ = 0.0f;
  /** Lazily built on the first #closest_hit; mirrored dabs need a 3D nearest query. */
  mutable BVHTree *bvh_ = nullptr;
  mutable bool bvh_built_ = false;

  void build_uv_bins();
  void ensure_bvh() const;
 ...
 public:
  ...
  /* Owns a #BVHTree; copying would double-free it. The stroke holds this in a shared_ptr. */
  AreaPlaneMesh(const AreaPlaneMesh &) = delete;
  AreaPlaneMesh &operator=(const AreaPlaneMesh &) = delete;

  bool is_valid() const;

  /**
   * Median triangle edge length in object space, computed once on construction.
   *
   * The symmetry snap tolerance is derived from this rather than from the brush radius,
   * because a mirrored point misses the surface by an amount set by how asymmetric the mesh
   * is — not by how large the brush is or how big the destination image is. Several material
   * channels share one mirrored-dab lookup, so that tolerance must not depend on any one
   * channel's image.
   *
   * Zero when the mesh has no usable triangles.
   */
  float edge_length_median() const;
  ...
  /**
   * Triangle whose closest object-space point to \a position lies within \a max_distance, with
   * that point's UV and interpolated normal.
   *
   * Lands a mirrored dab center back on the surface. A reflected point is only exactly on the
   * mesh when the mesh is perfectly symmetric, so this is a nearest query in 3D — never a UV
   * lookup, and never an extrapolation into empty UV.
   *
   * Degenerate triangles are skipped: they carry no usable frame and would report a spurious
   * zero distance along their collapsed edge. On equal distance the smallest index wins,
   * matching #hit_at_uv's tie rule so a dab does not jump between coincident faces mid-stroke.
   *
   * \a r_hit.tri_index is an index into #triangles(), the same convention #hit_at_uv uses.
   *
   * \return false when nothing usable is within \a max_distance.
   */
  bool closest_hit(const float3 &position,
                   float max_distance,
                   AreaPlaneHit &r_hit,
                   float2 &r_uv) const;
 ...
 *
 * \param mirrored: build the reflected frame, for a symmetry dab on the far side of the mesh.
 * The frame is rebuilt here rather than transformed by a reflection matrix, because this
 * function derives #AreaPlaneFrame.axis_z from the UV Jacobian and would overwrite any axes
 * handed to it.
 */
