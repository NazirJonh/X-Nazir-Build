...
{
  /* Match the proportions of the axis handles in the 3D Viewport Navigate Gizmo. */
  const float axis_size = 60.0f * UI_SCALE_FAC;
  const float axis_handle_size = 0.15f;
  ...
  float x_color[3];
  float y_color[3];
  ui::theme::get_color_3fv(TH_AXIS_X, x_color);
  ui::theme::get_color_3fv(TH_AXIS_Y, y_color);
   ...
   * top of the image. */
  {
    ...
    immUniform1f("lineWidth", (axis_size / 40.0f) * U.pixelsize);
  ...
  {
    const float cross_size = axis_size * 0.10f;
  ...
  auto draw_axis_circle = [&](const float center[2], const float color[3]) {
    ...
    ui::draw_roundbox_4fv_ex(
        &rect, fill_color, nullptr, 0.0f, fill_color, (axis_size / 60.0f) * U.pixelsize, rad);
  ...
  /* Axis labels, centered in the end caps. */
  {
    ...
    BLF_size(font_id, int(axis_size * axis_handle_size * 1.25f));
    const struct {
      const float *pos;
      const char *label;
...
static void image_rotate_draw_dial()
{
  ...
  /* Arc plus an arrow head at its end, indicating the drag direction. */
  {
    ...
    GPU_viewport_size_get_f(viewport_size);
    immUniform2fv("viewportSize", &viewport_size[2]);
    immUniform1f("lineWidth", 1.0f * UI_SCALE_FAC);
    immUniform1i("lineSmooth", 1);
    ...
    const float arrow_size = 5.0f * UI_SCALE_FAC;
