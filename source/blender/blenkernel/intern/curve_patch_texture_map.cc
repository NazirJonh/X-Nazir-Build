...
int curve_patch_stamp_pick_texture(const Span<float> weights_cdf, const float random01)
{
  ...
  /* Clamped rather than asserted: `random01` comes from a hash and a caller is free to pass
   * exactly 1.0, which would otherwise select past the last slot. */
                                    ...
                                    const bool cyclic)
{
   ...
   * holds a WHOLE number of tiles -- otherwise the last tile is cut mid-pattern and shows as a
   * seam. Repeat and Stretch already divide the length into an integer count and come out of the
   * snap unchanged; Default, whose tile follows the brush radius, generally does not, and gets its
   * tile stretched or squeezed by up to ~1.5x to the nearest whole count. */
                                                        ...
                                                        const bool cyclic)
{
  ...
  if (caps_enabled) {
     ...
     * `total_length` would divide `total_length` by a near-zero `caps_total` and produce
     * `inf`/NaN. Skipping the scale here is safe: `start_len`/`end_len` are left at their raw
     * (tiny) values, and #zone_single_tile_v independently guards its own near-zero `span` by
     * returning `0.0f` instead of dividing by it, so no NaN or divide-by-zero can reach the caller
     * even though `end_begin` below may go slightly negative in this regime. */
   ...
   * that yields `2n - 1`, congruent to `-1` modulo the texture's period of 2: the pattern closes
   * on itself. Dropping the `- 1` would put the join mid-tile and, in Stretch/Default (which do
   * not wrap `v` below), push the whole loop outside the tile the texture actually occupies. */
  float v = tile_span > 1e-8f ?
                (middle_cyclic ? middle_offset / tile_span * 2.0f - 1.0f :
                                 (middle_offset - middle_length * 0.5f) / tile_span * 2.0f) :
                0.0f;
