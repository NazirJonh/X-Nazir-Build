...
/* Owning payload for a glyph search result button.
 *
 * This is passed as the auto-managed `func_argN` of the button, so the block owns it and frees it
 * on destruction (via the registered free function) regardless of whether the button is clicked.
 * It carries everything the result callback needs, including an optional custom callback that is
 * invoked on click. */
struct GlyphSearchResultButtonData {
  std::string glyph_unicode;
  std::string glyph_propname;
  std::string search_propname;
  wmOperator *target_op = nullptr;
  IDProperty *target_op_properties = nullptr;
  /* When set, this custom callback is invoked on click instead of the default behavior. */
  ButtonHandleFunc custom_callback = nullptr;
  void *custom_callback_user_data = nullptr;
};

/* Free function registered with `button_funcN_set` for #GlyphSearchResultButtonData. */
static void glyph_search_result_button_data_free(void *argN)
{
  MEM_delete(static_cast<GlyphSearchResultButtonData *>(argN));
}

/* Copy function registered with `button_funcN_set` for #GlyphSearchResultButtonData. */
static void *glyph_search_result_button_data_copy(const void *argN)
{
  return MEM_new<GlyphSearchResultButtonData>(
      __func__, *static_cast<const GlyphSearchResultButtonData *>(argN));
}
...
/* Callback for glyph search result buttons, dispatched via the auto-managed `func_argN`.
 *
 * The payload (#GlyphSearchResultButtonData) is owned by the block and freed automatically, so this
 * function never frees it. When a custom callback was provided it is invoked with a freshly
 * allocated copy of the glyph unicode (which the custom callback owns and frees); otherwise the
 * default behavior writes the selected glyph to the target property. */
static void glyph_template_result_button_cb(bContext *C, void *argN, void * /*arg2*/)
{
  GlyphSearchResultButtonData *data = static_cast<GlyphSearchResultButtonData *>(argN);
  if (!data || data->glyph_unicode.empty()) {
    return;
  }

  if (data->custom_callback) {
    /* Hand the custom callback its own heap copy of the unicode; it owns and frees it. */
    char *glyph_unicode_copy = BLI_strdupn(data->glyph_unicode.c_str(),
                                           data->glyph_unicode.length());
    data->custom_callback(C, data->custom_callback_user_data, glyph_unicode_copy);
    return;
  }

  const char *glyph_unicode = data->glyph_unicode.c_str();
  ...
  const char *glyph_propname = !data->glyph_propname.empty() ? data->glyph_propname.c_str() :
                                                               "glyph";

  const char *search_propname = !data->search_propname.empty() ? data->search_propname.c_str() :
                                                                 "glyph_search";
  ...
  /* The payload is owned by the block and freed automatically; nothing to free here. */
}
                                             ...
                                             void *callback_user_data)
{
  ...
  if (more_glyphs_callback) {
    ...
    if (category && category[0] != '\0') {
      const size_t category_size = strlen(category) + 1;
      data->category = MEM_new_array<char>(category_size, __func__);
      memcpy(data->category, category, category_size);
    }
    else {
    ...
    const size_t glyph_propname_size = strlen(glyph_propname) + 1;
    data->glyph_propname = MEM_new_array<char>(glyph_propname_size, __func__);
    memcpy(data->glyph_propname, glyph_propname, glyph_propname_size);
                                                  ...
                                                  void *callback_user_data)
{
  ...
  if (search_results.is_empty()) {
    ...
    for (const auto &result : search_results) {
      ...
      /* Build the owning payload once and let the block manage its lifetime. This avoids the
       * accumulating per-redraw leak that occurred when the data was only freed on click. The same
       * callback handles both the custom-callback and the default branch. */
      GlyphSearchResultButtonData *button_data = MEM_new<GlyphSearchResultButtonData>(__func__);
      button_data->glyph_unicode = glyph_unicode;

      if (result_callback) {
        button_data->custom_callback = result_callback;
        button_data->custom_callback_user_data = callback_user_data;
      }
      else {
        button_data->glyph_propname = (glyph_propname && glyph_propname[0] != '\0') ? glyph_propname :
                                                                                      "glyph";
        button_data->search_propname = (search_propname && search_propname[0] != '\0') ?
                                           search_propname :
                                           "glyph_search";
        button_data->target_op = glyph_find_operator_from_properties_ptr(C, ptr);
        button_data->target_op_properties = static_cast<IDProperty *>(ptr->data);
      }

      button_funcN_set(result_but,
                       glyph_template_result_button_cb,
                       button_data,
                       nullptr,
                       glyph_search_result_button_data_free,
                       glyph_search_result_button_data_copy);
    }
  }
}
