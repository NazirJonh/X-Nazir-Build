...
def _on_extension_repos_update_post(dummy=None):
    ...
    try:
        ...
        category_debug_print(
            f"[GLYPH EXTENSION UPDATE DEBUG] before merge: cache={len(cache_before)}"
        )
        ...
        category_debug_print(
            ...
            f"merge_result={merge_result}, cache={len(cache_after)}, "
...
def _on_version_update(dummy):
    """No-op on blend load: load_post runs discovery + WM sync immediately after.

    BKE_CB_EVT_VERSION_UPDATE always precedes BKE_CB_EVT_LOAD_POST when a blend file
    is loaded (wm_files.cc). Heavy discovery here duplicated load_post work (~600ms+
    of triple _discover_active_categories plus icon detection per startup).
    """
    category_debug_print("[GLYPH VERSION UPDATE] Skipped — load_post will sync")
