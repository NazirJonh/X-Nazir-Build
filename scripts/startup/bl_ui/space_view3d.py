...
class VIEW3D_MT_curves_vertex_group(Menu):
    ...
    def draw(self, context):
        ...
        if ob and ob.mode == "WEIGHT_CURVES":
            layout.operator("curves.vertex_group_add", text="Add New Group")
            layout.operator("curves.vertex_group_remove", text="Remove Group")
        else:
            layout.operator("object.vertex_group_add", text="Add New Group")
...
class VIEW3D_MT_weight_curves(Menu):
    ...
    def draw(self, context):
        ...
        layout.operator("curves.vertex_group_normalize_all", text="Normalize All")
        layout.operator("curves.vertex_group_normalize", text="Normalize")

        layout.separator()

        layout.operator("curves.weight_invert", text="Invert")
        layout.operator("curves.vertex_group_smooth", text="Smooth")

        layout.separator()

        layout.operator("curves.weight_sample", text="Sample Weight")
        layout.operator("curves.weight_toggle_direction", text="Toggle Direction")
...
class VIEW3D_PT_curves_weight_context_menu(Panel):
    bl_space_type = "VIEW_3D"
    bl_region_type = "WINDOW"
    bl_label = "Weight Curves"

    def draw(self, context):
        layout = self.layout

        brush = context.tool_settings.curves_weight_paint.brush
        if brush is None:
            return
