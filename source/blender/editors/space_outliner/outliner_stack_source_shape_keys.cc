...
Key *shape_keys_get(const StackReadContext &ctx, const StackFocus &focus)
{
  Object *object = outliner_stack_focus_object_get(ctx, focus);
  return object != nullptr ? BKE_key_from_object(object) : nullptr;
}

/**
 * A #StackRow::stable_id for a shape key, which has no UUID of its own.
 *
 * Deterministic from the name rather than random: a shape key's name is unique within its #Key,
 * so hashing it gives an id that is stable across a rebuild without having to store anything new
 * on #KeyBlock. Renaming the key changes it, which just means the row's collapsed and selected
 * state resets on rename -- an acceptable trade for not touching the DNA struct.
 */
bUUID stable_id_from_name(const StringRef name)
{
  const uint64_t low = get_default_hash(name);
  const uint64_t high = get_default_hash(name, 1);
  bUUID id{};
  id.time_low = uint32_t(low);
  id.time_mid = uint16_t(low >> 32);
  id.time_hi_and_version = uint16_t(low >> 48);
  id.clock_seq_hi_and_reserved = uint8_t(high);
  id.clock_seq_low = uint8_t(high >> 8);
  for (int i = 0; i < 6; i++) {
    id.node[i] = uint8_t(high >> (16 + i * 8));
  }
  return id;
}

class ShapeKeyStackSource final : public StackSource, public StackEditor {
 public:
  ...
  {
    Key *key = shape_keys_get(ctx, focus);
                  ...
                  Vector<StackRow> &r_rows) const override
  {
    ...
    for (KeyBlock &block : key.block) {
      ...
      StackRow row;
      row.ordinal = ordinal;
      row.stable_id = stable_id_from_name(block.name);
      row.name = block.name;
  ...
  {
    Object *object = outliner_stack_focus_object_get(outliner_stack_read_context(C), focus);
    if (object == nullptr) {
      return false;
    }
    object->shapenr = short(ordinal + 1);
    DEG_id_tag_update(&object->id, ID_RECALC_GEOMETRY);
    WM_event_add_notifier(&C, NC_OBJECT | ND_DATA, &object->id);
    return true;
  }
  ...
  bool row_is_active(const StackReadContext &ctx,
                     ...
                     const StackRow &row) const override
  {
    const Object *object = outliner_stack_focus_object_get(ctx, focus);
    return object != nullptr && object->shapenr - 1 == row.ordinal;
  }
                   ...
                   const int to_ordinal) const override
  {
    ...
    if (!BKE_keyblock_move(object, from_ordinal, to_ordinal)) {
      return false;
    }
    DEG_id_tag_update(&object->id, ID_RECALC_GEOMETRY);
    WM_event_add_notifier(&C, NC_OBJECT | ND_DATA, &object->id);
