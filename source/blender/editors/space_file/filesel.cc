...
{
  printf("[IMG_ASSET_DROP] ED_fileselect_clear: sfile=%p files=%p\n",
         static_cast<void *>(sfile),
         static_cast<void *>(sfile ? sfile->files : nullptr));
  fflush(stdout);
  filelist_debug_log_state(sfile ? sfile->files : nullptr, "ED_fileselect_clear_before");
  ...
  if (sfile->files) {
    ...
    filelist_debug_log_state(sfile->files, "ED_fileselect_clear_after");
  }
