...
enum class NodeDataFlag : uint8_t {
  ...
  HideVert = (1 << 4),
  HideFace = (1 << 5),
  /* Poly Paint: named `CD_PROP_FLOAT` / color point attributes selected by
   * #PaintModeSettings. The names covered by the step live on #StepData, see
   * #MaterialUndoAttributes. */
  Material = (1 << 6),
};
 ...
 * Position undo step for \a object, passing each touched node's vertex indices together with
 * their pre-stroke positions. Returns false when there is no suitable in-progress mesh Position
 * step for \a object (e.g. after #push_end, or for a multires step). Must be called before
 * #push_end.
 *
 * Takes \a object rather than V3's original object-less lookup: undo is per-object on this fork
 * (#StepData lives per #Object in #SculptUndoStep::objects_data), so an object-less lookup would,
 * in a multi-object stroke, fold every participating object's recorded nodes into the caller's
 * layer base and interpret `unode->vert_indices` against the wrong object's vertex array.
 */
 ...
 *
 * Takes \a object for the same reason as #foreach_recorded_position_mesh.
 */
 ...
 * Position undo step for \a object, per touched node. Returns false when there is no suitable
 * in-progress grids Position step for \a object. Must be called before #push_end.
 *
 * Takes \a object for the same reason as #foreach_recorded_position_mesh.
 */
...
/**
 * Poly Paint: rolls every attribute covered by the in-progress #NodeDataFlag::Material step
 * back to its
 ...
 * #NodeDataFlag::Color steps.
 *
 * \return false when the current step carries no #NodeDataFlag::Material data, so the caller can fall back
