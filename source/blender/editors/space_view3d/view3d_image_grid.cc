...
static void brush_mtex_slot_set_texture(bContext &C, MTex *mtex, Tex *tex, Brush *brush)
{
  /* After memfile undo the slot may hold a dangling pointer to a freed local #Tex: the asset
   * brush is linked and thus skipped by the no-undo ID remap, while the local texture it
   * referenced was freed by the undo step. Drop the stale pointer directly so #id_us_min below
   * does not read freed memory. */
  Main &bmain = *CTX_data_main(&C);
  if (mtex->tex != nullptr && BLI_findindex(which_libbase(&bmain, ID_TE), mtex->tex) == -1) {
    mtex->tex = nullptr;
  }
                                             ...
                                             const bool use_mask_slot)
{
  ...
  brush_mtex_slot_set_texture(C, mtex, tex, &brush);
...
static wmOperatorStatus image_grid_assign_texture_exec(bContext *C, wmOperator *op)
{
  ...
  if (!image_grid_assign_image_to_brush(*C, *brush, *image, use_mask_slot)) {
    return OPERATOR_CANCELLED;
  }
