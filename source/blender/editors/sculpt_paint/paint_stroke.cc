 ...
 * It's strange that only texpaint had these guards. */
  ...
  if (!paint_runtime->last_hit) {
    ...
    if (need_roll_mapping() && is_location_is_set) {
  ...
  if (need_roll_mapping()) {
    ...
    roll_->last_painted_roll_idx = look_back;
...
PaintStroke::PaintStroke(bContext *C, wmOperator *op, int event_type) : event_type_(event_type)
{
  ...
  /* 3D Roll record path is built around SculptSession::cache->sculpt_normal and an object-space
   * `float3` location. The 2D Image Paint stroke has neither: its dab data comes from the
   * `mouse` property of the stroke RNA, and Roll mapping there is computed in
   * `ImagePaintStroke::update_step` against a UV trajectory built locally. Leaving the 3D path
   * enabled in Texture2D mode would either rely on null #StrokeCache (so the poly_3d / normal /
   * 3D-point knot stay (0,0,0) and nonsense tangent comes out) or trigger painter paths that
   * `Mesh::Runtime::sculpt_session` does not exist on, neither of which is correct.
   * ImagePaint-mode Roll is wired separately and never creates the state here. */
  const PaintMode roll_paint_mode = BKE_paintmode_get_active_from_context(C);
  const bool roll_is_3d_only = roll_paint_mode != PaintMode::Texture2D;
  if (roll_is_3d_only && (ELEM(this->brush->stroke_method, BRUSH_STROKE_ROLL) ||
                          (this->brush->stroke_method == BRUSH_STROKE_CURVE &&
                           paint_stroke_uses_roll_texture_mapping(*this->brush))))
  {
    /* The only place this is created; its existence is what #need_roll_mapping reports from here
     * on, so no separate flag can drift away from the state it describes. */
    roll_ = std::make_unique<RollStrokeState>();
    init_roll_cursors();
  }
}
...
void PaintStroke::done(bContext *C, const bool is_cancel)
{
  ...
  this->post_done(C, is_cancel, stroke_started_);
  ...
  if (roll_) {
    if (roll_->roll_cursor) {
      WM_paint_cursor_end(static_cast<wmPaintCursor *>(roll_->roll_cursor));
    }
    if (roll_->debug_cursor) {
      WM_paint_cursor_end(static_cast<wmPaintCursor *>(roll_->debug_cursor));
    }
  }
}
...
bool PaintStroke::curve_end(bContext *C, wmOperator *op)
{
  ...
  /* Object-space paint curves cannot be evaluated in an Image Editor, which has no 3D projection
   * context. A new curve created there is converted to screen space by the add-point operator. */
  if (paintcurve_uses_3d_geometry(pc) && this->vc.rv3d == nullptr) {
    return true;
  }
  ...
  auto finish_curve_roll_stroke = [&]() {
    if (need_roll_mapping() && stroke_started_) {
  ...
  const bool do_roll_texture = need_roll_mapping();
