                                             ...
                                             const bool is_popover)
{
  ...
  const int tile_h = max_ii(1, ui::preview_tile_size_y_no_label(image_grid_preview_size_get(v3d)));
                              ...
                              const bool is_popover)
{
  ...
  const int total_rows = (cols > 0) ? int(divide_ceil_u(uint(state.viewport.cached_item_count),
                                                        uint(cols))) :
                                      effective_rows;
                                 ...
                                 const bool is_popover)
{
  state.viewport.scroll_row = clamp_i(
      ...
      0,
      image_grid_max_scroll_row(state, v3d, is_mask_slot, is_popover));
}
                           ...
                           bool *r_is_mask_slot)
{
  ...
  if (over) {
     ...
     * still, and only for the region the latch belongs to (#ImageGridWheelLatch). During a
     * fast-scroll rebuild the grid can momentarily disappear OR shrink to a shorter (still
     * non-empty) layout that no longer reaches the cursor; consuming purely on cursor stillness
     * keeps the wheel from leaking into the region's View2D pan (which collapses the N-panel to a
     * single visible row). A genuine cursor exit moves the cursor, which #held_still detects and
     * releases; events in other regions never touch the latch. */
    bool consume_via_latch = false;
    if (latch.region == region && latch.over) {
      const int tile_h = max_ii(
          1, ui::preview_tile_size_y_no_label(image_grid_preview_size_get(*v3d)));
...
int handle_image_grid_wheel_event(bContext *C, const wmEvent *event, ARegion *region)
{
  ...
  /* Popovers use a temporary region; CTX_wm_region() still points at the opener (e.g. tool
   * header). */
  ED_region_tag_redraw(region);
  ED_region_tag_refresh_ui(region);
...
int handle_image_grid_drag_scroll_event(bContext *C, const wmEvent *event, ARegion *region)
{
  ...
  /* One drag gesture per View3D; state lives in #View3D_Runtime (see
   * #image_grid_drag_scroll_state) rather than a process-global, so independent viewports never
   * share it. */
  ...
  if (event->type == LEFTMOUSE && event->val == KM_PRESS) {
     ...
     * manager's top-most hit test (#region_view_item_topmost_at) means a widget drawn over the
     * grid (the overlay scrollbar) or below it (the resize grip) keeps the press by z-order, so
     * moving the bar or resizing the grid is never mistaken for a drag-scroll. */
    bool is_mask = false;
  ...
  if (event->type == MOUSEMOVE && drag.active) {
    ...
    if (drag.dragging) {
      ...
      const int max_scroll_px = image_grid_max_scroll_row(
                                    state, *v3d, drag.is_mask_slot, is_popover) *
                                tile_h;
...
void image_grid_auto_focus_on_brush_change(bContext &C, View3D &v3d, const bool is_mask_slot)
{
  Paint *paint = BKE_paint_get_active_from_context(&C);
...
static wmOperatorStatus image_grid_scroll_exec(bContext *C, wmOperator *op)
{
  ...
  /* Popover keeps its own grip height/columns; clamp scrolling against the grid the wheel event
   * was over, not the sidebar. The caller resolves this from the region (CTX_wm_region_popup is
   * unset during pre-button handling) and passes it as a property. */
  const bool is_popover = RNA_boolean_get(op->ptr, "is_popover");
  ImageGridUIState &state = image_grid_state_get(*v3d, is_mask_slot);
  const int delta = RNA_int_get(op->ptr, "delta");
  image_grid_clamp_scroll_row(state, *v3d, is_mask_slot, is_popover);
  state.viewport.scroll_row = clamp_i(
      state.viewport.scroll_row + delta,
      0,
      image_grid_max_scroll_row(state, *v3d, is_mask_slot, is_popover));
...
void VIEW3D_OT_image_grid_scroll(wmOperatorType *ot)
{
  ...
  PropertyRNA *prop = RNA_def_boolean(ot->srna,
                                      ...
                                      false,
                                      "Is Popover",
                                      "Scroll the popover grid (uses its own height)");
  RNA_def_property_flag(prop, PROP_SKIP_SAVE | PROP_HIDDEN);
}
