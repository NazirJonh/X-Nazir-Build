...
"""Cache -> WindowManager synchronisation for the Tabs-System.

Split out of ``wm_sync.py`` (Candidate 6): this module owns the cache -> WM direction —
tag manipulation helpers that update ``wm.category_glyph_overrides``/``category_tags``,
the periodic/background sync machinery, and registration. The reverse direction
(``sync_wm_to_glyph_cache``) lives in ``wm_sync_from_wm.py`` and has no dependency on
this module.

Cross-module calls to ``_merge_discovered_categories`` (discovery_merge) and
``_auto_detect_extension_icon_path`` (discovery_scan) use lazy imports inside function
bodies to avoid circular dependencies (this is a one-directional dependency: this
module reaches into discovery, discovery never reaches back into wm_sync_to_wm).
...
# -----------------------------------------------------------------------------
# Sync + background sync functions (cache -> WM bidirectional trigger)
# -----------------------------------------------------------------------------
...
def _background_discovery_sync():
    ...
    First run after startup only refreshes icons (discovery already ran in load_post).
    Later runs perform full discovery merge to catch newly installed extensions.
    """
    run_count = increment_background_sync_run_count()

    # After load_post, run #1 only needs icon detection — not another full discovery scan.
    force_discovery = run_count > 1
    try:
        sync_glyph_mappings_to_wm(
            force_discovery_merge=force_discovery,
            skip_icon_detection=False,
        )
        category_debug_print(
            f"[BACKGROUND SYNC] Completed sync (run #{run_count}, "
            f"force_discovery={force_discovery})"
        )
    except Exception as e:
        category_debug_print(f"[BACKGROUND SYNC] Error: {e}")

    # Re-register timer for next run
    # First run: 30 seconds (defer full rediscovery until UI is idle)
    # Subsequent runs: 60 seconds
    set_background_sync_timer(_background_discovery_sync)
    next_interval = 30.0 if run_count == 1 else 60.0
    try:
...
def _sync_glyph_mappings_to_wm_impl(force_discovery_merge=False, skip_icon_detection=False):
    ...
    if force_discovery_merge:
        ...
        try:
            from bl_ui.glyph_tag_system.discovery_merge import _merge_discovered_categories
                ...
                try:
                    ...
                    if icon_source_str == "auto" and (not icon_key_val) and (not icon_path_val):
                        ...
                        if not skip_icon_detection:
                            ...
                            from bl_ui.glyph_tag_system.discovery_scan import _auto_detect_extension_icon_path
...
def register_category_glyph_mappings():
    ...
    # Start background sync after UI settles — icon detection only on first run.
    try:
        bpy.app.timers.register(start_background_sync_delayed, first_interval=10.0)
        category_debug_print("[BACKGROUND SYNC] Scheduled to start in 10 seconds")
    except Exception as e:
