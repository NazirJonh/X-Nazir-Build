   ...
   public:
    AllItem(PointerRNA settings) : BasicTreeViewItem(IFACE_("All")), settings_(settings)
    {
      ...
      set_is_active_fn([this]() -> bool {
        return grid_settings::enabled_catalogs_get(settings_).is_empty();
      });
    }
  };
   ...
   public:
    Item(const asset_system::AssetCatalogTreeItem &catalog_item, PointerRNA settings)
        : BasicTreeViewItem(catalog_item.get_name()),
          ...
          catalog_path_enabled_(grid_settings::is_catalog_path_enabled(
                                    settings, catalog_item.catalog_path().str()) ?
                                1 :
                                0)
    {
      disable_activatable();
