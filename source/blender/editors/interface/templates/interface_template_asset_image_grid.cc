  ...
  void build_context_menu(bContext &C, Layout &layout) const override
  {
    ...
    if (!linked && id) {
      ...
      PointerRNA catalog_props = layout.op(
          "VIEW3D_OT_image_grid_assign_catalog", IFACE_("Assign to Catalog"), ICON_NONE);
      set_grid_item_operator_props(C, catalog_props);
    }

    if (can_relocate_to_disk_library()) {
      PointerRNA copy_props = layout.op(
          "VIEW3D_OT_image_grid_copy_to_library", IFACE_("Copy to Library"), ICON_NONE);
      set_grid_item_operator_props(C, copy_props);

      PointerRNA move_props = layout.op(
          "VIEW3D_OT_image_grid_move_to_library", IFACE_("Move to Library"), ICON_NONE);
...
class ImageAssetGridView : public AbstractGridView {
 ...
 public:
  ...
  void build_items() override
  {
     ...
     * transient value to #cached_item_count collapses #image_grid_max_scroll_row, which clamps
     * #scroll_row to the top and momentarily shrinks the grid (the shrink in turn lets the wheel
...
class View3DGridStateAccess : public GridStateAccess {
 ...
 public:
  ...
  {
    return state_.viewport.scroll_row;
  }
  void scroll_row_set(const int value) override
  {
    state_.viewport.scroll_row = value;
  }
  ...
  {
    return state_.viewport.scroll_offset_px;
  }
  ...
  {
    return state_.viewport.cached_item_count;
  }
  ...
  {
    return cached_cols_ref_();
  }
  void cached_cols_set(const int value) override
  {
    cached_cols_ref_() = value;
  }
                             ...
                             const bool is_popover)
{
   ...
   * offset for *every* pass of the gesture. When the grid shrinks while the region is scrolled,
   * the shorter content would raise #View2D::tot.ymin and snap the view up, drifting the header.
   * The grip change callback (#make_grip_change_fn) fires only on the frames the value moves, but
   * idle redraws in between would still reset tot to the shrunk height and snap. So that callback
   * arms a short frame latch (#grip_resize_hold_frames); keep the offset while it is non-zero,
   * which spans the idle redraws around each move. The popover lives in its own temporary region —
   * unaffected. */
  if (!is_popover && state.viewport.grip_resize_hold_frames > 0) {
    if (ARegion *region = CTX_wm_region(&C)) {
       ...
       * #grip_resize_hold_frames; during the drag the callback keeps re-arming it, so the
       * countdown only runs to completion after the drag ends. */
      ED_region_tag_redraw(region);
      ED_region_tag_refresh_ui(region);
    }
  ...
  build_grid_view(C,
                  ...
                  *grid_view,
                  state_access,
                  state.viewport.cached_item_count,
                  cols_est,
                  panel_width);
}
