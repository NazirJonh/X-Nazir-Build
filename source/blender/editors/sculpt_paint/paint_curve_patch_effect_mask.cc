 ...
 * \ingroup edsculpt
 *
 * Sculpt mask target for a Curve Patch session. The target is kept separate from the relief effect:
 * a mask changes an attribute, not vertex positions, and therefore needs its own snapshot, restore
 * and undo handling.
 */
...
static float mask_from_sample(const float original, const float value, const bool increase)
{
   ...
   * toward one using the unmasked part; negative values move toward zero using the mask itself. The
   * direction comes from the brush strength, not from the sampled value, so a texture cannot change
  ...
  switch (pbvh.type()) {
    case bke::pbvh::Type::Mesh: {
      ...
      const CurvePatchSampler sampler(
          item, patch.texture, ctx, brush, source, Span<float>(), ss.tex_pool_ensure());
      ...
      for (const LocalData &local : all_tls) {
        for (const MaskWrite &write : local.writes) {
          ...
          masks.span[write.index] = mask_from_sample(write.original, blended, ctx.bstrength > 0.0f);
        }
      }
