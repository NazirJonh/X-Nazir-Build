...
struct Material;
struct Object;
/* #StackColorSession lives in #outliner_stack_source.hh; the fill-color helpers below only carry
 * it between that header's own methods. */
struct Scene;
struct ScrArea;
...
struct SpaceOutliner_Runtime {
  ...
  /** Copies focus for a duplicated space, not cached tree data. */
...
/** The identity of the row currently at \a ordinal, for #outliner_stack_identity_resolve. */
 ...
 *
 * When \a session is given, the source captures the row's pristine pixels into it before the
 * bake writes, so the caller can commit one undo entry or roll back. Null means a one-shot call
 * with no session around it.
 *
 ...
 * captured into \a session on first touch when given. The dialog's exec owns the commit (a
 * memfile step plus the source's own undo entry), its cancel the rollback to the captured state.
 *
