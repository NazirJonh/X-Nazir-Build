...
static void mesh_init_data(ID *id)
{
  ...
  mesh->totselect = 0;
  mesh->act_face = 0;
  mesh->texcomesh = nullptr;
  mesh->texspace_flag = 1; /* ME_TEXSPACE_FLAG_AUTO */
  mesh->vertex_group_active_index = 0;
  mesh->attributes_active_index = 0;
                           ...
                           const int flag)
{
  ...
  if (mesh_src->face_set_colors && mesh_src->face_set_colors_num > 0) {
    mesh_dst->face_set_colors = MEM_new_array_uninitialized<FaceSetColor>(
        mesh_src->face_set_colors_num, __func__);
    memcpy(mesh_dst->face_set_colors,
           mesh_src->face_set_colors,
           mesh_src->face_set_colors_num * sizeof(FaceSetColor));
