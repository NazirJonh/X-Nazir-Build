 ...
 * source of truth for scroll/grip since Stage 4). The View3D keeps only the DNA row fallback and
...
class View3DGridStateAccess : public GridStateAccess {
 ...
 public:
  View3DGridStateAccess(GridSessionState &session,
                        View3D &v3d,
                        std::string idname,
                                                        ...
                                                        const int /*store_rows*/) const override
  {
    ...
    return [is_mask_slot](bContext &C) {
      if (View3D *v3d = CTX_wm_view3d(&C)) {
        ed::view3d::image_grid_focus_clear(ed::view3d::image_grid_state_get(*v3d, is_mask_slot).viewport);
      }
                             ...
                             const bool is_popover)
{
  ...
  const std::string grid_id = ed::view3d::image_grid_session_id(*v3d, is_mask_slot, is_popover);
  ...
  const int focus_row = ed::view3d::image_grid_apply_focus_scroll(
      C, *v3d, state, cols_est, effective_rows_hint);
  if (focus_row >= 0) {
