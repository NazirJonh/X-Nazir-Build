...
def _ensure_sync_settings_source_version(context):
    ...
    if not settings.backup_directory:
        settings.backup_directory = _sync_backup_default_directory()
    versions = PREFERENCES_OT_copy_settings.find_versions_for_sync('STOCK')
...
_SYNC_BACKUP_PREFIX = "preferences_sync_"
_SYNC_BACKUP_MANIFEST = ".blender_sync_backup.json"
_SYNC_BACKUP_KEEP = 2


def _sync_backup_default_directory():
    import os

    user_root = bpy.utils.resource_path('USER')
    blender_root = os.path.dirname(os.path.normpath(user_root))
    return os.path.join(blender_root, "sync_backups")


def _sync_path_is_inside(path, directory):
    import os
...
def _create_sync_backup(destination, backup_directory, source_version):
    """Create a complete ZIP snapshot of the destination profile before changing it."""
    import json
    import os
    import datetime
    import time
    ...
    if not backup_directory:
        raise OSError("Backup directory is empty")

    backup_directory = os.path.abspath(os.path.expanduser(backup_directory))
    destination = os.path.abspath(destination)
    try:
        if os.path.commonpath((destination, backup_directory)) == destination:
            raise OSError("Backup directory must not be inside the Blender configuration directory")
    except ValueError:
        pass
    os.makedirs(backup_directory, exist_ok=True)
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    archive_name = "{:s}{:s}.zip".format(_SYNC_BACKUP_PREFIX, timestamp)
    ...
    try:
        with zipfile.ZipFile(temp_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            archive.writestr(_SYNC_BACKUP_MANIFEST, json.dumps(manifest, indent=2))
            for root, directories, filenames in os.walk(destination, topdown=True, followlinks=False):
                directories[:] = [
                    name for name in directories
                    if not _sync_path_is_inside(os.path.join(root, name), backup_directory)
                    and not os.path.islink(os.path.join(root, name))
                ]
                for filename in filenames:
                    filepath = os.path.join(root, filename)
                    if os.path.islink(filepath) or _sync_path_is_inside(filepath, backup_directory):
                        continue
                    archive.write(filepath, os.path.relpath(filepath, destination))
    ...
    for filename in os.listdir(backup_directory):
        if not filename.startswith(_SYNC_BACKUP_PREFIX) or not filename.endswith(".zip"):
            continue
    ...
    for old_archive in archives[_SYNC_BACKUP_KEEP:]:
        try:
...
def _restore_sync_backup(archive_path, destination):
    """Restore a validated synchronization archive into the destination profile."""
    import json
    import os
    import shutil
    import tempfile
    ...
    with zipfile.ZipFile(archive_path, "r") as archive:
        try:
            manifest = json.loads(archive.read(_SYNC_BACKUP_MANIFEST).decode("utf-8"))
        except (KeyError, UnicodeDecodeError, json.JSONDecodeError) as ex:
            raise OSError("Invalid Blender synchronization backup manifest") from ex
        if manifest.get("format") != 1:
            raise OSError("Unsupported Blender synchronization backup format")

        for entry in archive.infolist():
            name = entry.filename.replace("\\", "/")
            if name == _SYNC_BACKUP_MANIFEST:
                continue
            if not name or name.startswith("/") or os.path.isabs(name):
                raise OSError("Backup contains an absolute path")
            normalized = os.path.normpath(name)
            if normalized == ".." or normalized.startswith(".." + os.sep):
                raise OSError("Backup contains an unsafe path")

        parent = os.path.dirname(destination)
        temporary = tempfile.mkdtemp(prefix="blender_sync_restore_", dir=parent)
        extracted_profile = os.path.join(temporary, "profile")
        old_destination = destination + ".restore_old"
        try:
            os.makedirs(extracted_profile, exist_ok=True)
            archive.extractall(extracted_profile)
            os.remove(os.path.join(extracted_profile, _SYNC_BACKUP_MANIFEST))

            if os.path.exists(old_destination):
                raise OSError("A previous restore attempt is still present")
            if os.path.exists(destination):
                os.replace(destination, old_destination)
            try:
                os.replace(extracted_profile, destination)
            except OSError:
                if os.path.exists(old_destination) and not os.path.exists(destination):
...
class PreferencesSyncSettings(PropertyGroup):
    ...
    use_backup: BoolProperty(
        name="Create Backup",
        description="Create a ZIP snapshot before synchronization",
        default=True,
    )
    backup_directory: StringProperty(
        name="Backup Directory",
        description="Directory where the two latest synchronization backups are stored",
        subtype='DIR_PATH',
        default="",
    )
...
class PREFERENCES_OT_sync_settings_read(Operator):
    ...
    def execute(self, context):
        ...
        def add_changes(category, source_dir, dest_dir, *, nested=False, exclude_theme_extensions=False):
            ...
            # Existing add-ons/extensions are intentionally left untouched. Their files may contain
            # local changes, and this operator only installs modules that are not present yet.
            # Theme presets are handled separately below and may still expose UPDATE entries.
            if not nested:
                for name in sorted(source_names & dest_names):
                    if not name.startswith('.'):
                        item = settings.changes.add()
...
class PREFERENCES_OT_sync_settings_backup_directory(Operator):
    """Choose the directory used for synchronization backups."""
    bl_idname = "preferences.sync_settings_backup_directory"
    bl_label = "Select Backup Directory"

    directory: StringProperty(subtype='DIR_PATH')

    def invoke(self, context, _event):
        self.directory = context.window_manager.sync_settings.backup_directory
    ...
    def execute(self, context):
        ...
        directory = os.path.abspath(os.path.expanduser(self.directory))
        if not os.path.isdir(directory):
            self.report({'ERROR'}, "Backup directory does not exist")
            return {'CANCELLED'}
        context.window_manager.sync_settings.backup_directory = directory
        return {'FINISHED'}


class PREFERENCES_OT_sync_settings_restore(Operator):
    """Restore the current configuration from a synchronization ZIP backup."""
    bl_idname = "preferences.sync_settings_restore"
    bl_label = "Restore Synchronization Backup"
    ...
    def execute(self, context):
        ...
        destination = bpy.utils.resource_path('USER')
        if not os.path.isfile(self.filepath):
            self.report({'ERROR'}, "Backup archive not found")
            return {'CANCELLED'}
        try:
            _restore_sync_backup(self.filepath, destination)
        except (OSError, ValueError, zipfile.BadZipFile) as ex:
            self.report({'ERROR'}, "Unable to restore synchronization backup: {:s}".format(str(ex)))
            return {'CANCELLED'}
        result = bpy.ops.wm.read_userpref()
        if result != {'FINISHED'}:
            self.report({'WARNING'}, "Backup files restored, but Blender preferences were not reloaded")
            return {'FINISHED'}
        self.report({'INFO'}, "Synchronization backup restored")
        return {'FINISHED'}
...
class PREFERENCES_OT_sync_settings(Operator):
    ...
    def execute(self, context):
        ...
        if settings.use_backup:
            backup_directory = settings.backup_directory or _sync_backup_default_directory()
            try:
                backup_path = _create_sync_backup(dest, backup_directory, self.source_version)
            except OSError as ex:
                self.report({'ERROR'}, "Unable to create synchronization backup: {:s}".format(str(ex)))
                return {'CANCELLED'}
        ...
        result = bpy.ops.preferences.userdef_merge(
        ...
        if result != {'FINISHED'}:
            message = "Preferences merge failed"
            if backup_path:
                message += ". Backup: {:s}".format(backup_path)
            self.report({'ERROR'}, message)
            return {'CANCELLED'}

        # Keep missing add-on entries untouched. They may have existed before this operation, and
        # deleting them would make a merge destructive outside the selected source changes.
        disabled = []
        ...
        if backup_path:
            message += ". Backup created at {:s}".format(backup_path)
        if disabled:
    ...
    def _copy_missing_modules(
        ...
        """Copy only missing extension and legacy add-on directories.
...
classes = (
    ...
    PREFERENCES_OT_sync_settings_backup_directory,
    PREFERENCES_OT_sync_settings_restore,
