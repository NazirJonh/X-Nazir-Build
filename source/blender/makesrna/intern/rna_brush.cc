...
static bool rna_Brush_use_write_color_attribute_get(PointerRNA *ptr)
{
  Brush *br = static_cast<Brush *>(ptr->data);
  return br->write_vcol != 0;
}

static void rna_Brush_use_write_color_attribute_set(PointerRNA *ptr, bool value)
{
  Brush *br = static_cast<Brush *>(ptr->data);
  const bool was_enabled = br->write_vcol != 0;
  br->write_vcol = value;
  if (value && !was_enabled) {
    br->vcol_channel = BRUSH_VCOL_CHANNEL_RGB;
  }
}
...
static void rna_Brush_face_set_color_tex_update(bContext *C, PointerRNA *ptr)
{
  Main *bmain = CTX_data_main(C);
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  Brush *br = static_cast<Brush *>(ptr->data);
  BKE_paint_invalidate_overlay_tex(*bmain, scene, view_layer, br->face_set_color_mtex.tex);
  rna_Brush_update(bmain, scene, ptr);
}
                                                      ...
                                                      bool *r_free)
{
  ...
  /* Image Paint now exposes Roll and Curve Patch alongside the historical Texture Paint
   * items. Curve Patch is further restricted at runtime to Image Editor + DRAW brush via
   * `image_curve_patch_session_begin()` in `paint_image_curve_patch.cc`, which is the
   * authoritative "this brush can run a session" gate; the RNA here only decides which
   * enum values show up in the picker so unsupported combinations don't toggle into a
   * no-op stroke method. */
  static const EnumPropertyItem texture_paint_stroke_method_items[] = {
      ...
      {BRUSH_STROKE_ROLL,
       ...
       0,
       "Roll",
       "Roll texture along the stroke path. Available in Image Paint since spec rollout."},
      {BRUSH_STROKE_CURVE_PATCH,
       "CURVE_PATCH",
       0,
       "Curve Patch",
       "Draw a Curve Patch in the Image Editor. Only available for Draw brushes "
       "(see `image_curve_patch_session_begin()`)."},
      {0, nullptr, 0, nullptr, nullptr},
  };
  ...
  switch (mode) {
    ...
    case PaintMode::Texture2D: {
      /* Texture 2D shows Roll and Curve Patch in the picker. Curve Patch is gated to
       * baseline Draw brushes because the planned session determinism (`Paint Curve in
       * Sculpt Mode` §7.3) only snapshots image paint state for that brush type -- Clone,
       * Smear, Fill, Soften need additional capture paths that are out of scope for the
       * first rollout. */
      const Brush *brush = static_cast<const Brush *>(ptr->data);
      ...
      EnumPropertyItem *item = nullptr;
      int totitem = 0;
      for (const EnumPropertyItem *it = texture_paint_stroke_method_items; it->identifier; it++) {
        if (it->value == BRUSH_STROKE_CURVE_PATCH) {
          continue;
        }
        RNA_enum_item_add(&item, &totitem, it);
    ...
    case PaintMode::Texture3D:
      /* Texture3D keeps the historical list only. Roll/Curve Patch remain 2D-only since
       * 3D paint runs through the 3D Roll path (#PaintStroke::need_roll_mapping) and
       * follows `texture_paint_stroke_method_items`. */
      return texture_paint_stroke_method_items;
                                                                   ...
                                                                   bool * /*r_free*/)
{
  ...
  /* The Image Editor rasterizes Area Plane through the surface->UV path, so the mode has to be
   * selectable there. Without it the enum cannot render its own default (PBR shared mapping
   * starts in Area Plane) and the Mapping field draws blank. Projection painting in the 3D
   * viewport has no such path, so it keeps the list above. */
  static const EnumPropertyItem rna_enum_brush_texture_slot_map_image_mode_items[] = {
      {MTEX_MAP_MODE_VIEW, "VIEW_PLANE", 0, "View Plane", ""},
      {MTEX_MAP_MODE_AREA, "AREA_PLANE", 0, "Area Plane", ""},
      {MTEX_MAP_MODE_TILED, "TILED", 0, "Tiled", ""},
      {MTEX_MAP_MODE_3D, "3D", 0, "3D", ""},
...
#  define rna_enum_brush_texture_slot_map_sculpt_mode_items \
  ...
  if (mode == PaintMode::Texture2D) {
    return rna_enum_brush_texture_slot_map_image_mode_items;
  }
 ...
 * is the settings block, and a cast of it to `Brush` would reinterpret unrelated bytes. Mirrors
 * #rna_BrushCurvePatchTextureSlot_update. */
...
static void rna_BrushCurvePatchSettings_texture_active_index_range(
    PointerRNA *ptr, int *min, int *max, int * /*softmin*/, int * /*softmax*/)
{
  const Brush *br = reinterpret_cast<Brush *>(ptr->owner_id);
  *min = 0;
  *max = max_ii(0, br->curve_patch.texture_slots.count() - 1);
}
...
static const EnumPropertyItem brush_texture_data_mode_items[] = {
    ...
    {BRUSH_TEXTURE_DATA_MODE_FACE_SETS_COLOR_FROM_TEXTURE,
     "FACE_SETS_COLOR_FROM_TEXTURE",
     0,
     "Face Sets from Color Texture",
     "Sample RGB texture per face to assign Face Set IDs; alpha texture remains the stroke mask"},
    {0, nullptr, 0, nullptr, nullptr},
};

static const EnumPropertyItem brush_write_color_channel_items[] = {
...
static const EnumPropertyItem brush_write_color_mode_items[] = {
    {BRUSH_VCOL_MODE_GRAYSCALE,
     "GRAYSCALE",
     0,
     "Continuous",
     "Blend with mask strength; edges may darken toward the background"},
    {BRUSH_VCOL_MODE_BINARY,
     "BINARY",
     0,
     "Binary",
     "Write full color above threshold; write black below threshold"},
    {BRUSH_VCOL_MODE_MASK,
     "MASK",
     0,
     "Masked",
     "Write full color above threshold; leave existing vertex color below threshold"},
    {BRUSH_VCOL_MODE_DUAL,
     "DUAL",
     0,
     "Dual",
     "Primary Face Set color above threshold; secondary Face Set color below threshold"},
    {0, nullptr, 0, nullptr, nullptr},
};
...
static void rna_def_brush_curve_patch_settings(BlenderRNA *brna)
{
  ...
  srna = RNA_def_struct(brna, "BrushCurvePatchSettings", nullptr);
  RNA_def_struct_sdna(srna, "BrushCurvePatchSettings");
  RNA_def_struct_path_func(srna, "rna_BrushCurvePatchSettings_path");
  RNA_def_struct_ui_text(srna, "Curve Patch Settings", "Curve Patch settings of a brush");

  prop = RNA_def_property(srna, "stamp_mode", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "stamp_mode");
  ...
  RNA_def_property_int_sdna(prop, nullptr, "stamp_size_random");
  ...
  prop = RNA_def_property(srna, "stamp_strength_random", PROP_INT, PROP_PERCENTAGE);
  ...
  RNA_def_property_enum_sdna(prop, nullptr, "stamp_projection");
  ...
  prop = RNA_def_property(srna, "ribbon_texture_source", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "ribbon_texture_source");
  ...
  RNA_def_property_enum_sdna(prop, nullptr, "length_mode");
  ...
  prop = RNA_def_property(srna, "length_repeat", PROP_INT, PROP_NONE);
  RNA_def_property_int_sdna(prop, nullptr, "length_repeat");
  ...
  prop = RNA_def_property(srna, "end_falloff", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "end_falloff");
   ...
   * percentage; only the RNA identifier used to call it a length. */
  prop = RNA_def_property(srna, "end_falloff_percent", PROP_INT, PROP_PERCENTAGE);
  RNA_def_property_int_sdna(prop, nullptr, "end_falloff_percent");
  ...
  prop = RNA_def_property(srna, "texture_active_index", PROP_INT, PROP_NONE);
  RNA_def_property_int_sdna(prop, nullptr, "texture_active_index");
  RNA_def_property_int_funcs(
      prop, nullptr, nullptr, "rna_BrushCurvePatchSettings_texture_active_index_range");
  RNA_def_property_ui_text(prop, "Active Curve Patch Texture", "Index of the active texture slot");
  RNA_def_property_update(prop, 0, "rna_BrushCurvePatchSettings_update");

  prop = RNA_def_property(srna, "use_face_set", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "face_set", 1);
  RNA_def_property_ui_text(prop,
                           "Create Face Set",
  ...
  prop = RNA_def_property(srna, "texture_middle", PROP_POINTER, PROP_NONE);
  RNA_def_property_pointer_sdna(prop, nullptr, "tex_middle");
  RNA_def_property_struct_type(prop, "Texture");
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
  ...
  prop = RNA_def_property(srna, "texture_end", PROP_POINTER, PROP_NONE);
  RNA_def_property_pointer_sdna(prop, nullptr, "tex_end");
  RNA_def_property_struct_type(prop, "Texture");
  RNA_def_property_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(
  ...
  prop = RNA_def_property(srna, "cap_start_length", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "cap_start_length");
  RNA_def_property_range(prop, 0.0f, 100.0f);
  RNA_def_property_ui_range(prop, 0.0f, 4.0f, 0.1f, 2);
  RNA_def_property_ui_text(
      prop, "Start Length", "Arc length the start texture occupies, in brush diameters");
  RNA_def_property_update(prop, 0, "rna_BrushCurvePatchSettings_update");

  prop = RNA_def_property(srna, "cap_end_length", PROP_FLOAT, PROP_NONE);
  RNA_def_property_float_sdna(prop, nullptr, "cap_end_length");
  RNA_def_property_range(prop, 0.0f, 100.0f);
  RNA_def_property_ui_range(prop, 0.0f, 4.0f, 0.1f, 2);
  RNA_def_property_ui_text(
...
static void rna_def_brush(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "use_invert_texture_alpha", PROP_BOOLEAN, PROP_NONE);
  ...
  prop = RNA_def_property(srna, "use_write_face_sets", PROP_BOOLEAN, PROP_NONE);
  ...
  prop = RNA_def_property(srna, "use_write_color_attribute", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_funcs(
      prop, "rna_Brush_use_write_color_attribute_get", "rna_Brush_use_write_color_attribute_set");
  RNA_def_property_ui_text(
      prop,
      "Write to Color Attribute",
      "Simultaneously write sampled texture values to the active color attribute");
  RNA_def_property_update(prop, 0, "rna_Brush_update");

  prop = RNA_def_property(srna, "write_color_channel", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "vcol_channel");
  RNA_def_property_enum_items(prop, brush_write_color_channel_items);
  RNA_def_property_enum_default(prop, BRUSH_VCOL_CHANNEL_RGB);
  RNA_def_property_ui_text(
      prop, "Color Channel", "Which channel of the color attribute to write to");
  RNA_def_property_update(prop, 0, "rna_Brush_update");

  prop = RNA_def_property(srna, "write_color_mode", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_sdna(prop, nullptr, "vcol_mode");
  RNA_def_property_enum_items(prop, brush_write_color_mode_items);
  RNA_def_property_enum_default(prop, BRUSH_VCOL_MODE_MASK);
  RNA_def_property_ui_text(
      prop, "Color Mode", "How sampled texture values are written to the color attribute");
  RNA_def_property_update(prop, 0, "rna_Brush_update");
  ...
  prop = RNA_def_property(srna, "use_insert_mesh", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag2", BRUSH_INSERT_MESH);
  RNA_def_property_ui_text(
      prop,
      "Insert Mesh",
      "Build a watertight mesh from the brush stamp (vector displacement or alpha texture) "
      "instead of deforming the active mesh");
  RNA_def_property_update(prop, 0, "rna_Brush_update");

  prop = RNA_def_property(srna, "use_insert_into_active", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag2", BRUSH_INSERT_INTO_ACTIVE);
  RNA_def_property_ui_text(prop,
                           "Into Active Mesh",
  ...
  prop = RNA_def_property(srna, "curve_patch", PROP_POINTER, PROP_NONE);
  RNA_def_property_struct_type(prop, "BrushCurvePatchSettings");
  RNA_def_property_pointer_sdna(prop, nullptr, "curve_patch");
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  ...
  prop = RNA_def_property(srna, "face_set_color_texture_slot", PROP_POINTER, PROP_NONE);
  RNA_def_property_struct_type(prop, "BrushTextureSlot");
  RNA_def_property_pointer_sdna(prop, nullptr, "face_set_color_mtex");
  RNA_def_property_clear_flag(prop, PROP_EDITABLE);
  RNA_def_property_ui_text(prop, "Face Set Color Texture Slot", "");

  prop = RNA_def_property(srna, "face_set_color_texture", PROP_POINTER, PROP_NONE);
  RNA_def_property_pointer_sdna(prop, nullptr, "face_set_color_mtex.tex");
  RNA_def_property_flag(prop, PROP_EDITABLE | PROP_CONTEXT_UPDATE);
  ...
  /* Not #rna_Brush_color_update: the Face Set color is unrelated to #Brush.rgb, so it must not
   * trigger the legacy paint color sync. */
  RNA_def_property_update(prop, 0, "rna_Brush_update");
