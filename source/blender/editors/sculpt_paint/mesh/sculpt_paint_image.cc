...
static void debug_log_simple_image_brush_step(const SimpleImageBrushStepSettings &settings,
...
static PixelRowLinearPosition calc_pixel_row_linear_position(const Span<float3> vert_positions,
                                                             const Span<int3> vert_tris,
                                                             const Span<UVPrimitivePaintInput>
                                                                 uv_primitives,
                                                             const PackedPixelRow &pixel_row)
{
  const UVPrimitivePaintInput &primitive = uv_primitives[pixel_row.uv_primitive_index];
  const float3 start = calc_pixel_position(
      vert_positions, vert_tris, primitive.tri_index, pixel_row.start_barycentric_coord);
  const float3 next = calc_pixel_position(vert_positions,
                                          vert_tris,
                                          primitive.tri_index,
                                          pixel_row.start_barycentric_coord +
                                              primitive.delta_barycentric_coord_u);
                                     ...
                                     const MutableSpan<float3> positions)
{
  const PixelRowLinearPosition row_linear =
      calc_pixel_row_linear_position(vert_positions, vert_tris, uv_primitives, pixel_row);
...
static void calc_brush_strength_distances_no_texture_row_linear(const SculptSession &ss,
                                                                const StrokeCache &cache,
                                                                const Brush &brush,
                                                                const eBrushFalloffShape
                                                                    falloff_shape,
                                                                const float bstrength,
                                                                const PixelRowLinearPosition
                                                                    &row_linear,
                                                                MutableSpan<float> distances,
                                                                MutableSpan<float> factors)
{
 ...
 public:
                             ...
                             const int last_positive)
  {
    ...
    for (int x = first_positive; x <= last_positive; x++) {
      ...
      const __m128 buffer_alpha =
          _mm_shuffle_ps(buffer_color, buffer_color, _MM_SHUFFLE(3, 3, 3, 3));
...
{
  const int3 tri = vert_tris[paint_input.tri_index];
  float3 closest;
                            ...
                            SimpleImageBrushPaintTelemetry *r_paint_telemetry)
{
  ...
  BitVector<> brush_test = init_uv_primitives_brush_test(
      ss, pbvh_data.vert_tris, node_data.uv_primitives, positions, brush_test_has_true);
  ...
  for (UDIMTilePixels &tile_data : node_data.tiles) {
    ...
    for (const PackedPixelRow &pixel_row : tile_data.pixel_rows) {
      ...
      const double sphere_cull_start =
          (r_paint_telemetry != nullptr && use_spherical_row_cull) ? BLI_time_now_seconds() : 0.0;
      const bool rejected_by_spherical_cull =
          ...
          ([&]() {
            ...
            if (primitive_state == -1) {
              primitive_state = primitive_might_intersect_spherical_brush(positions,
                                                                          pbvh_data.vert_tris,
            ...
            if (!row_linear_is_valid) {
              row_linear = calc_pixel_row_linear_position(
                  positions, pbvh_data.vert_tris, node_data.uv_primitives, pixel_row);
      ...
      const double row_pre_kernel_start =
      ...
      if (use_brush_texture) {
        const double row_positions_start =
            (r_paint_telemetry != nullptr) ? BLI_time_now_seconds() : 0.0;
        ...
        const double row_texture_start =
            (r_paint_telemetry != nullptr) ? BLI_time_now_seconds() : 0.0;
      ...
      if (!PaintingKernel<ImageBufferFloat4>::find_positive_factor_bounds(
        ...
        if (r_paint_telemetry != nullptr) {
          telemetry_counter_inc(use_brush_texture ? r_paint_telemetry->rows_rejected_by_zero_texture :
                                                   r_paint_telemetry
                                                       ->rows_rejected_by_zero_strength);
      ...
      if (image_buffer == nullptr) {
        const double ibuf_acquire_start =
            (r_paint_telemetry != nullptr) ? BLI_time_now_seconds() : 0.0;
        image_buffer = BKE_image_acquire_ibuf(image_data.image, &image_user, nullptr);
        if (r_paint_telemetry != nullptr) {
          telemetry_counter_add(
              r_paint_telemetry->ibuf_acquire_release_time_us,
    ...
    if (image_buffer != nullptr) {
      const double ibuf_release_start =
          (r_paint_telemetry != nullptr) ? BLI_time_now_seconds() : 0.0;
        ...
        ([&]() {
          ...
          if (primitive_state == -1) {
            primitive_state = primitive_might_intersect_spherical_brush(
                                  positions,
                                  pbvh_data.vert_tris,
                                  node_data.uv_primitives[uv_primitive_index],
                                  cache.location_symm,
                                  cache.radius_squared) ?
                                int8_t(1) :
                                int8_t(0);
...
static void do_paint_pixels_gradient(
    ...
    const int8_t radial_symmetry[3])
{
  ...
  const bool radial_symmetry_enabled = (radial_symmetry[0] != 1 || 
                                        radial_symmetry[1] != 1 || 
                                        radial_symmetry[2] != 1);
  const bool use_batch_projection = (symmetry == 0 && !radial_symmetry_enabled);
  std::fprintf(stderr, "[gradient_paint] symmetry=%d radial_symmetry=[%d,%d,%d] use_batch=%d\n",
               symmetry, radial_symmetry[0], radial_symmetry[1], radial_symmetry[2], use_batch_projection);
  ...
  std::fprintf(stderr,
               "[gradient_paint] tile_mode use_tile_parallel=%d tiles=%lld large_tiles=%lld total_rows=%lld\n",
               use_tile_parallel,
  ...
  auto process_tile = [&](UDIMTilePixels &tile_data, const bool allow_row_parallel) {
    ...
    const float gradient_axis_inv_len_sq =
        (gradient_axis_len_sq > 1.0e-12f) ? (1.0f / gradient_axis_len_sq) : 0.0f;
    ...
    if (allow_row_parallel && pixel_rows.size() >= row_threading_min_rows) {
      ...
      threading::parallel_for(pixel_rows.index_range(), row_threading_grain_size, [&](const IndexRange range) {
        ...
        for (const int row_index : range) {
          ...
          const double positions_start = kEnableGradientPaintDebugTelemetry ? BLI_time_now_seconds() : 0.0;
          pixel_positions.resize(pixel_row.num_pixels);
          calc_pixel_row_positions(
              positions, pbvh_data.vert_tris, node_data.uv_primitives, pixel_row, pixel_positions);
          if (kEnableGradientPaintDebugTelemetry) {
          ...
          const double gradient_calc_start = kEnableGradientPaintDebugTelemetry ? BLI_time_now_seconds() : 0.0;
          ...
          if (use_batch_projection) {
            ...
            ED_view3d_project_float_object_array(
                &region,
          ...
          const double kernel_start = kEnableGradientPaintDebugTelemetry ? BLI_time_now_seconds() : 0.0;
          bool pixels_painted = false;
          if (is_float_image) {
          ...
          if (kEnableGradientPaintDebugTelemetry) {
            local_kernel_time_us_atomic.fetch_add(
                int64_t((BLI_time_now_seconds() - kernel_start) * 1.0e6), std::memory_order_relaxed);
          }
      ...
      local_rows_rejected_no_influence = local_rows_rejected_no_influence_atomic.load(
          std::memory_order_relaxed);
      local_rows_painted_attempted = local_rows_painted_attempted_atomic.load(std::memory_order_relaxed);
      local_rows_painted_success = local_rows_painted_success_atomic.load(std::memory_order_relaxed);
      local_pre_positions_time_us = local_pre_positions_time_us_atomic.load(std::memory_order_relaxed);
      local_gradient_calc_time_us = local_gradient_calc_time_us_atomic.load(std::memory_order_relaxed);
      ...
      for (const PackedPixelRow &pixel_row : pixel_rows) {
        ...
        calc_pixel_row_positions(
            positions, pbvh_data.vert_tris, node_data.uv_primitives, pixel_row, pixel_positions);
        if (kEnableGradientPaintDebugTelemetry) {
          local_pre_positions_time_us += int64_t((BLI_time_now_seconds() - positions_start) * 1.0e6);
        }
    ...
    if (kEnableGradientPaintDebugTelemetry) {
      ...
      telemetry_counter_add(
          g_gradient_telemetry.rows_rejected_no_influence, local_rows_rejected_no_influence);
      telemetry_counter_add(g_gradient_telemetry.rows_painted_attempted, local_rows_painted_attempted);
      telemetry_counter_add(g_gradient_telemetry.rows_painted_success, local_rows_painted_success);
      telemetry_counter_add(g_gradient_telemetry.pre_positions_time_us, local_pre_positions_time_us);
      telemetry_counter_add(g_gradient_telemetry.gradient_calc_time_us, local_gradient_calc_time_us);
  ...
  if (kEnableGradientPaintDebugTelemetry) {
    ...
    std::fprintf(stderr,
                 ...
                 static_cast<long long>(g_gradient_telemetry.rows_rejected_no_influence.load(std::memory_order_relaxed)),
                 static_cast<long long>(g_gradient_telemetry.rows_painted_attempted.load(std::memory_order_relaxed)),
                 static_cast<long long>(g_gradient_telemetry.rows_painted_success.load(std::memory_order_relaxed)),
                 double(g_gradient_telemetry.pre_positions_time_us.load(std::memory_order_relaxed)) / 1000.0,
                 double(g_gradient_telemetry.gradient_calc_time_us.load(std::memory_order_relaxed)) / 1000.0,
                 double(g_gradient_telemetry.kernel_time_us.load(std::memory_order_relaxed)) / 1000.0,
                 double(g_gradient_telemetry.ibuf_acquire_release_time_us.load(std::memory_order_relaxed)) / 1000.0);
    std::fflush(stderr);
                                 ...
                                 const IndexMask &node_mask)
{
  ...
  SimpleImageBrushStepSettings step_settings =
      make_simple_image_brush_step_settings_begin(sd, *brush, ss, canvas_longest_side_px);
  double stage_time = BLI_time_now_seconds();
  if (step_settings.push_undo_tiles) {
    do_push_undo_tiles_batched(
        *image_data.image, *image_data.image_user, ss, nodes, node_mask);
  }
  ...
  if (use_extended_tile_batch) {
    do_paint_pixels_batched_by_tile(
        depsgraph, ob, sd.paint, *brush, image_data, nodes, node_mask, &paint_telemetry);
  }
  else {
    node_mask.foreach_index(
        [&](const int i) {
          do_paint_pixels(depsgraph, ob, sd.paint, *brush, image_data, nodes[i], &paint_telemetry);
        },
  ...
  if (step_settings.apply_seam_fix) {
    fix_non_manifold_seam_bleeding(ob, *image_data.image, *image_data.image_user, nodes, dirty_node_mask);
  }
                                          ...
                                          const bool clip_before_start)
{
  ...
  const IndexMask pixels_node_mask = IndexMask::from_predicate(
      node_mask, pixels_node_mask_memory, [&](const int64_t i) { return nodes[i].pixels_ != nullptr; });
