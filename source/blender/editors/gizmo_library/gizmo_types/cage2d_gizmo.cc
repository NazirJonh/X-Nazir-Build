...
static void cage2d_draw_rect_all_edge_handles(const rctf *r,
                                              ...
                                              const bool solid)
{
  cage2d_draw_rect_edge_handles(r, ED_GIZMO_CAGE2D_PART_SCALE_MIN_X, size, margin, color, solid);
  cage2d_draw_rect_edge_handles(r, ED_GIZMO_CAGE2D_PART_SCALE_MAX_X, size, margin, color, solid);
  cage2d_draw_rect_edge_handles(r, ED_GIZMO_CAGE2D_PART_SCALE_MIN_Y, size, margin, color, solid);
  cage2d_draw_rect_edge_handles(r, ED_GIZMO_CAGE2D_PART_SCALE_MAX_Y, size, margin, color, solid);
}
...
static int gizmo_cage2d_get_cursor(wmGizmo *gz)
{
  ...
  switch (highlight_part) {
    case ED_GIZMO_CAGE2D_PART_TRANSLATE:
      /* Interior move uses the full rectangle as a hot-spot; show the move cursor only while
       * dragging. */
