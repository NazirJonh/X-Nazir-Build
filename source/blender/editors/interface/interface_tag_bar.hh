...
/**
 * Free all per-window-manager tag bar runtime data and clear the cache.
 * Called from #ED_editors_exit (on quit and before a file load), giving the process-global cache a
 * deterministic lifetime: this prevents the shutdown leak and the dangling/ABA case where a freed
 * window manager's pointer is reused by a newly allocated one after a file load.
 */
