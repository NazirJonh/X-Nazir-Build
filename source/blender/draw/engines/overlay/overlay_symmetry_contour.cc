                              ...
                              float3 &r_end)
{
   ...
   * value with two extra zero-weighted terms. This runs once per triangle of every straddling
   * node, so it is the hottest arithmetic in the whole extraction. */
                ...
                node_indices.index_range(), grain_size, [&](const IndexRange range) {
                  ...
                  for (const int n : node_indices.as_span().slice(range)) {
                    ...
                    if (node_needs_recompute(n) || extra_recompute) {
                       ...
                       * otherwise a second-long freeze; #filling_ raised the budget so that fill
                       * still completes in a handful of interactive frames rather than dozens. */
