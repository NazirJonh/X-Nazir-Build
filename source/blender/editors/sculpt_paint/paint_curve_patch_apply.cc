                       ...
                       ReportList *reports)
{
  ...
  /* `MaskEffect` writes `.sculpt_mask` on a mesh -- which it can create on demand -- but on
   * Multires it writes `SubdivCCG::masks`, which is an EMPTY span until `CD_GRID_PAINT_MASK`
   * exists and cannot be grown from there. Ensured here rather than in the operator wrapper so
   * every caller of this function is covered: the wrapper is only one of them, and
   * `curve_patch_apply()` is the public headless entry a script reaches directly. Ahead of
   * `curve_patch_session_publish()` because that is where `MaskEffect::element_num()` reads the
   * span's size, and a zero recorded there would defeat the mid-session topology check.
   *
   * `bmain` comes off the depsgraph: taking it as a parameter would change a public signature for
   * one branch. */
  if (effect_type == CurvePatchEffectType::Mask) {
    Depsgraph &depsgraph_mutable = const_cast<Depsgraph &>(depsgraph);
    BKE_sculpt_mask_layers_ensure(&depsgraph_mutable,
                                  DEG_get_bmain(&depsgraph),
                                  &ob,
                                  BKE_sculpt_multires_active(&scene, &ob));
    /* Paired with the ensure, exactly as every other mask entry point pairs them
     * (`sculpt_filter_mask.cc`, `sculpt_ops.cc`): creating the Multires layer tags
     * `ID_RECALC_GEOMETRY` and re-evaluates the scene, which leaves the sculpt session pointing at
     * a stale `SubdivCCG` and Paint BVH until this runs. */
    BKE_sculpt_update_object_for_edit(&depsgraph_mutable, &ob, false);
  }
...
static const EnumPropertyItem curve_patch_apply_effect_items[] = {
    ...
    {int(CurvePatchEffectType::Mask), "MASK", 0, "Mask", "Write the sculpt mask"},
...
static wmOperatorStatus curve_patch_apply_exec(bContext *C, wmOperator *op)
{
  ...
  if (is_paint_brush && !SCULPT_use_image_paint_brush(
                            tool_settings.paint_mode, ob, brush, sd.paint.visible_material_channels))
  {
  ...
  /* The sculpt-mask layer this operator may need is NOT ensured here: `curve_patch_apply()` does
   * it once the effect is resolved, so the headless callers that bypass this wrapper are covered
   * too -- and so that a caller asking for RELIEF while a Mask brush happens to be active does not
   * get a `CD_GRID_PAINT_MASK` layer built for nothing. */
  ...
  if (effect_choice < 0) {
    effect_type = curve_patch_effect_type_for_brush(
        *brush, sd.paint, ob, tool_settings.paint_mode);
    if (!effect_type) {
                                                       ...
                                                       const bke::CurvesGeometry &control_curve)
{
  ...
  /* A headless caller has no stroke and no viewport, so there is nothing measured against the
   * surface to fall back on for a curve that spans no plane; the object's own +Z is the only
   * view-independent answer available here. */
  const float3 plane_normal = ed::sculpt_paint::curve_patch_plane_normal_from_curve(
      control_curve, float3(0.0f, 0.0f, 1.0f));
