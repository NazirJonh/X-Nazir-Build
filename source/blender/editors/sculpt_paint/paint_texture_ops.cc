...
static Image *assign_image_get(Main *bmain, wmOperator *op, bool *r_existed, ReportList *reports)
{
  *r_existed = false;
  ...
  if (filepath[0] != '\0') {
    ...
    /* #BKE_image_load_exists takes a loan on the user count regardless of `*r_existed`; release
     * it here so this function always returns a borrowed pointer, matching the data-block branch
     * below (#WM_operator_properties_id_lookup_from_name_or_session_uid takes no loan). */
    id_us_min(&image->id);
    return image;
  }
