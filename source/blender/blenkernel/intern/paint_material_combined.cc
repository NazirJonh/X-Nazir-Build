...
struct CombinedCacheEntry {
  ...
  /**
   * One partial-update subscription per image the inputs read directly, maintained by
   * #bke::paint_material_cache::subscriptions_poll. Layer-stack images are absent by design: their
   * edits arrive as a `changed_region` from the composite that owns them.
   *
   * Being told instead of polling meant being told twice -- once precisely by the stroke, and once
   * as "everything" by the depsgraph flush that follows the same dab -- and the blanket answer
   * won. That turned every dab into a full re-shade of the visible canvas, which measured at
   * ~25 ms a frame on a 2048 canvas against ~4 ms for the dab alone.
   */
...
static void combined_cache_enforce_budget(const uint32_t keep)
{
  bke::paint_material_cache::enforce_budget(
      g_cache.entries, keep, COMBINED_CACHE_BUDGET_BYTES, combined_entry_size_in_bytes);
}
...
static void combined_entry_poll_dependencies(CombinedCacheEntry &entry, Span<Image *> images)
{
  ...
  const bke::paint_material_cache::PollResult poll = bke::paint_material_cache::subscriptions_poll(
      entry.partial_update_users, images, to_output);
