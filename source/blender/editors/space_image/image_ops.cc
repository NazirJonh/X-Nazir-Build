...
static wmOperatorStatus image_rotate_orthogonal_exec(bContext *C, wmOperator *op)
{
  ...
  ED_image_undo_push_begin_with_image(op->type->name, ima, ibuf, &iuser);
  ED_image_undo_capture_selection_mask(ima, tile_number);
  ...
  ibuf->userflags |= IB_DISPLAY_BUFFER_INVALID;
  BKE_image_mark_dirty(ima, ibuf);
  /* 90/270 degree rotations swap width and height, and even 180 degrees permutes every pixel's
   * position, so the mask cannot remain valid at its old layout; drop it (mirrors the resize
   * operator's handling of the same invariant). */
...
void IMAGE_OT_crop_selection(wmOperatorType *ot)
{
  ...
  /* Image undo is pushed manually; OPTYPE_UNDO would add a second memfile step.
   * Neighbors that also use #ED_image_undo_push_begin_with_image (flip/invert/resize) are
   * OPTYPE_REGISTER only. */
  ot->flag = OPTYPE_REGISTER;
}
...
static wmOperatorStatus image_scale_exec(bContext *C, wmOperator *op)
{
  ...
  if (!is_scaling_all) {
    ...
    BKE_image_mark_dirty(ima, ibuf);
    /* The mask is sized to the old resolution and cannot remain valid; drop it rather than
     * leave it mismatched with the new buffer dimensions (see #BKE_image_paint_selection_mask_get
     * invariant that a mask always matches its tile's ImBuf). */
    BKE_image_paint_selection_mask_tile_free(ima, tile_number);
    BKE_image_release_ibuf(ima, ibuf, nullptr);
    ED_image_undo_push_end();
  }
    ...
    for (ImageTile &current_tile : ima->tiles) {
      ...
      ED_image_undo_capture_selection_mask(ima, current_tile.tile_number);
      ibuf->userflags |= IB_DISPLAY_BUFFER_INVALID;
      IMB_scale(ibuf, size[0], size[1], IMBScaleFilter::Box, false);
      BKE_image_mark_dirty(ima, ibuf);
      /* See the single-tile branch above: the mask cannot remain valid at the old resolution. */
      BKE_image_paint_selection_mask_tile_free(ima, current_tile.tile_number);
      BKE_image_release_ibuf(ima, ibuf, nullptr);
    }
    ED_image_undo_push_end();
  }
