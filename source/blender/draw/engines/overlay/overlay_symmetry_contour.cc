                              ...
                              float3 &r_end)
{
  ...
  /* Full signed distance rather than an offset along #PlaneParams.axis: a shared multi-object
   * symmetry plane carried into this object's space (see #transform_plane_params) is no longer
   * axis-aligned. For the axis-aligned case the two agree exactly, since the extra terms are
   * weighted by an exactly-zero normal component. */
...
/**
 * Carry a plane built by #build_plane_params from the space it was defined in into the object's
 * local space. Used by multi-object sculpt, where every mesh mirrors across one shared plane
 * (the active object's axes, world axes, or the 3D cursor - see #Paint.symmetry_space) rather
 * than across its own local axes.
 *
 * The tolerance fields are left alone: they are already scaled by the *drawn* object's size,
 * which is the space they are consumed in.
 */
PlaneParams transform_plane_params(PlaneParams plane, const float4x4 &to_object_local)
{
  plane.point = math::transform_point(to_object_local, plane.point);
  /* In-plane directions transform directly; the normal is perpendicular and needs the
...
void SymmetryContour::begin_sync(Resources &res, const State &state)
{
  ...
  /* #contour_lines_ is intentionally not cleared here: it is retained across frames so an
   * unchanged frame (camera-only navigation) can reuse it without re-emitting or re-uploading.
   * #update_contours clears it when a rebuild is required; #end_sync clears it when the frame
   * produced nothing. */
    ...
    const bool has_dirty_nodes) const
{
   ...
   * without tripping `has_dirty_nodes`. Left undetected, #ObjectCache.segments_by_axis would keep
  ...
  const bool topology_changed = pbvh != nullptr && pbvh_nodes_num != cache.pbvh_nodes_num;
  ...
  const bool positions_changed = pbvh != nullptr && positions_count != cache.positions_count;

  /* The cached segments are triangle/plane intersections, so any move of the plane itself - a
   * symmetry axis toggled, or the shared multi-object symmetry space changed - invalidates all of
   * them, exactly like a change of object used to. */
  const bool plane_changed = symmetry_flags != cache.symmetry_flags ||
                             symmetry_space_to_object != cache.symmetry_space_to_object;
  ...
  decision.plane_changed = plane_changed;
  decision.need_regenerate = cache.contours_dirty || enabled_ != prev_enabled_ || plane_changed ||
                                      ...
                                      const float4x4 *symmetry_space_to_object)
{
  ...
  if (pbvh != nullptr) {
    /* Unlike the tree's own transient bounds-dirty mask - already cleared by the brush's
     * #flush_bounds_to_parents call by the time this overlay runs, on essentially every stroke
     * step - this stays precise even mid-stroke. */
    dirty_nodes = pbvh->consume_external_positions_dirty(dirty_memory);
    has_dirty_nodes = !dirty_nodes.is_empty();
  }

  const RegenDecision decision = compute_regen_decision(
      ob, cache, symmetry_flags, plane_to_object, pbvh, has_dirty_nodes);
  ...
  if (!decision.need_regenerate) {
    const float depth_bias = depth_bias_get(cache.bounds, object_to_world);
    for (const ContourLoop &loop : cache.contours) {
  ...
  /* A Mesh paint BVH is built from the ORIGINAL mesh (#build_pbvh_from_regular_mesh), so its node
   * face indices - and the positions #resolve_positions returns for it - are indexed by the
   ...
   * interactive frames instead of freezing on one. Since #search_nodes already visits only the
   * thin slab of nodes near the plane, the per-frame fixed cost of resuming is small. A live
   * incremental stroke keeps the smaller budget so a single dab never hitches. */
  if (decision.reset_cache) {
    cache.filling = true;
  }
  const double recompute_time_budget_ms = cache.filling ? 20.0 : 4.0;
  ...
  if (decision.reset_cache) {
    for (int axis = 0; axis < 3; axis++) {
      ...
      cache.segments_by_axis[axis].clear_and_keep_capacity();
    }
  }
  ...
  for (int axis = 0; axis < 3; axis++) {
    ...
    PlaneParams plane = build_plane_params(axis, diag);
    if (symmetry_space_to_object != nullptr) {
      plane = transform_plane_params(plane, plane_to_object);
    }
    ...
    if (pbvh != nullptr) {
      Map<int, Vector<ContourSegment>> &axis_cache = cache.segments_by_axis[axis];
       ...
       * by #search_nodes above, so every node here is a candidate: it is either recomputed
       * (subject to the per-frame time budget) or re-used from the cache. Only the leaf-node type,
       * the grain size and the extraction callback differ between Mesh, BMesh and Grids;
       * `extra_recompute` forces recompute regardless of the dirty state (used by Grids when the
       * CCG coordinates are dirty). */
      auto process_nodes =
          [&](auto nodes, const int64_t grain_size, const bool extra_recompute, auto &&extract) {
                ...
                node_indices.index_range(), grain_size, [&](const IndexRange range) {
                  ...
                  for (const int n : node_indices.as_span().slice(range)) {
                    ...
                    if (node_needs_recompute(n) || extra_recompute) {
                      /* Once this frame's recompute budget is spent, leave the remaining nodes for
                       * the next frame (marking the contour dirty). This throttles both a live
                       * incremental stroke and the initial full build, which for a heavy mesh is
                       * otherwise a second-long freeze; #ObjectCache.filling raised the budget so
                       * that fill still completes in a handful of interactive frames rather than
                       * dozens. */
    ...
    cache.contours.extend(std::move(axis_loops));
  }

  /* Recompute next frame if the per-frame budget left dirty nodes unprocessed. */
  cache.contours_dirty = pending_dirty;
  ...
  cache.symmetry_flags = symmetry_flags;
  cache.symmetry_space_to_object = plane_to_object;
  cache.pbvh_nodes_num = decision.pbvh_nodes_num;
  cache.positions_count = decision.positions_count;
...
void SymmetryContour::end_sync(PassSimple::Sub &pass)
{
  ...
  /* No object contributed this frame (e.g. symmetry disabled on the mesh, or the object left
   * sculpt mode) - the retained buffer is now stale, so drop it rather than redraw it. */
  if (frame_object_count_ == 0) {
    contour_lines_.clear();
    lines_valid_ = false;
  }
  /* Drop the retained caches of objects that no longer sync (left sculpt mode, were deleted, had
   * their symmetry turned off). Their per-node segment cache is the memory-heavy part. */
  object_caches_.remove_if(
      [&](auto item) { return item.value.last_seen_frame != frame_counter_; });
  pass.shader_set(contour_shader_);
  pass.push_constant("contour_width", line_thickness_);
  /* Skip the GPU re-upload when every synced object reused its buffer (nothing was re-emitted);
   * the retained buffer on the GPU is still exactly what would be uploaded. */
...
{
  /* The per-node segment caches dominate: each object holds a #ContourSegment vector for every
   * PBVH leaf near a symmetry plane, for each enabled axis. Dropping the whole #object_caches_ map
   * frees that storage outright, unlike the #clear_and_keep_capacity used between rebuilds of a
   * cache that is about to be refilled. The line buffer only drops its item count
   * - the GPU wrapper offers no way to release the allocation - but it is small next to the cache
   * and nothing is re-uploaded while the overlay stays off. */
...
SymmetryPlanePlacement symmetry_plane_placement(const int axis,
                                                const float4x4 *symmetry_space_to_object)
{
  /* The tolerances #build_plane_params derives from the object size are irrelevant here, only the
   * placement is, so the size argument is arbitrary. */
  PlaneParams plane = build_plane_params(axis, 1.0f);
  if (symmetry_space_to_object != nullptr) {
    plane = transform_plane_params(plane, *symmetry_space_to_object);
  }
  return {plane.point, plane.tangent, plane.bitangent};
}
                                         ...
                                         const float4x4 *symmetry_space_to_object)
{
  ...
  contour_.update_contours(ob, symmetry_flags, state, edit_bm, symmetry_space_to_object);
}
...
void SymmetryContourOverlay::draw_line(Framebuffer & /*framebuffer*/, Manager &manager, View &view)
{
  ...
  /* Target the depth-less line frame-buffer (shares `line_tx` so post-AA still applies) rather
   * than the caller's line frame-buffer: the latter has the scene depth attached, which cannot be
   * bound as `scene_depth_tx` at the same time. Occlusion is done in the fragment shader instead.
