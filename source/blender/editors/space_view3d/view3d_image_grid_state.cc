...
static Map<const View3D *, ImageGridStatesPerView3D> g_image_grid_states;
                                                    ...
                                                    const ImageGridView3DDNAFields &dna)
{
  ...
  for (ImageGridLibraryCatalogState *libcat_state = static_cast<ImageGridLibraryCatalogState *>(
           dna.library_catalog_states.first);
       libcat_state;
  ...
  if (state.filter.enabled_catalogs_by_library.is_empty()) {
    ...
    for (AssetCatalogPathLink *path_link = static_cast<AssetCatalogPathLink *>(
             dna.legacy_enabled_catalog_paths.first);
         path_link;
         path_link = path_link->next)
    {
      if (path_link->path && path_link->path[0] != '\0') {
...
{
  const ImageGridView3DDNAFields dna = image_grid_view3d_dna_fields(
      const_cast<View3D &>(v3d), is_mask_slot);
...
{
  ImageGridStatesPerView3D &per_v3d = g_image_grid_states.lookup_or_add_default(&v3d);
