...
TEST(WarpBilinear, EvalIdentitySquareReturnsUV)
{
  ...
  for (const float2 uv : {float2(0.0f, 0.0f),
                          float2(1.0f, 0.0f),
                          float2(0.0f, 1.0f),
                          float2(1.0f, 1.0f),
                          float2(0.3f, 0.7f)})
...
TEST(WarpSmooth, ReproducesLinearGridExactly)
{
  ...
  for (const float2 g :
       {float2(0.5f, 0.5f), float2(0.1f, 2.7f), float2(2.9f, 0.05f), float2(1.5f, 1.5f)})
  {
...
TEST(WarpSmooth, IsC1ContinuousAcrossCellSeam)
{
  ...
  const float2 left = (warp_grid_eval_smooth(pts, grid_size, 2.0f, gy) -
                       warp_grid_eval_smooth(pts, grid_size, 2.0f - h, gy)) /
                      h;
  const float2 right = (warp_grid_eval_smooth(pts, grid_size, 2.0f + h, gy) -
                        warp_grid_eval_smooth(pts, grid_size, 2.0f, gy)) /
                       h;
