 ...
 * \ingroup edsculpt
 *
 * Paint-curve ID operators (add/select/delete/convert). Slide lives in `paint_curve_slide.cc`.
 */
...
/** True when the standalone Curve Edit tool is the active tool. */
static bool paintcurve_edit_tool_is_active(bContext *C)
{
  const bToolRef *tref = paintcurve_tool_ref_from_context(C);
  return tref && STREQ(tref->idname, PAINT_CURVE_EDIT_TOOL_IDNAME);
}
...
bool paint_curve_poll(bContext *C)
{
  ...
  if (paintcurve_edit_tool_is_active(C)) {
    return true;
  }
...
/* Overlay snap marker. Stored here (not on a modal's custom-data) because the Overlay engine
 * cannot reach operator state; both #PAINTCURVE_OT_slide and #SCULPT_OT_curve_patch_edit publish
 * hits through this slot. */
                                 ...
                                 const bool snap_to_surface = false)
{
  ...
  /* A newly created curve in the Image Editor stores region pixel coordinates. The generic
   * PaintCurve constructor defaults to object space for 3D sculpting, which would otherwise make
   * the first point appear to be 3D data and route subsequent editing through View3D projection.
   */
  if (CTX_wm_space_image(C) && pc->geometry.wrap().points_num() == 0) {
    pc->use_3d_space = false;
  }
                                              ...
                                              float r_obj_delta[3])
{
  ...
  sub_v3_v3v3(r_obj_delta, obj_curr, obj_init);
}
...
{
  for (int h = 0; h < 3; h++) {
    paintcurve_geom_co(geom, point_index, h) = initial_loc_3d[h] + obj_delta;
  }
}
...
{
  const float3 obj_delta = hit_obj - initial_loc_3d[1];
  paintcurve_geom_set_surface_normal(geom, point_index, hit_no_obj);
  paintcurve_apply_point_translate_3d(geom, point_index, initial_loc_3d, obj_delta);
  geom.calculate_bezier_auto_handles();
  geom.calculate_bezier_aligned_handles();
}
...
void paintcurve_apply_segment_move_to_point(bke::CurvesGeometry &geom,
                                            const int point_i1,
                                            const int point_i2,
...
{
  /* The solve itself is space-agnostic; only the target point needs a 3D viewport to unproject
   * the cursor onto the segment's depth plane. Flat-canvas callers (2D Curve Patch) pass their
   * target directly to #paintcurve_apply_segment_move_to_point instead. */
  float3 target;
  paintcurve_screen_to_object(vc, depth_world, world_to_ob, mval, target);
  paintcurve_apply_segment_move_to_point(geom, point_i1, point_i2, segment_t, target);
}
                                                ...
                                                const float edge_t)
{
  ...
  /* The shift loop below only covers the built-in attributes it names, so surface normals are
   * moved separately -- exactly as #paintcurve_geometry_add_point does. Without this every point
   * from the insertion onwards keeps its predecessor's normal, which surface snapping and the
   * patch's surface wrap then read. Gathered before the resize and written back after: adding an
   * attribute while the mutable spans below are alive would invalidate them. */
  Vector<float3> normals_shifted;
  normals_shifted.reserve(old_tot + 1);
  for (const int i : IndexRange(old_tot)) {
    normals_shifted.append(paintcurve_geom_get_surface_normal(geom, i));
  }
  /* The new point sits between its two neighbors, so its normal does too. */
  normals_shifted.insert(
      insert_index,
      math::interpolate(normals_shifted[segment_index], normals_shifted[point_i2], edge_t));
  ...
  /* Index of the segment's far end after the shift above. On the closing segment of a cyclic
   * spline that end is the spline's FIRST point, which sits before the insertion and therefore
   * does not shift; assuming `insert_index + 1` there would write into whatever point follows
   * the spline -- the first control point of the next spline, if any. */
  const int point_i2_shifted = point_i2 >= insert_index ? point_i2 + 1 : point_i2;
  ...
  if (segment_is_vector) {
    ...
    hl_w[point_i2_shifted] = inserted.handle_next;
    tl_w[point_i2_shifted] = BEZIER_HANDLE_VECTOR;
  }
  else {
    ...
    hl_w[point_i2_shifted] = inserted.handle_next;
    tl_w[point_i2_shifted] = paintcurve_aligned_or_free_handle_type(type_next_left);
  }

  /* Written back only here: the mutable spans above stay valid because nothing added an attribute
...
void PAINTCURVE_OT_from_curve_object(wmOperatorType *ot)
{
  ...
  /* #ED_paintcurve_import_from_source_object pushes explicitly (`use_undo=true`) and nothing
   * outside the paint curve changes, so #OPTYPE_UNDO would only add an empty second step. */
  ot->flag = OPTYPE_REGISTER;
}
...
void PAINTCURVE_OT_separate_to_curve_object(wmOperatorType *ot)
{
  ...
  /* Deliberately two undo steps, unlike the paint-curve-only operators above: #OPTYPE_UNDO is the
   * only thing that can undo the #Object this operator adds to the scene, while the point removal
   * needs the PAINTCURVE step -- a paint mode's undo stack does not restore the #PaintCurve ID. */
  ot->flag = OPTYPE_REGISTER | OPTYPE_UNDO;
...
static bool paintcurve_sculpt_pick_poll(bContext *C)
{
  ...
  return paintcurve_edit_tool_is_active(C);
}
...
{
  /* `bContext` is taken by const pointer because that is what the transform system holds, while
   * the tool-system lookup below needs a mutable one; every getter it calls is a read, so the
   * cast is safe and keeps the const promise at the boundary where callers see it. */
  return paintcurve_edit_tool_is_active(const_cast<bContext *>(C));
}
