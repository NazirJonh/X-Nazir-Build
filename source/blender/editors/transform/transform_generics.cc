...
void initTransInfo(bContext *C, TransInfo *t, wmOperator *op, const wmEvent *event)
{
  ...
  if (t->spacetype == SPACE_VIEW3D) {
        ...
        (object_mode & (OB_MODE_SCULPT_CURVES | OB_MODE_SCULPT_GREASE_PENCIL)))
    {
      ...
      if (!use_paint_curve) {
        /* The standalone Curve Edit tool edits the paint curve regardless of the active brush
         * stroke method, so transforms must target the curve, not the mesh. Which tools those are
         * is the paint module's business -- asking it keeps the tool's `idname` out of here, where
         * a rename in Python would silently break transforms instead of failing to compile. */
        use_paint_curve = ED_paint_curve_transform_target_poll(C);
      }
      if (use_paint_curve) {
        t->options |= CTX_PAINT_CURVE;
        /* Resolved here and nowhere else -- see #CTX_PAINT_CURVE_3D. */
        const PaintCurve *paint_curve = brush ? brush->paint_curve : nullptr;
        if (paint_curve && paint_curve->use_3d_space) {
          t->options |= CTX_PAINT_CURVE_3D;
        }
      }
    }
    ...
    if (object_mode == OB_MODE_SCULPT) {
      ...
      if (ob && ED_curve_patch_session_get(*ob) != nullptr) {
        /* #CTX_PAINT_CURVE_3D unconditionally: a live Curve Patch's control curve is never
         * screen-bound the way a 2D `PaintCurve` can be -- it always lives in object space. The
         * brush's own `paint_curve` has nothing to do with this session-local curve and may be
         * null or unrelated, so it is deliberately not consulted. */
        t->options |= CTX_PAINT_CURVE | CTX_CURVE_PATCH | CTX_PAINT_CURVE_3D;
      }
    }
    ...
    if (t->options & CTX_MASK) {
      ...
      if (t->spacetype == SPACE_IMAGE) {
        ui::view2d_view_to_region_fl(
            &t->region->v2d, cursor[0], cursor[1], &r_center[0], &r_center[1]);
      }
    }
    else {
...
void calculateCenterMedian(TransInfo *t, float r_center[3])
{
  ...
  const bool is_3d_paint_curve_3d_transform = (t->options & CTX_PAINT_CURVE_3D) &&
