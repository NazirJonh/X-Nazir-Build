...
/**
 * How the N-panel image grid interprets its library/catalog filter relative to the shared
 * image-texture asset shelf (binding §10.3). Recent/Favorites are membership modes — they never
 * store sentinel strings in #ImageGridFilter::enabled_catalog_paths.
 */
enum class ImageGridShelfCatalogMode {
  All,
  CatalogPath,
  Recent,
  Favorites,
...
void image_grid_filter_set_membership(ImageGridUIState &state,
                                      ImageGridShelfCatalogMode membership_mode);
...
int image_grid_apply_focus_scroll(
    const bContext &C, ImageGridUIState &state, int cols, int effective_rows_hint);
