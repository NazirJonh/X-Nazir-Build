...
static wmDropBox *wm_dropbox_active(bContext *C, wmDrag *drag, const wmEvent *event)
{
  ...
  for (ARegion *region = static_cast<ARegion *>(screen->regionbase.last); region;
       region = region->prev)
  {
    /* Null area: these popups hang off the screen, not off an area, and a temporary region never
     * has a panel-category gutter -- the only thing the area is consulted for. */
    if (region->regiontype != RGN_TYPE_TEMPORARY || !region->runtime->visible ||
        !ED_region_contains_xy(nullptr, region, event->xy))
    {
      continue;
    }
