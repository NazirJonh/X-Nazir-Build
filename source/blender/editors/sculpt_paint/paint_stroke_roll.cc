...
void PaintStroke::compute_roll_center(StrokeCache &cache)
{
  ...
  /* Surface interpolation: precompute border curves for the search range. Each polyline vertex
   * gets a binormal (perpendicular to tangent in the view plane) and left/right border points at
   * +/-brush_radius. */
  cache.roll_surface_ready = false;
  {
     ...
     * rasterization is restricted to the dab neighborhood (via eval_row_lo/hi). */
    {
      const int grid_lo = 0;
                                              ...
                                              Vector<float> &r_radii) const
{
  ...
  const bool width_from_pressure = brush && brush->mtex.roll_pressure_scale &&
                                   BKE_brush_use_size_pressure(brush);
