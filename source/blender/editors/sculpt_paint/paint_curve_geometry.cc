...
bke::CurvesGeometry paintcurve_geometry_build_from_selected_points(const bke::CurvesGeometry &geom)
{
  ...
  /* The selection is deliberately not carried over -- but "nothing selected" has to be written
   * out, because an absent selection attribute means everything IS selected. */
  paintcurve_geom_set_all_selection(dst, 0);
...
/** Defined with the rest of the selection helpers further down; used here already. */
static Span<StringRef> paintcurve_selection_attribute_names();
...
void paintcurve_geometry_select_linked(bke::CurvesGeometry &geom)
{
  ...
  /* Deliberately NOT `ed::curves::select_linked()`. That one works one selection layer at a time
   * and stops at the first layer with a hit, so a spline picked by a HANDLE alone comes back with
   * only the handle layer filled -- Transform would then drag a row of handles and leave the
   * control points behind, and `X` would still delete the whole spline. A paint curve treats the
   * three layers as a single packed flag (see the Per-Point Selection Attributes section below),
   * so a hit anywhere in a spline selects that spline whole, handles included. */
  const OffsetIndices<int> points_by_curve = geom.points_by_curve();

  Vector<bke::GSpanAttributeWriter> writers;
  for (const StringRef name : paintcurve_selection_attribute_names()) {
    writers.append(ed::curves::ensure_selection_attribute(
        geom, bke::AttrDomain::Point, bke::AttrType::Bool, name));
  }
  ...
  for (const int curve_i : geom.curves_range()) {
    ...
    for (bke::GSpanAttributeWriter &writer : writers) {
      if (ed::curves::has_anything_selected(writer.span.slice(points))) {
        any_selected = true;
        break;
      }
    }
    ...
    for (bke::GSpanAttributeWriter &writer : writers) {
      ed::curves::fill_selection(writer.span.slice(points), true);
    }
  }
...
bool paintcurve_geometry_any_point_selected(const bke::CurvesGeometry &geom)
{
  ...
  return ed::curves::has_anything_selected(geom, bke::AttrDomain::Point);
}
...
void paintcurve_geometry_init_bezier(bke::CurvesGeometry &geom, const int point_num)
{
  ...
  /* A new paint curve starts with nothing selected, and in the curves selection model that has to
   * be written out: an ABSENT selection attribute means everything IS selected. See the note on
   * the Per-Point Selection Attributes section below. */
  paintcurve_geom_set_all_selection(geom, 0);
  geom.tag_topology_changed();
}
 ...
 *
 * Selection lives in the three attributes every other curves editor uses -- `.selection` for the
 * control point, `.selection_handle_left` / `.selection_handle_right` for the two Bezier handles
 * -- so `ed::curves` operators, Transform and the overlay all read the same data as this module.
 * The packed `uint8_t` these helpers take and return (bit 0 = left, bit 1 = point, bit 2 = right)
 * is a call-site convenience over those three, not a storage format.
 *
 * \note The curves convention is that a MISSING selection attribute means everything is selected
 * (`lookup_or_default(..., true)`; `select_all(SEL_SELECT)` removes the attributes outright). A
 * paint curve starts with nothing selected, which is the opposite -- so
 * #paintcurve_geometry_init_bezier writes an explicit all-false selection rather than leaving the
 * attributes absent. Both readers then agree in every state, including the one `select_all` leaves
 * behind.
 * \{ */

/** Bit position of each packed flag, in the order #paintcurve_geom_get_selection returns them. */
static constexpr uint8_t PAINT_CURVE_SEL_HANDLE_LEFT = 0x01;
static constexpr uint8_t PAINT_CURVE_SEL_POINT = 0x02;
static constexpr uint8_t PAINT_CURVE_SEL_HANDLE_RIGHT = 0x04;

/** The three attributes in packed-bit order. */
static Span<StringRef> paintcurve_selection_attribute_names()
{
  static const std::array<StringRef, 3> names = {
...
uint8_t paintcurve_geom_get_selection(const bke::CurvesGeometry &geom, const int point_index)
{
  ...
  uint8_t flags = 0;
  uint8_t bit = PAINT_CURVE_SEL_HANDLE_LEFT;
  for (const StringRef name : paintcurve_selection_attribute_names()) {
    /* Default true, matching every other reader of these attributes. */
    const VArray<bool> sel = *attrs.lookup_or_default<bool>(name, bke::AttrDomain::Point, true);
    if (sel[point_index]) {
      flags |= bit;
    }
    bit = uint8_t(bit << 1);
                                   ...
                                   const uint8_t flags)
{
  uint8_t bit = PAINT_CURVE_SEL_HANDLE_LEFT;
  for (const StringRef name : paintcurve_selection_attribute_names()) {
    bke::GSpanAttributeWriter sel = ed::curves::ensure_selection_attribute(
        geom, bke::AttrDomain::Point, bke::AttrType::Bool, name);
    sel.span.typed<bool>()[point_index] = (flags & bit) != 0;
    sel.finish();
    bit = uint8_t(bit << 1);
  }
}
...
void paintcurve_geom_set_all_selection(bke::CurvesGeometry &geom, const uint8_t flags)
{
  ...
  uint8_t bit = PAINT_CURVE_SEL_HANDLE_LEFT;
  for (const StringRef name : paintcurve_selection_attribute_names()) {
    /* Written rather than removed even when clearing: absent means SELECTED to every other reader
     * of these attributes (see the note above this section). */
    bke::GSpanAttributeWriter sel = ed::curves::ensure_selection_attribute(
        geom, bke::AttrDomain::Point, bke::AttrType::Bool, name);
    ed::curves::fill_selection(sel.span, (flags & bit) != 0);
    sel.finish();
    bit = uint8_t(bit << 1);
  }
}
