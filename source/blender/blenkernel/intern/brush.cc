...
{
  /* Mask Smooth needs neighboring mask values. Curve Patch currently has no neighborhood-aware
   * smoothing pass, while Mask Draw maps directly to the sampler's per-element value. */
  if (brush.sculpt_brush_type == SCULPT_BRUSH_TYPE_MASK) {
    return brush.mask_tool == BRUSH_MASK_DRAW;
  }
