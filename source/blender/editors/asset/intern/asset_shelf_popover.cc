...
AssetShelf *popup_shelf_get_or_create(const bContext &C, AssetShelfType &shelf_type)
{
  ...
  if (AssetShelf *shelf = lookup_shelf_for_popup(C, shelf_type)) {
    if (const wmWindowManager *wm = CTX_wm_manager(&C)) {
      popup_shelf_sync_per_file_state_from_wm(*wm, *shelf);
    }
    return shelf;
  }
                                             ...
                                             void * /*arg*/)
{
  ...
  popup_shelves_sync_per_file_state_from_wm(*wm);
}

struct PopupShelvesBlendLoadPostCallback {
  bCallbackFuncStore store = {};
  PopupShelvesBlendLoadPostCallback()
  {
    store.func = popup_shelves_on_blend_load_post;
    store.alloc = false;
    BKE_callback_add(&store, BKE_CB_EVT_LOAD_POST);
  }
};
...
class RecentCatalogTreeViewItem : public ui::BasicTreeViewItem {
 public:
  ...
  {
    column.op("ASSETSHELF_OT_asset_recent_clear", IFACE_("Clear Recent"), ICON_NONE);
  }
};
...
class FavoritesCatalogTreeViewItem : public ui::BasicTreeViewItem {
 public:
  ...
  void build_context_menu(bContext & /*C*/, ui::Layout &column) const override
  {
    column.op("ASSETSHELF_OT_asset_favorites_clear", IFACE_("Clear Favorites"), ICON_NONE);
  }
};
...
class AssetCatalogTreeView : public ui::AbstractTreeView {
 ...
 public:
  ...
  void build_tree() override
  {
     ...
     * them. */
    if (shelf::shelf_supports_asset_lists(shelf_.type->idname)) {
...
static void popover_panel_draw(const bContext *C, Panel *panel)
{
  ...
  /* D7: stamp this temporary popup region with the shelf idname so list operators resolve the
   * correct shelf even when button CTX is empty (e.g. cursor in gaps between Favorites tiles). */
  if (ARegion *region = CTX_wm_region_popup(C) ? CTX_wm_region_popup(C) : CTX_wm_region(C)) {
    if (region->regiontype == RGN_TYPE_TEMPORARY) {
      shelf_popup_region_bind(*region, shelf_type->idname);
    }
  }
  ...
  if (ui::block_is_first_open(panel->layout->block())) {
    ...
    BKE_preferences_asset_shelf_popup_view_load(
        &U, shelf->idname, nullptr, nullptr, &width_units, &height_units, nullptr);
    if (const wmWindowManager *wm = CTX_wm_manager(C)) {
