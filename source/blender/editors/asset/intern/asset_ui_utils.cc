...
std::string asset_brush_hotkey(const bContext *C, const asset_system::AssetRepresentation &asset)
{
                   ...
                   const bool include_name)
{
  if (include_name) {
    ...
    if (C && asset.get_id_type() == ID_BR) {
      const std::string hotkey = asset_brush_hotkey(C, asset);
      if (!hotkey.empty()) {
        ui::tooltip_multicolor_text_field_add(tip,
                                              TIP_("Shortcut: "),
                                              hotkey,
                                              ui::TIP_STYLE_NORMAL,
                                              ui::TIP_LC_VALUE,
