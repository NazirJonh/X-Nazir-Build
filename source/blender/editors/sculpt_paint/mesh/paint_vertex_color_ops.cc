        ...
        color_attribute.varray.type().to_static_type<ColorGeometry4f, ColorGeometry4b>(
            [&]<typename T>() {
              ...
              for (const int i : segment) {
                const int vert = (color_attribute.domain == bke::AttrDomain::Point) ? i :
                                                                                       corner_verts[i];

                if (hide_vert[vert]) {
                  continue;
                }
                if (color_attribute.domain == bke::AttrDomain::Corner && hide_poly[corner_to_face[i]]) {
                  continue;
                }
                ...
                if constexpr (std::is_same_v<T, ColorGeometry4f>) {
                  ...
                  ColorGeometry4f color =
                      color::decode(color_attribute.varray.get<ColorGeometry4b>(i));
                  for (const int channel : IndexRange(3)) {
                    color[channel] = interpf(brush_color[channel], color[channel], factor);
                  }
