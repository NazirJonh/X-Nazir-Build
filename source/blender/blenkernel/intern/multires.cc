                                           ...
                                           Vector<Object *> *r_skipped)
{
  ...
  for (Object *ob : candidates) {
    ...
    /* A member is a bystander: it moves as a side effect of an action aimed at `active_ob`, so it
     * must never pay for that with lost work. Two live session states cannot survive the CCG
     * rebuild a level change triggers, and neither can be settled from blenkernel — closing a
     * weight-mask edit is #ed::sculpt_paint::layers::finish_mask_edit, which lives in the editors.
     * Hence refuse to move such a member rather than silently destroying it; the caller reports it
     * by name and the user re-syncs deliberately via #OBJECT_OT_multires_level_sync. Only the
     * detection is needed here, and both states are plain session data. */
    const SculptSession *ss = ob->runtime->sculpt_session;
    if (ss != nullptr && (ss->cache != nullptr || ss->layers.mask_edit.node_uid != 0)) {
      if (r_skipped != nullptr) {
        r_skipped->append(ob);
      }
      continue;
    }
    /* Settle pending base sculpt edits while the live CCG and the stored layer set still agree:
     * the re-evaluation this level change triggers would otherwise flush them against a CCG
     * rebuilt at the new level and leak the difference into the base MDisps. Fully guarded — a
     * no-op unless this object really has a dirty grid session. */
