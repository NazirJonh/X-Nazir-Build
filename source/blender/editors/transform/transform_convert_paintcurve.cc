...
static void createTransPaintCurveVerts(bContext *C, TransInfo *t)
{
  ...
  /* The standard curves selection attributes, read once rather than per point. A MISSING one means
   * everything is selected, which is the convention every curves editor shares; a paint curve
   * writes them out explicitly at creation so "nothing selected" is a real state. */
  const bke::AttributeAccessor attrs = geom.attributes();
  const VArray<bool> sel_left = *attrs.lookup_or_default<bool>(
      ".selection_handle_left", bke::AttrDomain::Point, true);
  const VArray<bool> sel_point = *attrs.lookup_or_default<bool>(
      ".selection", bke::AttrDomain::Point, true);
  const VArray<bool> sel_right = *attrs.lookup_or_default<bool>(
      ".selection_handle_right", bke::AttrDomain::Point, true);
  /* Packed left / point / right bits, the form the loops below were written against. */
  auto selection_bits_at = [&](const int point_index) -> uint8_t {
    return uint8_t(sel_left[point_index] ? 0x01 : 0) | uint8_t(sel_point[point_index] ? 0x02 : 0) |
           uint8_t(sel_right[point_index] ? 0x04 : 0);
  };

  /* Count total handle entries that need transform data. */
  int total = 0;
  for (const int i : geom.points_range()) {
    const uint8_t sel = selection_bits_at(i);
    if (!sel) {
      continue;
    }
    if (sel & 0x02) {
