                       ...
                       [[out]] FragOut &frag_out)
{
  ...
  if (srt.lighting_mode == WORKBENCH_LIGHTING_MATCAP) [[static_branch]] {
    ...
    float specular = WORKBENCH_DEFAULT_SPECULAR;
    ...
    if (srt.use_material_ext) {
      /* Bypass the packed pair entirely for frames that carry the full-precision buffer, so
       * painted Metallic/Roughness/Specular gradients stay smooth instead of banding at
       * #TARGET_BITCOUNT's discrete levels - and so Specular can be per-vertex at all, which the
       * packed alpha has no room for. */
