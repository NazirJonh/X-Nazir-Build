...
static Image *image_grid_resolve_context_image(bContext &C, wmOperator &op)
{
  ...
  if (asset_system::AssetRepresentation *asset = image_grid_asset_from_op(C, op)) {
    ...
    if (!asset->full_path().empty()) {
      if (Image *ima = BKE_image_load_exists(bmain, asset->full_path().c_str())) {
        /* #BKE_image_load_exists takes a loan on the user count regardless of whether the block
         * was newly created or already existed. Release it here so every return path of this
         * function hands back a borrowed pointer, matching the session-UID lookup, the local
         * asset ID, and the context-`id` paths below, none of which take a loan. Without this,
         * repeated invocations on the same on-disk file (e.g. "Assign Catalog", "Copy to
         * Library") would raise `us` by one per call, without bound. */
        id_us_min(&ima->id);
        return ima;
      }
      return nullptr;
...
static bool image_grid_image_source_abs_path(Main *bmain, Image &image, char r_abs[FILE_MAX])
{
...
static void image_grid_visit_catalogs_for_search(
    const bContext *C,
    PointerRNA *ptr,
    PropertyRNA * /*prop*/,
    ...
    FunctionRef<void(StringPropertySearchVisitParams)> visit_fn)
{
  ed::asset::visit_library_catalogs_catalog_for_search(
      *CTX_data_main(C), ed::asset::get_asset_library_ref_from_opptr(*ptr), edit_text, visit_fn);
}

static wmOperatorStatus image_grid_relocate_invoke(bContext *C, wmOperator *op, const char *title)
{
...
static wmOperatorStatus image_grid_drop_import_exec(bContext *C, wmOperator *op)
{
  ...
  RNA_BEGIN (op->ptr, itemptr, "files") {
     ...
     * newly created or already existed. Release it unconditionally: in Library mode below,
     * #image_mark_as_asset (via #mark_id) sets a fake user instead of a real reference, so
     * without this the block would end up with `us == 2` for a single fake user; in Temporary
     * mode nothing else references it at all. */
    id_us_min(&image->id);
    loaded_num++;
...
void IMAGE_GRID_OT_assign_catalog(wmOperatorType *ot)
{
  ...
  ot->idname = "IMAGE_GRID_OT_assign_catalog";
...
/* Join `name` under `catalog_path_str` into a library-relative path, treating the virtual root
 * catalog (#IMAGE_LIBRARY_ROOT_CATALOG_PATH or empty) as the library root with no sub-directory.
 */
static void image_grid_catalog_relative_join(const char *catalog_path_str,
                                             const char *name,
                                             char r_relative[FILE_MAX])
{
  const bool catalog_is_root = catalog_path_str[0] == '\0' ||
                               STREQ(catalog_path_str, ed::asset::IMAGE_LIBRARY_ROOT_CATALOG_PATH);
  if (catalog_is_root) {
    BLI_strncpy(r_relative, name, FILE_MAX);
  }
  else {
    BLI_path_join(r_relative, FILE_MAX, catalog_path_str, name);
  }
}
                                              ...
                                              char r_focus_identifier[FILE_MAX])
{
  ...
  if (!ed::asset::image_library_catalog_directory_ensure(dest_lib.dirpath, catalog.path.c_str())) {
  ...
  image_grid_catalog_relative_join(catalog_path_str, filename, dest_relative);
  ...
  if (BLI_exists(dest_abs)) {
    ...
    BLI_uniquename_cb(
        [&](const StringRef check_name) {
          ...
          image_grid_catalog_relative_join(catalog_path_str, check_name.data(), test_relative);
    ...
    image_grid_catalog_relative_join(catalog_path_str, unique_name, dest_relative);
    BLI_path_join(dest_abs, sizeof(dest_abs), dest_lib.dirpath, dest_relative);
    BLI_path_normalize(dest_abs);
  }
...
static wmOperatorStatus image_grid_copy_move_exec(bContext *C, wmOperator *op, const bool do_move)
{
  ...
  if (asset_system::AssetRepresentation *asset = image_grid_asset_from_op(*C, *op)) {
        ...
        STREQ(library_root, dest_lib->dirpath))
    {
      ...
      if (!ed::asset::image_library_assign_image_to_catalog(
              library_root, *library, asset->library_relative_identifier(), catalog.catalog_id))
      {
      ...
      image_grid_catalog_relative_join(catalog.path.c_str(), focus_id, moved_relative);
...
static void image_grid_copy_move_ot_common(wmOperatorType *ot, const bool do_move)
{
  if (do_move) {
    ...
    ot->idname = "IMAGE_GRID_OT_move_to_library";
    ...
    ot->idname = "IMAGE_GRID_OT_copy_to_library";
  ...
  prop = RNA_def_string(ot->srna,
                        ...
                        MAX_NAME,
                        "Catalog",
                        "Catalog in the destination library");
  RNA_def_property_string_search_func_runtime(
      prop, image_grid_visit_catalogs_for_search, PROP_STRING_SEARCH_SUGGESTION);
