...
VERTEX_SHADER_CREATE_INFO(overlay_sculpt_curves_points)
...
void main()
{
  ...
  float3 base_color = float3(0.8f, 0.8f, 0.8f);
  float3 highlight_color = brush_highlight_color;
  
  if (highlight_factor > 0.0f) {
    finalColor = mix(base_color, highlight_color, highlight_factor * brush_highlight_opacity);
  }
  else {
