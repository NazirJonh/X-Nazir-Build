                                     ...
                                     const float4x4 &transform)
{
  ...
  /* An imported curve arrives with nothing selected, and that has to be written out: the source
   * `Curves` may carry no selection attribute at all, which in the curves model means everything
   * IS selected -- see the Per-Point Selection Attributes note in `paint_curve_geometry.cc`. */
...
bool ED_paintcurve_import_from_source_object(bContext *C, ReportList *reports, const bool use_undo)
{
  ...
  const bool is_curve_edit_tool = tref && STREQ(tref->idname, PAINT_CURVE_EDIT_TOOL_IDNAME);
...
/* Weak reference, exactly like #Sculpt.gravity_object: the pointer is registered #IDWALK_CB_NOP
 * in `scene_foreach_toolsettings()`, its RNA property is not #PROP_ID_REFCOUNT, and
 * `PAINTCURVE_OT_sculpt_pick` assigns it directly. Counting a user only here would leave a
 * dangling count behind on undo or file reload. */
