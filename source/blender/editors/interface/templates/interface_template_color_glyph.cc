 ...
 *
 * Per-button storage for RNA pointer data to be used by the popup menu.
 * \{ */

/* Owning payload passed as the popup button's auto-managed `func_argN`, so each dropdown button
 * carries its own RNA data instead of sharing a single file-static singleton (which could be
 * overwritten when several dropdown templates are drawn in the same frame). */
struct ColorGlyphPopupData {
  PointerRNA ptr;
  PropertyRNA *prop = nullptr;
  char propname[64] = "";
};

/* Free function registered with `uiDefBlockButN` for #ColorGlyphPopupData. */
static void color_glyph_popup_data_free(void *argN)
{
  MEM_delete(static_cast<ColorGlyphPopupData *>(argN));
}

/* Copy function registered with `uiDefBlockButN` for #ColorGlyphPopupData. */
static void *color_glyph_popup_data_copy(const void *argN)
{
  return MEM_new<ColorGlyphPopupData>(__func__, *static_cast<const ColorGlyphPopupData *>(argN));
}
...
static Block *color_glyph_preset_popup_callback(bContext *C, ARegion *region, void *arg)
{
  ...
  /* Get RNA data from the button's payload */
  ColorGlyphPopupData *popup_data = static_cast<ColorGlyphPopupData *>(arg);
  if (!popup_data) {
    layout.label("Error: Color data not available", ICON_ERROR);
    return block;
  }

  PointerRNA *ptr = &popup_data->ptr;
  PropertyRNA *prop = popup_data->prop;
  const char *propname = popup_data->propname;
                                  ...
                                  const char *propname)
{
  ...
  if (use_inline_buttons) {
    ...
    /* Store RNA data in a per-button payload for the popup callback to access */
    ColorGlyphPopupData *popup_data = MEM_new<ColorGlyphPopupData>(__func__);
    popup_data->ptr = *ptr;
    popup_data->prop = prop;
    BLI_strncpy(popup_data->propname, propname, sizeof(popup_data->propname));
    ...
    Button *popup_but = uiDefBlockButN(
        ...
        UI_UNIT_Y,
        std::nullopt,
        color_glyph_popup_data_free,
        color_glyph_popup_data_copy);
