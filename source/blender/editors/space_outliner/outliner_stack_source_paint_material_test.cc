...
TEST_F(OutlinerStackPaintMaterialSourceTest, stack_rows_report_layer_kind)
{
  ...
  fill_params.kind = PaintMaterialLayerKind::Fill;
...
TEST_F(OutlinerStackPaintMaterialSourceTest, material_layer_row_resolves_its_source_material)
{
  ...
  /* The row reads as its material: the material icon, and the material's preview beside the
   * map. */
...
TEST_F(OutlinerStackPaintMaterialSourceTest, row_owns_mask_image_matches_the_masks_row)
{
  Material &material = add_material_with_texture(add_image("Base4"));
  PaintMaterialLayerAddParams params_a;
  params_a.image_size = 8;
  int ordinal_a = -1;
  ASSERT_TRUE(BKE_paint_material_layer_add(bmain, material, params_a, &ordinal_a));
  ...
  /* What row activation checks before leaving mask editing: the edited mask belongs to the row
   * it was opened from, and to no other row. */
  ASSERT_TRUE(paint_material_mask_preview_activate(
      *bmain, *scene, scene->toolsettings->imapaint.paint, *row_a, "MASK"));
  Image *mask_a = scene->toolsettings->paint_mode.mask_image_binding.image;
  ASSERT_NE(mask_a, nullptr);
  EXPECT_TRUE(paint_row_owns_mask_image(*row_a, mask_a));
  EXPECT_FALSE(paint_row_owns_mask_image(*row_b, mask_a));
  EXPECT_FALSE(paint_row_owns_mask_image(*row_a, nullptr));
