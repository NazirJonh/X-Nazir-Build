...
#define U BLI_STATIC_ASSERT(false, "Global 'U' not allowed, only use arguments passed in!")
  ...
  for (bUserAssetBrowserSettings &settings : userdef->asset_browser_settings.items_mutable()) {
    BKE_asset_catalog_state_list_free(settings.catalog_states);
    MEM_delete(&settings);
  }
...
static void userdef_sync_asset_libraries(UserDef *userdef_dst, UserDef *userdef_src)
{
  for (bUserAssetLibrary &library_src : userdef_src->asset_libraries.items_mutable()) {
    ...
    bUserAssetLibrary *library_dst = BKE_preferences_asset_library_find_by_name(userdef_dst,
                                                                                  library_src.name);
    if (library_dst != nullptr) {
      STRNCPY(library_dst->dirpath, library_src.dirpath);
      STRNCPY(library_dst->remote_url, library_src.remote_url);
      library_dst->import_method = library_src.import_method;
      library_dst->flag = library_src.flag;
