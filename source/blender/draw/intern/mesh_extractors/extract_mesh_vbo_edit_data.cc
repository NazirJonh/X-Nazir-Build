...
{
  if (face_set_id == default_id) {
    return uchar4(0);
  }
  uchar4 color(0);
  BKE_paint_face_set_overlay_color_get(face_set_id, seed, color, custom_colors);
  /* The shader uses a zero alpha as the "unassigned Face Set" sentinel, so an assigned Face Set
   * must always be fully opaque regardless of what the color source stored in that channel. */
  color.w = 255;
  return color;
}
...
{
  gpu::VertBufPtr vbo = gpu::VertBufPtr(GPU_vertbuf_create_with_format(get_edit_face_set_format()));
  const int size = mr.corners_num + mr.loose_indices_num;
  GPU_vertbuf_data_alloc(*vbo, size);
  MutableSpan<uchar4> data = vbo->data<uchar4>();
  const uchar4 default_color(0);
  const int default_id = mr.mesh->face_sets_color_default;
  const int seed = mr.mesh->face_sets_color_seed;
  ...
  if (mr.extract_type == MeshExtractType::Mesh) {
    ...
    if (face_sets.is_empty()) {
      ...
      threading::parallel_for(faces.index_range(), 2048, [&](const IndexRange range) {
        for (const int face : range) {
          const uchar4 color = face_set_color_get(
              face_sets[face], default_id, seed, custom_colors);
          data.slice(faces[face]).fill(color);
        }
      });
    ...
    if (offset == -1) {
      ...
      threading::parallel_for(IndexRange(bm.totface), 2048, [&](const IndexRange range) {
        for (const int face_index : range) {
          const BMFace &face = *BM_face_at_index(&const_cast<BMesh &>(bm), face_index);
          const int face_set_id = BM_ELEM_CD_GET_INT(&face, offset);
          const uchar4 color = face_set_color_get(face_set_id, default_id, seed, custom_colors);
          const BMLoop *loop = BM_FACE_FIRST_LOOP(&face);
          for ([[maybe_unused]] const int i : IndexRange(face.len)) {
            data[BM_elem_index_get(loop)] = color;
            loop = loop->next;
          }
        }
      });
                                             ...
                                             const DRWSubdivCache &subdiv_cache)
{
  ...
  const int seed = mr.mesh->face_sets_color_seed;
  const Map<int, uchar4> custom_colors = BKE_paint_face_set_custom_colors_map(mr.mesh);
  const Span<int> loop_faces(subdiv_cache.subdiv_loop_face_index, subdiv_cache.num_subdiv_loops);
  const bke::AttributeAccessor attributes = mr.mesh->attributes();
  const VArraySpan<int> face_sets = *attributes.lookup<int>(".sculpt_face_set",
                                                            bke::AttrDomain::Face);
  if (face_sets.is_empty()) {
    ...
    threading::parallel_for(IndexRange(subdiv_cache.num_subdiv_loops), 4096,
                             [&](const IndexRange range) {
      for (const int i : range) {
        const int face_set_id = face_sets[loop_faces[i]];
        data[i] = face_set_color_get(face_set_id, default_id, seed, custom_colors);
      }
    });
