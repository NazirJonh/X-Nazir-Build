                                                   ...
                                                   const MaterialBakeToImagesParams &params)
{
  ...
  /* Deduplicated per channel for new maps; an explicit target is its own entry, so two layers baked
...
/**
 * Collect the maps of \a ma to re-bake: those \a wanted selects, plus every map of \a ma whose bake
 ...
 */
static void rebake_targets_collect(Main &bmain,
                                   const Material &ma,
                                   const FunctionRef<bool(const Image &, const ImageMaterialSource &)>
                                       wanted,
                                   ...
                                   bool &r_all_pending)
{
  ...
  for (Image &image : bmain.images) {
    ...
    if (!BKE_image_material_source_get(image, source) || source.material != &ma ||
        !ID_IS_EDITABLE(&image.id) || BKE_image_material_source_parked_get(image))
    {
      /* A parked map belongs to a switched-off channel; it is brought up to date when switched
       * back on, not on every edit of the material while nothing shows it. */
      continue;
    }
    bool pending;
    {
