...
void file_draw_list(const bContext *C, ARegion *region)
{
  ...
  if (numfiles < 1) {
    ...
    const int line_x = tile_draw_rect.xmin + UI_UNIT_X;
    const int line1_y = tile_draw_rect.ymax - UI_UNIT_Y;

    ui::fontstyle_draw_simple(&style->widget, line_x, line1_y, message, text_col_mod);

    /* For Asset Browser with no search filter, if filtering by ID types or Name Match map types
     * is active, show the active filter types and their icons on the next line. */
    if (filelist_is_ready(files) && !is_filtered && layout->is_asset_browser) {
      struct FilterTypeItem {
        int icon;
        std::string name;
      };
      Vector<FilterTypeItem> active_filters;

      const uint64_t all_filter_flags = (FILTER_ID_BR | FILTER_ID_MA | FILTER_ID_GR |
                                         FILTER_ID_OB | FILTER_ID_AC | FILTER_ID_WO |
                                         FILTER_ID_NT | FILTER_ID_SCE | FILTER_ID_IM);
      const bool has_id_filter = params->filter_id != 0 &&
                                 (params->filter_id & all_filter_flags) != all_filter_flags;

      if (has_id_filter) {
        for (uint idx = 0; rna_enum_id_type_filter_items[idx].identifier; idx++) {
          const IDFilterEnumPropertyItem *item = &rna_enum_id_type_filter_items[idx];
          if ((params->filter_id & item->flag) != 0) {
            active_filters.append({item->icon, IFACE_(item->name)});
          }
        }
      }

      FileAssetSelectParams *asset_params = ED_fileselect_get_asset_params(sfile);
      if (asset_params && (asset_params->asset_flags & FILE_ASSET_FILTER_NAME_MATCH_ENABLED) &&
          BLI_listbase_head_is_plausible(&asset_params->filter_name_match_map_types))
      {
        for (const AssetNameMatchIdLink *link = static_cast<const AssetNameMatchIdLink *>(
                 asset_params->filter_name_match_map_types.first);
             link;
             link = link->next)
        {
          if (STREQ(link->id, "MASK")) {
            active_filters.append({ICON_MOD_MASK, IFACE_("Mask")});
          }
          else if (STREQ(link->id, "ALPHA")) {
            active_filters.append({ICON_IMAGE_ALPHA, IFACE_("Opacity / Alpha")});
          }
        }
      }

      if (!active_filters.is_empty()) {
        const int line_height = sfile->layout ? sfile->layout->text_line_height : UI_UNIT_Y;
        const int line2_y = line1_y - int(line_height * 1.3f);
        const char *filter_prefix = IFACE_("Filtered by: ");
        int curr_x = line_x;

        ui::fontstyle_draw_simple(
            &style->widget, curr_x, line2_y, filter_prefix, text_col_mod);
        curr_x += ui::fontstyle_string_width(&style->widget, filter_prefix);

        for (int f_idx = 0; f_idx < active_filters.size(); f_idx++) {
          const FilterTypeItem &filter_item = active_filters[f_idx];

          file_add_overlay_icon_but(block, curr_x, line2_y - UI_UNIT_Y / 4, filter_item.icon);
          curr_x += ICON_DEFAULT_WIDTH_SCALE + int(0.25f * UI_UNIT_X);

          ui::fontstyle_draw_simple(
              &style->widget, curr_x, line2_y, filter_item.name.c_str(), text_col_mod);
          curr_x += ui::fontstyle_string_width(&style->widget, filter_item.name.c_str());

          if (f_idx < active_filters.size() - 1) {
            const char *sep = ", ";
            ui::fontstyle_draw_simple(&style->widget, curr_x, line2_y, sep, text_col_mod);
            curr_x += ui::fontstyle_string_width(&style->widget, sep);
          }
        }
      }
    }
