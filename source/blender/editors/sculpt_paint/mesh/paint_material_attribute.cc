 ...
 *
 * Fixed channels are resolved through #BKE_paint_material_channel_attribute_name, the same
 * override-aware path painting itself uses, so Add/Remove always target whatever attribute a
 * stroke would actually write to (its fixed name, or an add-on's
 * #PaintModeSettings::channel_layer_bindings redirect). Only the Custom channel, which has no
 * fixed name of its own, falls back to the operator's `name` property.
 */
...
static wmOperatorStatus material_attribute_add_exec(bContext *C, wmOperator *op)
{
  ...
  bool created = false;
  MaterialPaintAttributeStatus status;
  if (info.is_color) {
    /* A redirected channel must ensure its actual (layer) attribute and must not become the
     * mesh's active/default color - #BKE_paint_mesh_material_color_attribute_ensure_named's
     * contract - while the unredirected case keeps the fixed-channel function so a newly created
     * Base Color attribute still becomes active/default, as it always has. */
    const Scene *scene = CTX_data_scene(C);
    const bool redirected = scene && scene->toolsettings &&
                            scene->toolsettings->paint_mode.channel_layer_bindings[channel]
                                    .attribute_name[0] != '\0';
    status = redirected ?
                 BKE_paint_mesh_material_color_attribute_ensure_named(*mesh, attr_name, &created) :
                 BKE_paint_mesh_material_color_attribute_ensure(*mesh, channel, &created);
  }
  else {
    status = BKE_paint_mesh_material_attribute_ensure(*mesh, attr_name, &created);
  }
