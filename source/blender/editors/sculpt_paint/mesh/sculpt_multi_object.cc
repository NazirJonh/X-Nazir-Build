                                                 ...
                                                 const float3 &cursor_world)
{
  ...
  /* The view axis travels with the center: a mirrored daub's brush volume is the mirror image of the
   * original, axis included. Carried as a DIRECTION (matching how #view_normal is transformed
   * everywhere else in this module -- see #multi_object_area_sample_active), so the symmetry frame's
   * translation is ignored and any scale it carries cancels across the round trip. */
...
static bool mirror_snap_enabled(const Paint &paint, const Brush &brush)
{
   ...
   * the daub center doubles as the cylinder axis origin -- moving it would slide the projected disc
   * sideways instead of correcting depth. */
                                     ...
                                     StrokeCache &cache)
{
   ...
   * surface -- so the mirrored daub lands where a mirrored cursor would have hit this mesh, and only
   * its depth is corrected: its position across the view (the mirror's own lateral answer) is
   * preserved exactly. #StrokeCache.view_normal_symm is the mirrored viewer's direction, recomputed
   * for this pass by #cache_calc_brushdata_symm immediately before this call; it is fixed for the
   * whole pass and does not depend on the surface at all.
   *
   ...
   * as it sculpts). The daub center then lurches by a good fraction of a brush radius between steps
   * -- and the depth/lateral budget tests flicker in and out with it -- so the mirrored stroke comes
   * out as a chain of separate craters instead of a line. A ray along a fixed axis has no such
   * feedback loop, and being a true ray-surface intersection it is not quantized to vertex spacing
   * either.
   *
   * The cast uses the DEFORMED (current) surface, which is what the per-vertex falloff of the pass
   * about to run measures for most brushes. Grab, Rotate, Thumb and Elastic Deform measure against
   * stroke-start positions instead, but #restore_from_undo_step_if_necessary has already restored
   * them by the time this runs, so their current geometry IS the stroke-start geometry.
   *
   * KNOWN LIMITATION: Draw Sharp, Layer and Topology Slide measure their falloff against
   * stroke-start positions AND are not restored each step, so on a long stroke the snapped center
   * tracks the deformed surface while their falloff still measures the original one. The drift is
   * bounded by how far the brush itself has displaced the surface, so it shrinks the mirrored daub
  ...
  /* #last_location_symm is only ever read as `location_symm - last_location_symm` (stroke direction
   * for Cloth, Smear, Topology Slide, color paint). Shifting it by the SAME delta leaves that
   * difference bit-identical, so the snap changes where the daub sits without changing which way
   * the stroke is going.
   *
...
SharedStrokeStateSnapshot capture_shared_stroke_state(const Object &primary_ob)
{
  ...
  if (ss && ss->cache) {
     ...
     * leave it alone, so it still holds the main-pass value here (after all of the primary's passes
     * have run). Carry it in world space: the secondaries it is handed to have their own, possibly
     * unapplied, transforms. Normals transform by the inverse transpose. Only ever read by a
     * secondary object, so skip the matrix work entirely for single-object strokes. */
                                   ...
                                   const SharedStrokeStateSnapshot &primary_state)
{
  ...
  if (primary_state.sculpt_normal_world) {
     ...
     * basis. Seeded unconditionally and every step: this runs BEFORE the object's own passes, so if
     * its main pass does produce a normal that computation still wins, and if it does not (main daub
     * missing, or covering only masked/hidden geometry) the mirror pass has a usable one instead of
     * zero. Re-seeding each step also keeps the mirrored daub tracking the surface as the stroke
     * moves, exactly as it would on a joined mesh. */
                                                 ...
                                                 const Span<MirroredDaub> symm_daubs) const
{
  ...
  if (!has_location && this->shared_symmetry_active) {
     ...
     * surface, so widen the test by the same amount -- otherwise the object is rejected here before
     * the snap ever runs. The multiplier is 1.0 when the snap is inactive, and multiplying the
     * radius by 1.0 is exact, so the gate then tests what it always did.
     *
     ...
     * (It is the same center, round-tripped through the reference-space transforms, so re-testing it
     * here was redundant even before the snap.) */
