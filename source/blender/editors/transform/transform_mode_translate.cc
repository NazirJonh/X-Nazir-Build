...
static void ApplySnapTranslation(TransInfo *t, float vec[3])
{
  ...
  if (t->spacetype == SPACE_SEQ) {
    ...
    /* The spacetype test stays: #CTX_PAINT_CURVE is set for the Image Editor too, and only a 2D
     * paint curve IN a 3D viewport needs this projection. */
    if (t->spacetype == SPACE_VIEW3D && (t->options & CTX_PAINT_CURVE) &&
        (t->options & CTX_PAINT_CURVE_3D) == 0)
    {
...
static void applyTranslation(TransInfo *t)
{
  ...
  if (t->flag & T_INPUT_IS_VALUES_FINAL) {
    ...
    if (!paint_curve_geom_snap && !(transform_snap_is_active(t) && validSnap(t)) &&
...
static void initTranslation(TransInfo *t, wmOperator * /*op*/)
{
  ...
  const bool is_3d_paint_curve_translation = (t->options & CTX_PAINT_CURVE_3D) != 0;
