 ...
 * \ingroup edsculpt
 *
 * Session-local undo for a live Curve Patch edit. See `CurvePatchDocument::undo_steps` for
...
void curve_patch_undo_push(CurvePatchSession &patch)
{
  ...
  patch.doc.undo_steps.resize(patch.doc.undo_step_current + 1);

  CurvePatchEditStep step;
  step.items.reserve(patch.doc.patches.size());
  for (const CurvePatchItem &item : patch.doc.patches) {
    step.items.append({item.control_curve, item.params});
  }
  step.active_patch = patch.doc.active_patch;
  patch.doc.undo_steps.append(std::move(step));

  if (patch.doc.undo_steps.size() > CURVE_PATCH_UNDO_STEPS_MAX) {
    patch.doc.undo_steps.remove(0);
    ...
    patch.doc.undo_anchor_trimmed = true;
  }
  patch.doc.undo_step_current = int(patch.doc.undo_steps.size()) - 1;
}

static void curve_patch_undo_restore(bContext &C, Object &ob, CurvePatchSession &patch)
{
  const CurvePatchEditStep &step = patch.doc.undo_steps[patch.doc.undo_step_current];
  ...
  patch.doc.patches.resize(step.items.size());
  for (const int i : step.items.index_range()) {
    patch.doc.patches[i].control_curve = step.items[i].curve;
    patch.doc.patches[i].params = step.items[i].params;
  }
  patch.doc.active_patch = std::min(step.active_patch, int(patch.doc.patches.size()) - 1);
  /* The restored curve may hold fewer points than the one just replaced. */
  patch.doc.active_point = -1;
 ...
 *
 * When the history was trimmed (#CurvePatchDocument::undo_anchor_trimmed), index 0 is some
...
{
  if (patch.doc.undo_step_current <= 0) {
    return patch.doc.undo_anchor_trimmed;
  }
  patch.doc.undo_step_current--;
...
{
  if (patch.doc.undo_step_current + 1 >= int(patch.doc.undo_steps.size())) {
    return;
  }
  patch.doc.undo_step_current++;
  curve_patch_undo_restore(C, ob, patch);
