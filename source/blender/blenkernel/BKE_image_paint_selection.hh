 ...
 *
 * \note Like the mutable #BKE_image_paint_selection_mask_lookup overload, this conservatively
 * bumps the mask revision: every caller of this function writes into the buffer it returns.
 *
...
/**
 * Move the selection mask (and its derived blend-mask cache) of \a old_tile_number to
 * \a new_tile_number. Any mask already present at \a new_tile_number is discarded first. Does
 * nothing when \a old_tile_number has no mask, or when the numbers are equal.
 */
