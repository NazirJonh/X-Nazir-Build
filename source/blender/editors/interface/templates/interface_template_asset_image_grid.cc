...
class ImageAssetGridItem : public PreviewGridItem {
 ...
 public:
  ...
  static bool image_matches_asset(const Image &image,
...
class ImageAssetGridView : public AbstractGridView {
 ...
 public:
  ...
  void build_items() override
  {
    ...
    const int cols = state_.viewport.cached_cols > 0 ? state_.viewport.cached_cols : cols_hint_;
                             ...
                             const bool is_mask_slot)
{
  ...
  const int cols_est = (state.viewport.cached_cols > 0) ?
  ...
  state.viewport.grip_pixel_height = clamp_i(
      state.viewport.grip_pixel_height, tile_h, 16 * tile_h);
  const int effective_rows = clamp_i(
      round_fl_to_int(float(state.viewport.grip_pixel_height) / float(tile_h)), 1, 16);
  state.viewport.grip_pixel_height = effective_rows * tile_h;
